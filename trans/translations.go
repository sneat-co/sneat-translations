...
我们的目标是尽可能透明和值得信赖。`},
	...
	ShareSpotInlineDescription: {
		arEG: "{LANGUAGE}: انقر هنا لمشاركة رابط الموقع للدردشة الحالية.",
		deDE: "{LANGUAGE}: Klicken Sie hier, um den Spot-Link zum aktuellen Chat freizugeben.",
		enUK: "{LANGUAGE}: Click this to share spot link to the current chat.",
		esES: "{LANGUAGE}: Haga clic aquí para compartir el enlace al chat actual.",
		faIR: "{LANGUAGE}: برای اشتراک‌گذاری لینک گفتگوی فعلی، روی این کلیک کنید.",
		frFR: "{LANGUAGE}: Cliquez ici pour partager le lien du spot vers la discussion en cours.",
		idID: "{LANGUAGE}: Klik ini untuk membagikan tautan spot ke obrolan saat ini.",
		itIT: "{LANGUAGE}: Fai clic qui per condividere il collegamento alla chat corrente.",
		jaJP: "{LANGUAGE}: 現在のチャットへのスポットリンクを共有するには、これをクリックします。",
		koKR: "{LANGUAGE}: 현재 채팅에 스팟 링크를 공유하려면 여기를 클릭하세요.",
		plPL: "{LANGUAGE}: Kliknij tutaj, aby udostępnić link do bieżącego czatu.",
		ptBR: "{LANGUAGE}: Clique aqui para compartilhar o link do chat atual.",
		ptPT: "{LANGUAGE}: Clique aqui para partilhar o link do chat atual.",
		ruRU: "{LANGUAGE}: Нажмите тут чтобы поделиться ссылкой на спот в текущем чате.",
		trTR: "{LANGUAGE}: Mevcut sohbete ait spot bağlantıyı paylaşmak için buraya tıklayın.",
		ukUA: "{LANGUAGE}: Натисніть це, щоб поділитися посиланням на поточний чат.",
		uzUZ: "{LANGUAGE}: Joriy chatga joylashish havolasini baham ko&#39;rish uchun buni bosing.",
		zhCN: "{LANGUAGE}: 单击此处可将现场链接分享到当前聊天。"},
	ProfileOfUser: {
