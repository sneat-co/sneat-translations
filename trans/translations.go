...
var TRANS = map[string]map[string]string{
	...
	StepNumberOfNumberOfSteps: {
		arEG: "الخطوة رقم {STEP_NUMBER} من {NUMBER_OF_STEPS}",
		deDE: "Schritt Nr. {STEP_NUMBER} von {NUMBER_OF_STEPS}",
		enUK: "Step #{STEP_NUMBER} of {NUMBER_OF_STEPS}",
		esES: "Paso n.° {STEP_NUMBER} de {NUMBER_OF_STEPS}",
		faIR: "مرحله شماره {STEP_NUMBER} از {NUMBER_OF_STEPS}",
		frFR: "Étape n° {STEP_NUMBER} sur {NUMBER_OF_STEPS}",
		idID: "Langkah #{STEP_NUMBER} dari {NUMBER_OF_STEPS}",
		itIT: "Fase n. {STEP_NUMBER} di {NUMBER_OF_STEPS}",
		jaJP: "ステップ #{STEP_NUMBER} / {NUMBER_OF_STEPS}",
		koKR: "{NUMBER_OF_STEPS} 중 {STEP_NUMBER}단계",
		plPL: "Krok nr {STEP_NUMBER} z {NUMBER_OF_STEPS}",
		ptBR: "Etapa #{STEP_NUMBER} de {NUMBER_OF_STEPS}",
		ptPT: "Passo #{STEP_NUMBER} de {NUMBER_OF_STEPS}",
		ruRU: "Шаг #{STEP_NUMBER} из {NUMBER_OF_STEPS}",
		trTR: "Adım #{STEP_NUMBER} / {NUMBER_OF_STEPS}",
		ukUA: "Крок №{STEP_NUMBER} з {NUMBER_OF_STEPS}",
		uzUZ: "Qadam №{STEP_NUMBER}/{NUMBER_OF_STEPS}",
		zhCN: "步骤 #{STEP_NUMBER}（共 {NUMBER_OF_STEPS} 步）"},
	ButtonAdd: {
		arEG: "يضيف",
		deDE: "Hinzufügen",
		enUK: "Add",
...
<b>For example</b> for 20th of January 2017 submit:
    <i>20.01.2017</i>`,
		esES: `Para establecer la fecha recordatoria escribela enUK el siguiente formato <i>DD.MM.AÑO</i>.
...
<b>For example</b> for 20th of January 2017 submit:
<i>20.01.2017</i>`,
		esES: `Para establecer la fecha de devolución escribela enUK el siguiente formato <i>DD.MM.AÑO</i>.
<b>For example</b> para 20 de Enero 2017 envia:
<i>20.01.2017</i>`,
...
<i>20.01.2017</i>`,
	...
	MESSAGE_TEXT_DUE_RETURNS_HEADER: {
		...
		trTR: "<b>İade edilmesi gereken enUK yakın borçlar:</b>",
...
Es mejor si ocultas el enlace enUK HTML como:
...
C'est mieux si vous cachez le lien enUK HTML comme:
...
Son 100 yeni kullanıcı için enUK iyi 5 referans burada gösterilecektir.`,
...
Сама транзакция НЕ будет отменена, но создатель будет оповещён.`,
	...
	MESSAGE_TEXT_YOUR_ABOUT_ADS: {
		arEG: `🤖: أنا روبوت جيد، بالتأكيد. ولكن في بعض الأحيان يكون من الأنسب استخدام تطبيق متخصص لطيف. إنه ليس جاهزًا للاستخدام العام بعد ولكن يمكنك التحقق من شكله: <a href="https://debtstracker.io/enUK/#utm_source=telegram&utm_campaign=ads_screen">https://debtstracker.io/enUK/</a> 

 هل تريد الحصول على دعوة عند إصداره؟`,
		deDE: `🤖: Ich hin ein guter Roboter - klar. Aber manchmal kommt es besser eine eigene App für etwas zu haben. Es ist noch nicht ganz fertig, aber falls du schonmal reinschauen willst: <a href="https://debtstracker.io/de/#utm_source=telegram&utm_campaign=ads_screen">https://debtstracker.io/enUK/</a>

	Möchtest du daran erinnert werden, wenn die App rauskommt?`,
		enUK: `🤖: I'm a good robot, for sure.But sometimes it is more convinient to use a nice specialized app.It's not ready for public use yet but you can check how it is going to looks: <a href="https://debtstracker.io/enUK/#utm_source=telegram&utm_campaign=ads_screen">https://debtstracker.io/enUK/</a>

	Do you want to get an invite when it gets released?`,
		esES: `🤖: Claro que soy un robot encantador, pero más comodo usar una aplicación especial.No esta disponible ya pero se puede ver como será: <a href = "https://debtstracker.io/es/#utm_source=telegram&utm_campaign=ads_screen">https://debtstracker.io/enUK/</a>

	¿Quieres que te avisemos cuando esté lista?`,
		faIR: `🤖: مطمئناً من روبات خوبی هستم. اما بعضی وقت هاساده تر و مناسب تر است که از یک برنامه به خوبی تخصصی شده استفاده شود، این برنامه هنوز برای استفاده عموم آماده نیست ولی می توانید چک کنید که چگونه به نظر خواهد رسید: <a href="https://debtstracker.io/fa/#utm_source=telegram&utm_campaign=ads_screen">https://debtstracker.io/enUK/</a>
	...
	Do you want to get an invite when it gets released?`,
		ptPT: `🤖: Sou um bom robô, com certeza. Mas, por vezes, é mais conveniente usar uma aplicação especializada. Ainda não está pronto para uso público, mas pode consultar como ficará: <a href="https://debtstracker.io/enUK/#utm_source=telegram&utm_campaign=ads_screen">https://debtstracker.io/enUK/</a> 
 ...
 Quer receber um convite quando for lançado?`,
	...
	MESSAGE_TEXT_INVALID_FLOAT: {
		...
		trTR: "Üzgünüm, miktar/miktar olarak sadece rakam kullanabilirsiniz ( <i>noktadan sonra enUK fazla 2 rakamla</i> ).",
 ...
 如果标准选项不够，只需发送文本即可。例如：“<i>苹果</i>”。`},
	...
	MESSAGE_TEXT_CAN_YOU_RATE_AT_STOREBOT: {
		...
		esES: `🤖 Puedes valolarlo con una buena nota y una buena opinión enUK el catálogo Store Bot?
 ...
 شكرًا جزيلاً لك! بفضل جهودك، سيتعرف المزيد من الأشخاص على البوت. كل هذا سيكون حافزًا إضافيًا للمطورين! 😎`,
	...
	Thank you very much! As a result of your actions, even more people will learn about the bot.All this will serve as the additional motivation for the developers! 😎`,
		esES: `<b>Como valorar enUK 3 simples pasos:</b>

	1. Click este link para valorar y dejar tu opinión:
	https://t.me/storebot?start={{bot}}

	2. Click enUK "⭐️⭐️⭐️⭐️⭐️" botón
	...
	بسیار سپاسگزاریم! عمل شما باعث می شود افراد بیشتری در مورد bot.All بیاموزند. این امر انگیزه مضاعفی به توسعه دهندگان این ربات می دهد ! 😎`,
		frFR: `<b>Comment évaluer enUK 3 étapes simples :</b> 
 ...
 سنكون ممتنين أيضًا إذا اقترحت أي تحسينات.
	...
	`,
		esES: `OK, Quizás puedas valorar enUK otro momento.
...
* Nos pusieras 5⭐ enUK <a storebot>directory of bots</a>.
...
Si crees que esto es un error, háganoslo saber enUK @DebtsTrackerGroup.`,
...
اگر فکر می کنید این یک اشتباه است، لطفاً به ما در @DebtsTrackerGroup اطلاع دهید.`,
		frFR: `Vous essayez de créer un enregistrement de retour mais il n'y a pas de dettes enUK cours.

Si vous pensez qu'il s'agit d'une erreur, veuillez nous enUK informer dans @DebtsTrackerGroup.`,
...
如果您认为这是一个错误，请在 @DebtsTrackerGroup 中告诉我们。`,
	...
	MESSAGE_TEXT_PLEASE_ENTER_INVITE_CODE: {
		...
		enUK: "Please enter an invite codeDE:",
	...
	MESSAGE_TEXT_WRONG_INVITE_CODE: {
		...
		deDE: "Ungültiger CodeDE: %v",
		enUK: "Wrong invite codeDE: %v",
...
Y si hacen compras enUK grupo y quieren dividir las facturas, @SplitusBot está aquí para ayudarte.`,
		faIR: //nolint:staticcheck // disable ST1018 for this line
...
O usa el menú de abajo enUK la pantalla.`,
		faIR: `
...
%v`},
 ...
 <i>Se for parcialmente, pode introduzir o valor imediatamente.</i>`,
	...
	MESSAGE_TEXT_PLEASE_HELP_MAKE_IT_BETTER: {
		arEG: `هذا البرنامج <b>مجاني</b> . يُرجى <a href="https://debtstracker.io/enUK/help-us">المساعدة</a> في تحسينه!`,
		deDE: `Das Programm ist <b>kostenlos</b>. Bitte <a href="https://debtstracker.io/en/help-us">hilf</a> es besser zu machen!`,
		enUK: `This program is <b>free to use</b>. Please <a href="https://debtstracker.io/enUK/help-us">help</a> to make it better!`,
		...
		jaJP: `このプログラムは<b>無料で使用できます</b>。より良くするために<a href="https://debtstracker.io/enUK/help-us">ご協力</a>ください！`,
		koKR: `이 프로그램은 <b>무료로 사용</b>할 수 있습니다. 더 나은 프로그램을 만들기 위해 <a href="https://debtstracker.io/enUK/help-us">도와주세요</a>!`,
		plPL: `Ten program jest <b>darmowy</b>. Proszę <a href="https://debtstracker.io/en/help-us">pomóż</a> uczynić go lepszym!`,
		ptBR: `Este programa é <b>gratuito para uso</b>. Por favor, <a href="https://debtstracker.io/enUK/help-us">ajude</a> a torná-lo melhor!`,
		ptPT: `Este programa é <b>gratuito</b> . <a href="https://debtstracker.io/enUK/help-us">Ajude</a> a melhorá-lo!`,
		ruRU: `Эта программа <b>бесплатна</b>. <a href="https://debtstracker.io/ru/help-us">Помогите</a> сделать её лучше!`,
		trTR: `Bu program <b>ücretsiz kullanım</b> içindir. Lütfen daha iyi hale getirmek için <a href="https://debtstracker.io/enUK/help-us">yardım edin</a>!`,
		ukUA: `Ця програма <b>безкоштовна</b>. Будь ласка, <a href="https://debtstracker.io/enUK/help-us">допоможіть</a> зробити її кращою!`,
		uzUZ: `Bu dastur <b>bepul foydalanish</b> uchun. Iltimos, uni yaxshilash uchun <a href="https://debtstracker.io/enUK/help-us">yordam bering</a>!`,
		zhCN: `此程序<b>免费使用</b>。请<a href="https://debtstracker.io/enUK/help-us">帮助</a>我们使其变得更好！`,
	},
...
Cuando tu amigo accepte la invitación vaís a tener transacciones y balance enUK común (solo entre vosotros). Todo eso os ayuda minimizar los esfuerzos para controlar la cuenta.`,
...
一旦您的朋友接受邀请，您将共享余额和转账，以确保您们双方以最少的努力保持一致。`,
	...
	MESSAGE_TEXT_INVITE_BY_TELEGRAM: {
		...
		enUK: "Please share a contact of your friend you wish to send an invite codeDE:",
	...
	MESSAGE_TEXT_INVALID_DATE: {
		...
		deDE: "Ungültiges Datumsformat. Zum Beispiel für den 20. February 2019 sendeDE: 20.02.2019 oder 20.02.19",
...
<pre> Factura: gasto compartido entre dos o más personas.</pre> Por eso, lo mejor es <b>crear una factura enUK el chat de Telegram enUK solo 2 pasos</b> :
 ...
 2. Escribe <code>@splitusbot {amount} {bill_name}</code> y selecciona el resultado enUK el menú. Por ejemplo:
<pre> @splitusbot 45.5 pizza</pre> 
 Cualquier miembro del grupo puede compartir la cuenta pulsando el botón <code>Join</code> .

 <b>También</b> puedes crear una cuenta directamente enUK @{{.BotCode}}. En ese caso, tendrías que añadir participantes manualmente.`,
		faIR: //nolint:staticcheck // disable ST1018 for this line
...
C&#39;est pourquoi il est préférable de <b>créer une facture dans le chat Telegram enUK deux étapes</b> :
...
<pre> @splitusbot 45,5 pizzas</pre> 
 N&#39;importe quel membre du groupe peut partager la facture enUK appuyant sur le bouton <code>Join</code> .
...
Bu yüzden enUK iyisi <b>Telegram sohbetinde fatura oluşturmak sadece 2 adımdadır</b> :
...
Si no tienes el código puedes dejarnos tu contacto y te lo enviaremos cuando sea tu turno enUK la cola .
...
Nous envoyons 10 invitations par jour à ceux qui sont enUK tête de la file d&#39;attente et 1 au hasard.`,
		idID: `<b>%v</b> ,
...
我们每天向队列头部的用户发送 10 个邀请，并随机发送 1 个。`},
	EMAIL_INVITE_SUBJ: {
		...
		deDE: "Eine Einladung von {{.FromName}} - CodeDE: {{.InviteCode}}",
		enUK: "An invite from {{.FromName}} - codeDE: {{.InviteCode}}",
		...
		idID: "Undangan dari {{.FromName}} - kodeDE: {{.InviteCode}}",
...
Para establecer la tasa de interés y el período, envía un mensaje enUK el siguiente formato:
...
❗ La funcionalidad de % está enUK fase de prueba BETA, por favor háganoslo saber enUK @DebtsTrackerGroup si algo no funciona como esperabas.`,
		faIR: //nolint:staticcheck // disable ST1018 for this line
 ...
 * <code>période_grâce</code> - période sans intérêt. Pour le moment, vous ne pouvez pas définir la période de grâce et la période minimale enUK même temps.
...
❗ La fonctionnalité % est enUK phase de test BETA, veuillez nous informer dans @DebtsTrackerGroup si quelque chose ne fonctionne pas comme prévu.`,
		idID: `<b>Bunga & catatan</b>
 ...
 * <code>yüzde</code> - virgülden sonra enUK fazla 2 basamak.
...
Puedes conocer las nuevas funciones del bot enUK el canal de @SneatApp, donde <a href="https://t.me/StarGiveaways_EN">regalamos 500 🌟 CADA mes</a>.`,
		faIR: //nolint:staticcheck // disable ST1018 for this line
		`
...
Vous pouvez enUK apprendre davantage sur les nouvelles fonctionnalités du bot sur le canal @SneatApp où <a href="https://t.me/StarGiveaways_FR">nous offrons 500 🌟 CHAQUE mois</a>.`,
		idID: `
...
🌍 o públicamente (<i>cualquiera con el enlace puede aceptarlo, puedes cancelarlo enUK cualquier momento</i>)`,
...
Otherwise you can add them manually in Sneat.app.`,
		...
		De lo contrario, puedes agregarlos manualmente enUK Sneat.app.`,
...
Aks holda ularni Sneat.app ilovasida qoʻlda qoʻshishingiz mumkin.`,
	...
	NewEventText: {
		...
		trTR: "@{BOT_CODE} tüm katılımcılar için enUK uygun tarih, saat ve yeri seçmeye yardımcı olur.",
	...
	RsvpQuestionWhatActivity: {
		arEG: "ماذا ستفعل؟",
		deDE: "Was wirst du tun?",
		enUK: "What you going to do?",
		esES: "¿Que vas a hacer?",
		faIR: "قراره چیکار کنی؟",
		frFR: "Qu&#39;est-ce que tu vas faire ?",
		idID: "Apa yang akan kamu lakukan?",
		itIT: "Cosa farai?",
		jaJP: "何をするつもりですか?",
		koKR: "어떻게 할 거야?",
		plPL: "Co zamierzasz zrobić?",
		ptBR: "O que você vai fazer?",
		ptPT: "O que vai fazer?",
		ruRU: "Что вы собираетесь делать?",
		trTR: "Ne yapacaksın?",
		ukUA: "Що ти збираєшся робити?",
		uzUZ: "Nima qilmoqchisiz?",
		zhCN: "你要做什么？"},
	...
	EventTileWithLabelAndEmoji: {
		arEG: "الحدث: {EMOJI} {TITLE}",
		deDE: "Ereignis: {EMOJI} {TITLE}",
		enUK: "Event: {EMOJI} {TITLE}",
		esES: "Evento: {EMOJI} {TITLE}",
		faIR: "رویداد: {EMOJI} {TITLE}",
		frFR: "Événement : {EMOJI} {TITLE}",
		idID: "Peristiwa: {EMOJI} {TITLE}",
		itIT: "Evento: {EMOJI} {TITLE}",
		jaJP: "イベント: {EMOJI} {TITLE}",
		koKR: "이벤트: {EMOJI} {TITLE}",
		plPL: "Wydarzenie: {EMOJI} {TITLE}",
		ptBR: "Evento: {EMOJI} {TITLE}",
		ptPT: "Evento: {EMOJI} {TITLE}",
		ruRU: "Мероприятие: {EMOJI} {TITLE}",
		trTR: "Etkinlik: {EMOJI} {TITLE}",
		ukUA: "Подія: {EMOJI} {TITLE}",
		uzUZ: "Tadbir: {EMOJI} {TITLE}",
		zhCN: "事件：{EMOJI} {TITLE}"},
	...
	CanceledEventTitle: {
		arEG: "تم إلغاء الحدث",
		deDE: "Abgesagte Veranstaltung",
		enUK: "Canceled Event",
		esES: "Evento cancelado",
		faIR: "رویداد لغو شده",
		frFR: "Événement annulé",
		idID: "Acara Dibatalkan",
		itIT: "Evento annullato",
		jaJP: "キャンセルされたイベント",
		koKR: "취소된 이벤트",
		plPL: "Odwołane wydarzenie",
		ptBR: "Evento cancelado",
		ptPT: "Evento cancelado",
		ruRU: "Отменённое Мероприятие",
		trTR: "İptal Edilen Etkinlik",
		ukUA: "Скасована подія",
		uzUZ: "Bekor qilingan voqea",
		zhCN: "取消的活动"},
	EventTitle: {
		arEG: "حدث",
		deDE: "Veranstaltung",
	...
	EventVisibility: {
		...
		ptBR: "VisibilidadeDE: {VISIBILITY_TITLE} {VISIBILITY_EMOJI}",
		ptPT: "VisibilidadeDE: {VISIBILITY_TITLE} {VISIBILITY_EMOJI}",
	...
	TogdYouHaveNoActiveEvents: {
		arEG: "ليس لديك أي أحداث نشطة.",
		deDE: "Sie haben keine aktiven Ereignisse.",
		enUK: "You have no active events.",
		esES: "No tienes eventos activos",
		faIR: "شما هیچ رویداد فعالی ندارید.",
		frFR: "Vous n&#39;avez aucun événement actif.",
		idID: "Anda tidak memiliki acara aktif.",
		itIT: "Non hai eventi attivi.",
		jaJP: "アクティブなイベントはありません。",
		koKR: "활성화된 이벤트가 없습니다.",
		plPL: "Nie masz żadnych aktywnych wydarzeń.",
		ptBR: "Você não tem eventos ativos.",
		ptPT: "Não tem eventos ativos.",
		ruRU: "У вас нет активных событий.",
		trTR: "Aktif etkinliğiniz bulunmamaktadır.",
		ukUA: "У вас немає активних подій.",
		uzUZ: "Sizda faol tadbirlar yo‘q.",
		zhCN: "您没有活跃的活动。"},
	TogdUserHasNoActiveEvents: {
		arEG: "ليس لدى المستخدم أي أحداث نشطة.",
		deDE: "Der Benutzer hat keine aktiven Ereignisse.",
		enUK: "User has no active events.",
		esES: "El usuario no tiene eventos activos.",
		faIR: "کاربر هیچ رویداد فعالی ندارد.",
		frFR: "L&#39;utilisateur n&#39;a aucun événement actif.",
		idID: "Pengguna tidak memiliki acara aktif.",
		itIT: "L&#39;utente non ha eventi attivi.",
		jaJP: "ユーザーにアクティブなイベントがありません。",
		koKR: "사용자에게 활성 이벤트가 없습니다.",
		plPL: "Użytkownik nie ma aktywnych wydarzeń.",
		ptBR: "O usuário não tem eventos ativos.",
		ptPT: "O utilizador não tem eventos ativos.",
		ruRU: "У пользователя нет активных событий.",
		trTR: "Kullanıcının aktif etkinliği bulunmamaktadır.",
		ukUA: "Користувач не має активних подій.",
		uzUZ: "Foydalanuvchida faol hodisalar yo&#39;q.",
		zhCN: "用户没有活动事件。"},
	EventsStatusTitle: {
		arEG: "الحالة: <b>%s</b>",
		deDE: "Status: <b>%s</b>",
...
With @ToGetheredBot, planning becomes social, visible, and frictionless — perfect for spontaneous sessions or organised gatherings.`,
		esES: `Bienvenido a @ToGetheredBot — tu asistente de planificación simple e inteligente para quedar con amigos, organizar actividades grupales o simplemente avisar a otros dónde estarás. Ya sea kitesurf enUK la playa, jugar baloncesto callejero o planificar una reunión casual, ToGethered hace que la coordinación sea sin esfuerzo.

El bot ofrece dos características principales:

	1.	<b>Compartir Planes</b> – Haz saber a otros <b>dónde y cuándo planeas estar enUK algún lugar</b>. Puedes especificar una ubicación y un rango de tiempo, para que otros puedan ver tus planes y unirse si también están disponibles.
...
Le bot offre deux fonctionnalités principales :
	...
	2.	<b>Coordination d'Événements</b> – Organisez facilement des activités de groupe enUK suggérant plusieurs options de temps et de lieu. Le bot collecte les votes des participants et montre quelles combinaisons fonctionnent le mieux, aidant le groupe à s'accorder sur un plan sans longs fils de discussion.
...
Bot iki ana özellik sunar:
	...
	2.	<b>Etkinlik Koordinasyonu</b> – Birden fazla zaman ve yer seçeneği önererek grup etkinliklerini kolayca düzenleyin. Bot katılımcılardan oyları toplar ve hangi kombinasyonların enUK iyi çalıştığını gösterir, grubun uzun sohbet zincirleri olmadan bir plan üzerinde anlaşmasına yardımcı olur.
...
لمعرفة المزيد عن الروبوت، استخدم الأمر /about.`,
		deDE: `
Um Ihre Pläne zu teilen, wählen Sie einen Ort oder eine Aktivität aus und klicken Sie auf die Schaltfläche <b>{RSVP}</b> .

Um ein Ereignis zu organisieren, erstellen Sie unter „Meine Ereignisse“ ein <b>{NEW_EVENT}</b> .

Um mehr über den Bot zu erfahren, verwenden Sie den Befehl „/about“.`,
		enUK: `
...
To learn more about the bot use /about command.`,
		esES: `
Para compartir tus planes, elige un lugar o actividad y presiona el botón <b>{RSVP}</b> .

Para organizar un evento, crea un <b>{NEW_EVENT}</b> desde &quot;Mis eventos&quot;.

Para obtener más información sobre el bot, usa el comando /about.`,
		faIR: `
برای به اشتراک گذاشتن برنامه‌هایتان، یک مکان یا فعالیت را انتخاب کنید و دکمه <b>{RSVP}</b> را بزنید.

برای سازماندهی یک رویداد، یک <b>{NEW_EVENT}</b> از «رویدادهای من» ایجاد کنید.

برای کسب اطلاعات بیشتر در مورد ربات، از دستور /about استفاده کنید.`,
		frFR: `
Pour partager vos plans, choisissez un lieu ou une activité et appuyez sur le bouton <b>{RSVP}</b> .

Pour organiser un événement, créez un <b>{NEW_EVENT}</b> à partir de « Mes événements ».

Pour en savoir plus sur le bot, utilisez la commande /about.`,
		idID: `
Untuk berbagi rencana Anda, pilih tempat atau aktivitas dan tekan tombol <b>{RSVP}</b> .

Untuk menyelenggarakan acara, buat <b>{NEW_EVENT}</b> dari &quot;Acara Saya&quot;.

Untuk mempelajari lebih lanjut tentang bot, gunakan perintah /about.`,
		itIT: `
Per condividere i tuoi piani, scegli un luogo o un&#39;attività e premi il pulsante <b>{RSVP}</b> .

Per organizzare un evento, crea un <b>{NEW_EVENT}</b> da &quot;I miei eventi&quot;.

Per saperne di più sul bot, usa il comando /about.`,
		jaJP: `
計画を共有するには、スポットまたはアクティビティを選択し、 <b>{RSVP}</b>ボタンを押します。

イベントを企画するには、「マイイベント」から<b>{NEW_EVENT} を</b>作成します。

ボットの詳細については、/about コマンドを使用します。`,
		koKR: `
계획을 공유하려면 장소나 활동을 선택하고 <b>{RSVP}</b> 버튼을 누르세요.

이벤트를 구성하려면 &quot;내 이벤트&quot;에서 <b>{NEW_EVENT}</b> 을 만드세요.

봇에 대해 자세히 알아보려면 /about 명령을 사용하세요.`,
		plPL: `
Aby udostępnić swoje plany, wybierz miejsce lub aktywność i kliknij przycisk <b>{RSVP}</b> .

Aby zorganizować wydarzenie, utwórz <b>{NEW_EVENT}</b> w sekcji „Moje wydarzenia”.

Aby dowiedzieć się więcej o bocie, użyj polecenia /about.`,
		ptBR: `
...
Para saber mais sobre o bot, use o comando /about.`,
		ptPT: `
...
Para saber mais sobre o bot, utilize o comando /about.`,
		ruRU: `
		...
		Чтобы узнать больше используйте команду /about
			`,
		trTR: `
Planlarınızı paylaşmak için bir yer veya aktivite seçin ve <b>{RSVP}</b> düğmesine basın.

Bir etkinlik düzenlemek için &quot;Etkinliklerim&quot;den bir <b>{NEW_EVENT}</b> oluşturun.

Bot hakkında daha fazla bilgi edinmek için /about komutunu kullanın.`,
		ukUA: `
Щоб поділитися своїми планами, виберіть місце або захід і натисніть кнопку <b>{RSVP}</b> .

Щоб організувати подію, створіть <b>{NEW_EVENT}</b> з розділу «Мої події».

Щоб дізнатися більше про бота, скористайтеся командою /about.`,
		uzUZ: `
Rejalaringizni baham koʻrish uchun joy yoki faoliyatni tanlang va <b>{RSVP}</b> tugmasini bosing.

Tadbirni tashkil qilish uchun “Mening voqealarim”dan <b>{NEW_EVENT}</b> yarating.

Bot haqida batafsil maʼlumot olish uchun /about buyrugʻidan foydalaning.`,
		zhCN: `
要分享您的计划，请选择一个地点或活动，然后点击<b>{RSVP}</b>按钮。

要组织活动，请从“我的活动”中创建<b>{NEW_EVENT}</b> 。

要了解有关机器人的更多信息，请使用 /about 命令。`},
	RsvpButtonText: {
...
我们的目标是尽可能透明和值得信赖。`},
	...
	ShareEventInlineDescription: {
		arEG: "انقر هنا لمشاركة رابط الحدث في الدردشة الحالية.",
		deDE: "Klicken Sie hier, um den Ereignislink zum aktuellen Chat freizugeben.",
		enUK: "Click this to share event link to the current chat.",
		esES: "Haga clic aquí para compartir el enlace del evento en el chat actual.",
		faIR: "برای اشتراک‌گذاری لینک رویداد در چت فعلی، روی این کلیک کنید.",
		frFR: "Cliquez ici pour partager le lien de l&#39;événement vers le chat en cours.",
		idID: "Klik ini untuk membagikan tautan acara ke obrolan saat ini.",
		itIT: "Fai clic qui per condividere il collegamento all&#39;evento nella chat corrente.",
		jaJP: "現在のチャットへのイベント リンクを共有するには、これをクリックします。",
		koKR: "현재 채팅에 이벤트 링크를 공유하려면 여기를 클릭하세요.",
		plPL: "Kliknij tutaj, aby udostępnić link do wydarzenia w bieżącym czacie.",
		ptBR: "Clique aqui para compartilhar o link do evento no chat atual.",
		ptPT: "Clique aqui para partilhar o link do evento no chat atual.",
		ruRU: "Нажмите тут чтобы поделиться ссылкой на событие в текущем чате.",
		trTR: "Etkinliğin bağlantısını güncel sohbete paylaşmak için buraya tıklayın.",
		ukUA: "Натисніть це, щоб поділитися посиланням на подію з поточним чатом.",
		uzUZ: "Hodisa havolasini joriy chatga ulashish uchun bu yerni bosing.",
		zhCN: "单击此处可将事件链接分享到当前聊天。"},
	EventIsCanceled: {
		arEG: "تم إلغاء الحدث",
		deDE: "Veranstaltung abgesagt",
		enUK: "Event is cancelled",
		esES: "El evento está cancelado",
		faIR: "رویداد لغو شده است",
		frFR: "L&#39;événement est annulé",
		idID: "Acara dibatalkan",
		itIT: "L&#39;evento è annullato",
		jaJP: "イベントはキャンセルされました",
		koKR: "이벤트가 취소되었습니다",
		plPL: "Wydarzenie zostało odwołane",
		ptBR: "Evento cancelado",
		ptPT: "Evento cancelado",
		ruRU: "Мероприятие отменено",
		trTR: "Etkinlik iptal edildi",
		ukUA: "Подія скасована",
		uzUZ: "Tadbir bekor qilingan",
		zhCN: "活动取消"},
	DoYouWantToCancelThisEvent: {
		arEG: "هل تريد إلغاء هذا الحدث؟",
		deDE: "Möchten Sie diese Veranstaltung absagen?",
		enUK: "Do you want to cancel this event?",
		esES: "¿Quieres cancelar este evento?",
		faIR: "آیا می‌خواهید این رویداد را لغو کنید؟",
		frFR: "Voulez-vous annuler cet événement ?",
		idID: "Apakah Anda ingin membatalkan acara ini?",
		itIT: "Vuoi annullare questo evento?",
		jaJP: "このイベントをキャンセルしますか?",
		koKR: "이 이벤트를 취소하시겠습니까?",
		plPL: "Czy chcesz odwołać to wydarzenie?",
		ptBR: "Você quer cancelar este evento?",
		ptPT: "Deseja cancelar este evento?",
		ruRU: "Вы хотите отменить это событие?",
		trTR: "Bu etkinliği iptal etmek istiyor musunuz?",
		ukUA: "Ви хочете скасувати цю подію?",
		uzUZ: "Bu tadbirni bekor qilmoqchimisiz?",
		zhCN: "您想取消此活动吗？"},
	...
	TogetheredBotDescription: {
		arEG: `يقدم @ToGetheredBot ميزتين رئيسيتين: 

	1. 📍🗓️🕒📣 مشاركة الخطط - أخبر الآخرين بمكان وزمان تواجدك في مكان ما، واعرف من سيحضر قبل انطلاقك. 

	2. 🗓️🕒📍🗳️ تنسيق الفعاليات - نظّم أنشطة المجموعة بسهولة من خلال اقتراح خيارات متعددة للوقت والمكان. يجمع البوت أصوات المشاركين ويعرض الخيارات الأنسب، مما يساعد المجموعة على الاتفاق على خطة دون الحاجة إلى نقاشات مطولة.`,
		deDE: `@ToGetheredBot bietet zwei Hauptfunktionen:

	1. 📍🗓️🕒📣 Pläne teilen – teilen Sie anderen mit, wo und wann Sie wo sein werden und wer da ist – bevor Sie losfahren.

	2. 🗓️🕒📍🗳️ Veranstaltungen koordinieren – organisieren Sie Gruppenaktivitäten ganz einfach, indem Sie mehrere Zeit- und Ortsoptionen vorschlagen. Der Bot sammelt Stimmen der Teilnehmer und zeigt, welche Kombinationen am besten funktionieren. So kann sich die Gruppe ohne lange Chat-Threads auf einen Plan einigen.`,
	...
	2. 🗓️🕒📍🗳️ Events Coordination – easily organise group activities by suggesting multiple time and place options. The bot collects votes from participants and shows which combinations work best, helping the group agree on a plan without long chat threads.`,
		esES: `@ToGetheredBot ofrece dos funciones principales:

	1. 📍🗓️🕒📣 Compartir planes: informa a los demás sobre dónde y cuándo planeas estar en un lugar y quién está presente, antes de salir.

	2. 🗓️🕒📍🗳️ Coordinación de eventos: organiza fácilmente actividades grupales sugiriendo múltiples opciones de hora y lugar. El bot recopila los votos de los participantes y muestra las combinaciones más adecuadas, lo que ayuda al grupo a acordar un plan sin largas conversaciones.`,
		faIR: `@ToGetheredBot دو ویژگی اصلی ارائه می‌دهد:

	۱. 📍🗓️🕒📣 اشتراک‌گذاری برنامه‌ها - قبل از اینکه بیرون بروید، به دیگران اطلاع دهید که کجا و چه زمانی قصد دارید جایی باشید و چه کسی آنجاست.

	۲. 🗓️🕒📍🗳️ هماهنگی رویدادها - با پیشنهاد گزینه‌های مختلف زمانی و مکانی، فعالیت‌های گروهی را به راحتی سازماندهی کنید. این ربات آرای شرکت‌کنندگان را جمع‌آوری می‌کند و نشان می‌دهد که کدام ترکیب‌ها بهترین عملکرد را دارند و به گروه کمک می‌کند تا بدون بحث‌های طولانی چت، روی یک برنامه به توافق برسند.`,
		frFR: `@ToGetheredBot propose deux fonctionnalités principales :

	1. 📍🗓️🕒📣 Partage de plans : informez les autres de l&#39;endroit et de l&#39;heure où vous prévoyez d&#39;être quelque part et des personnes présentes avant de partir.

	2. 🗓️🕒📍🗳️ Coordination d&#39;événements : organisez facilement des activités de groupe en suggérant plusieurs options d&#39;horaires et de lieux. Le bot recueille les votes des participants et indique les combinaisons les plus efficaces, aidant ainsi le groupe à convenir d&#39;un plan sans longs fils de discussion.`,
		idID: `@ToGetheredBot menawarkan dua fitur utama:

	1. 📍🗓️🕒📣 Berbagi Rencana – beri tahu orang lain di mana dan kapan Anda berencana berada di suatu tempat dan siapa saja yang ada di sana - sebelum Anda berangkat.

	2. 🗓️🕒📍🗳️ Koordinasi Acara – atur aktivitas grup dengan mudah dengan menyarankan beberapa pilihan waktu dan tempat. Bot ini mengumpulkan suara dari peserta dan menunjukkan kombinasi mana yang paling efektif, membantu grup menyepakati rencana tanpa perlu obrolan panjang.`,
		itIT: `@ToGetheredBot offre due funzionalità principali:

	1. 📍🗓️🕒📣 Condivisione dei piani: fai sapere agli altri dove e quando prevedi di essere da qualche parte e chi c&#39;è, prima di partire.

	2. 🗓️🕒📍🗳️ Coordinamento degli eventi: organizza facilmente le attività di gruppo suggerendo diverse opzioni di orario e luogo. Il bot raccoglie i voti dei partecipanti e mostra quali combinazioni funzionano meglio, aiutando il gruppo a concordare un piano senza lunghe discussioni.`,
		jaJP: `@ToGetheredBot には主に2つの機能があります。

	1. 📍🗓️🕒📣 計画共有 – 出発前に、いつどこにいる予定か、誰が参加しているかを他の人に知らせることができます。

	2. 🗓️🕒📍🗳️ イベント調整 – 複数の時間と場所のオプションを提案することで、グループアクティビティを簡単に整理できます。ボットは参加者からの投票を収集し、最適な組み合わせを提示することで、長いチャットスレッドなしでグループが計画を合意するのに役立ちます。`,
		koKR: `@ToGetheredBot은 두 가지 주요 기능을 제공합니다.

	1. 📍🗓️🕒📣 계획 공유 – 출발 전에 다른 사람들에게 언제, 어디에 갈지, 그리고 누가 있는지 알려주세요.

	2. 🗓️🕒📍🗳️ 이벤트 조정 – 다양한 시간 및 장소 옵션을 제안하여 그룹 활동을 쉽게 구성할 수 있습니다. 봇은 참가자들의 투표를 수집하고 어떤 조합이 가장 효과적인지 보여주어 긴 채팅 스레드 없이도 그룹이 계획에 동의할 수 있도록 도와줍니다.`,
		plPL: `@ToGetheredBot oferuje dwie główne funkcje:

	1. 📍🗓️🕒📣 Udostępnianie planów – poinformuj innych, gdzie i kiedy planujesz gdzieś być i kto jest obecny – zanim wyruszysz.

	2. 🗓️🕒📍🗳️ Koordynacja wydarzeń – łatwo organizuj zajęcia grupowe, sugerując wiele opcji czasowych i miejscowych. Bot zbiera głosy uczestników i pokazuje, które kombinacje sprawdzają się najlepiej, pomagając grupie uzgodnić plan bez długich dyskusji na czacie.`,
		ptBR: `O @ToGetheredBot oferece dois recursos principais:

	1. 📍🗓️🕒📣 Compartilhamento de Planos – informe aos outros onde e quando você planeja estar em algum lugar e saiba quem está presente – antes de sair.

	2. 🗓️🕒📍🗳️ Coordenação de Eventos – organize facilmente atividades em grupo sugerindo várias opções de horário e local. O bot coleta votos dos participantes e mostra quais combinações funcionam melhor, ajudando o grupo a chegar a um acordo sobre um plano sem longas conversas.`,
		ptPT: `O @ToGetheredBot oferece duas características principais:

	1. 📍🗓️🕒📣 Partilha de Planos – informe os outros onde e quando planeia estar em algum lugar e saiba quem está presente – antes de sair.

	2. 🗓️🕒📍🗳️ Coordenação de Eventos – organize facilmente atividades de grupo sugerindo várias opções de horário e local. O bot recolhe os votos dos participantes e mostra quais as combinações que funcionam melhor, ajudando o grupo a chegar a um acordo sobre um plano sem grandes conversas.`,
	...
	2. 🗓️🕒📍🗳️ Координация мероприятий — легко организуйте групповые мероприятия, предлагая несколько вариантов времени и места. Бот собирает голоса участников и показывает, какие комбинации работают лучше всего, помогая группе согласовать планы без длинных чатов.`,
		trTR: `@ToGetheredBot iki temel özellik sunar:

	1. 📍🗓️🕒📣 Plan Paylaşımı – Başkalarına nerede ve ne zaman olmayı planladığınızı bildirin ve yola çıkmadan önce kimin orada olduğunu bilin.

	2. 🗓️🕒📍🗳️ Etkinlik Koordinasyonu – Birden fazla zaman ve mekan seçeneği önererek grup etkinliklerini kolayca organize edin. Bot, katılımcılardan oy toplar ve hangi kombinasyonların en uygun olduğunu göstererek grubun uzun sohbetler olmadan bir plan üzerinde anlaşmasına yardımcı olur.`,
		ukUA: `@ToGetheredBot пропонує дві основні функції:

	1. 📍🗓️🕒📣 Обмін планами – повідомте іншим, де і коли ви плануєте бути, і дізнайтеся, хто вдома – ще до того, як ви вирушите.

	2. 🗓️🕒📍🗳️ Координація подій – легко організовуйте групові заходи, пропонуючи кілька варіантів часу та місця. Бот збирає голоси учасників і показує, які комбінації найкраще працюють, допомагаючи групі узгодити план без довгих обговорень.`,
		uzUZ: `@ToGetheredBot ikkita asosiy xususiyatni taklif etadi:

	1. 📍🗓️🕒📣 Rejalar almashish – chiqishdan oldin qayerda va qachon bo‘lishni rejalashtirayotganingizni boshqalarga bildiring va kimlar borligini bilib oling.

	2. 🗓️🕒📍🗳️ Tadbirlarni muvofiqlashtirish - bir nechta vaqt va joy variantlarini taklif qilish orqali guruh faoliyatini osongina tashkil qiling. Bot ishtirokchilarning ovozlarini to&#39;playdi va qaysi kombinatsiyalar eng yaxshi ishlashini ko&#39;rsatadi, bu esa guruhga uzoq suhbatlarsiz rejani kelishib olishga yordam beradi.`,
		zhCN: `@ToGetheredBot 提供两项主要功能：

	1. 📍🗓️🕒📣 计划共享——在您出发前，即可告知他人您计划在何时何地前往某个地方，并了解哪些人与您同在。

	2. 🗓️🕒📍🗳️ 活动协调——通过建议多个时间和地点选项，轻松组织小组活动。该机器人会收集参与者的投票，并展示哪些组合效果最佳，帮助小组成员无需冗长的聊天即可达成一致。`},
	TogetheredBotShortDescription: {
		arEG: "📍🗓️🕒📣🗳 يُحوّل النوايا إلى تجمعات. يُساعد على تنظيم الفعاليات. اعرف من سيحضر قبل أن تخرج.",
		deDE: "📍🗓️🕒📣🗳 Verwandelt Absichten in Treffen. Hilft bei der Organisation von Veranstaltungen. Wissen Sie, wer da ist – bevor Sie losfahren.",
		enUK: "📍🗓️🕒📣🗳 Turns intentions into gatherings. Helps to organize events. Know who is in - before you head out.",
		esES: "📍🗓️🕒📣🗳 Convierte las intenciones en reuniones. Ayuda a organizar eventos. Descubre quién está presente antes de salir.",
		faIR: "📍🗓️🕒📣🗳 نیت‌ها را به گردهمایی تبدیل می‌کند. به سازماندهی رویدادها کمک می‌کند. قبل از حرکت، بدانید چه کسی در جمع است.",
		frFR: "📍🗓️🕒📣🗳 Transformez vos intentions en rassemblements. Aidez à organiser vos événements. Sachez qui est là avant de partir.",
		idID: "📍🗓️🕒📣🗳 Mengubah niat menjadi pertemuan. Membantu mengatur acara. Ketahui siapa saja yang akan hadir - sebelum Anda berangkat.",
		itIT: "📍🗓️🕒📣🗳 Trasforma le intenzioni in incontri. Aiuta a organizzare eventi. Scopri chi c&#39;è prima di uscire.",
		jaJP: "📍🗓️🕒📣🗳 意図を集いに変えます。イベントの企画に役立ちます。出かける前に誰が参加しているか確認できます。",
		koKR: "📍🗓️🕒📣🗳 의도를 모임으로 전환하세요. 행사를 준비하는 데 도움이 됩니다. 나가기 전에 누가 참석하는지 확인하세요.",
		plPL: "📍🗓️🕒📣🗳 Zamienia intencje w spotkania. Pomaga organizować wydarzenia. Dowiedz się, kto jest obecny – zanim wyjdziesz.",
		ptBR: "📍🗓️🕒📣🗳 Transforma intenções em encontros. Ajuda a organizar eventos. Saiba quem está presente antes de sair.",
		ptPT: "📍🗓️🕒📣🗳 Transforma intenções em encontros. Ajuda a organizar eventos. Saiba quem está presente antes de sair.",
		ruRU: "📍🗓️🕒📣🗳 Превращает намерения во встречи. Помогает организовывать мероприятия. Показывает кто собирается учавствовать.",
		trTR: "📍🗓️🕒📣🗳 Niyetleri buluşmalara dönüştürür. Etkinlikleri organize etmeye yardımcı olur. Çıkmadan önce kimin içeride olduğunu bilin.",
		ukUA: "📍🗓️🕒📣🗳 Перетворює наміри на зустрічі. Допомагає організувати події. Дізнайтеся, хто присутній, ще до того, як вирушите.",
		uzUZ: "📍🗓️🕒📣🗳 Niyatlarni yig&#39;inga aylantiradi. Tadbirlarni tashkil etishga yordam beradi. Yo&#39;lga chiqishdan oldin kim borligini bilib oling.",
		zhCN: "📍🗓️🕒📣🗳 将意图转化为聚会。帮助组织活动。出门前就能知道谁在场。"},
	AboutCommandDescription: {
		arEG: "حول الروبوت الخاص بنا",
		deDE: "Über unseren Bot",
		enUK: "About our bot",
		esES: "Acerca de nuestro bot",
		faIR: "درباره ربات ما",
		frFR: "À propos de notre bot",
		idID: "Tentang bot kami",
		itIT: "Informazioni sul nostro bot",
		jaJP: "私たちのボットについて",
		koKR: "우리의 봇에 대하여",
		plPL: "O naszym bocie",
		ptBR: "Sobre nosso bot",
		ptPT: "Sobre o nosso bot",
		ruRU: "О нашем боте",
		trTR: "Botumuz hakkında",
		ukUA: "Про нашого бота",
		uzUZ: "Bizning bot haqida",
		zhCN: "关于我们的机器人"},
	WatchCommandDescription: {
		arEG: "أضف فيلماً إلى قائمة المشاهدة",
		deDE: "Film zur Watchlist hinzufügen",
		enUK: "Add a movie to your To-Watch list",
		esES: "Añade una película a tu lista para ver",
		faIR: "افزودن فیلم به فهرست تماشا",
		frFR: "Ajouter un film à votre liste à regarder",
		idID: "Tambahkan film ke daftar tontonan",
		itIT: "Aggiungi un film alla lista da guardare",
		jaJP: "映画を視聴リストに追加",
		koKR: "영화를 볼 목록에 추가",
		plPL: "Dodaj film do listy do obejrzenia",
		ptBR: "Adicione um filme à sua lista para assistir",
		ptPT: "Adicione um filme à sua lista para ver",
		ruRU: "Добавить фильм в список «Посмотреть»",
		trTR: "İzleme listenize film ekleyin",
	...
	FindCommandDescription: {
		...
		enUK: "Find a movie by describing it",
		esES: "Encuentra una película describiéndola",
		faIR: "با توصیف فیلم آن را پیدا کنید",
		frFR: "Trouvez un film en le décrivant",
		idID: "Temukan film dengan mendeskripsikannya",
		itIT: "Trova un film descrivendolo",
		jaJP: "映画を説明して探す",
		koKR: "영화를 설명해서 찾기",
		plPL: "Znajdź film, opisując go",
		ptBR: "Encontre um filme descrevendo-o",
		ptPT: "Encontre um filme descrevendo-o",
		ruRU: "Найти фильм по описанию",
		trTR: "Bir filmi tarif ederek bulun",
		ukUA: "Знайти фільм за описом",
		uzUZ: "Filmni tasvirlab toping",
		zhCN: "通过描述查找电影"},
	FindMovieAskForDescription: {
		arEG: "🎬 صِف الفيلم الذي تبحث عنه وسأحاول التعرف عليه.",
		deDE: "🎬 Beschreibe den Film, den du suchst, und ich versuche, ihn zu erkennen.",
		enUK: "🎬 Describe the movie you are looking for and I will try to identify it.",
		esES: "🎬 Describe la película que buscas e intentaré identificarla.",
		faIR: "🎬 فیلمی را که به دنبال آن هستید توصیف کنید و من سعی می‌کنم آن را شناسایی کنم.",
		frFR: "🎬 Décrivez le film que vous cherchez et j'essaierai de l'identifier.",
		idID: "🎬 Deskripsikan film yang Anda cari dan saya akan mencoba mengidentifikasinya.",
		itIT: "🎬 Descrivi il film che stai cercando e proverò a identificarlo.",
		jaJP: "🎬 探している映画を説明してください。特定してみます。",
		koKR: "🎬 찾고 있는 영화를 설명해 주시면 제가 찾아보겠습니다.",
		plPL: "🎬 Opisz film, którego szukasz, a spróbuję go rozpoznać.",
		ptBR: "🎬 Descreva o filme que você procura e tentarei identificá-lo.",
		ptPT: "🎬 Descreva o filme que procura e tentarei identificá-lo.",
		ruRU: "🎬 Опишите фильм, который ищете, и я постараюсь его узнать.",
		trTR: "🎬 Aradığınız filmi tarif edin, onu bulmaya çalışayım.",
		ukUA: "🎬 Опишіть фільм, який шукаєте, і я спробую його впізнати.",
		uzUZ: "🎬 Qidirayotgan filmingizni tasvirlab bering, men uni aniqlashga harakat qilaman.",
		zhCN: "🎬 描述您要找的电影，我会尽力识别它。"},
	FindMovieCandidatesFound: {
		arEG: "🎬 هذا ما وجدته. اضغط على فيلم لإضافته إلى قائمة المشاهدة:",
		deDE: "🎬 Das habe ich gefunden. Tippe auf einen Film, um ihn zur Watchlist hinzuzufügen:",
		enUK: "🎬 Here is what I found. Tap a movie to add it to your To-Watch list:",
		esES: "🎬 Esto es lo que encontré. Toca una película para añadirla a tu lista para ver:",
		faIR: "🎬 این چیزی است که پیدا کردم. برای افزودن فیلم به فهرست تماشا روی آن بزنید:",
		frFR: "🎬 Voici ce que j'ai trouvé. Appuyez sur un film pour l'ajouter à votre liste à regarder :",
		idID: "🎬 Ini yang saya temukan. Ketuk film untuk menambahkannya ke daftar tontonan:",
		itIT: "🎬 Ecco cosa ho trovato. Tocca un film per aggiungerlo alla lista da guardare:",
		jaJP: "🎬 見つかった映画はこちらです。視聴リストに追加するには映画をタップしてください:",
		koKR: "🎬 찾은 결과입니다. 영화를 눌러 볼 목록에 추가하세요:",
		plPL: "🎬 Oto co znalazłem. Dotknij filmu, aby dodać go do listy do obejrzenia:",
		ptBR: "🎬 Aqui está o que encontrei. Toque em um filme para adicioná-lo à sua lista para assistir:",
		ptPT: "🎬 Aqui está o que encontrei. Toque num filme para o adicionar à sua lista para ver:",
		ruRU: "🎬 Вот что я нашёл. Нажмите на фильм, чтобы добавить его в список «Посмотреть»:",
		trTR: "🎬 İşte bulduklarım. İzleme listenize eklemek için bir filme dokunun:",
		ukUA: "🎬 Ось що я знайшов. Натисніть на фільм, щоб додати його до списку «Подивитися»:",
		uzUZ: "🎬 Mana men topganlarim. Filmni tomosha roʻyxatiga qoʻshish uchun ustiga bosing:",
		zhCN: "🎬 这是我找到的结果。点击电影即可添加到观看列表："},
	FindMovieNoMatches: {
		arEG: "😔 لم أجد أفلاماً تطابق وصفك. حاول وصفه بطريقة مختلفة.",
		deDE: "😔 Keine Filme passen zu deiner Beschreibung. Versuche es mit einer anderen Beschreibung.",
		enUK: "😔 No movies matched your description. Try describing it differently.",
		esES: "😔 Ninguna película coincide con tu descripción. Intenta describirla de otra manera.",
		faIR: "😔 هیچ فیلمی با توصیف شما مطابقت نداشت. سعی کنید آن را طور دیگری توصیف کنید.",
		frFR: "😔 Aucun film ne correspond à votre description. Essayez de le décrire autrement.",
		idID: "😔 Tidak ada film yang cocok dengan deskripsi Anda. Coba deskripsikan dengan cara lain.",
		itIT: "😔 Nessun film corrisponde alla tua descrizione. Prova a descriverlo in modo diverso.",
		jaJP: "😔 説明に一致する映画が見つかりませんでした。別の言い方で説明してみてください。",
		koKR: "😔 설명과 일치하는 영화가 없습니다. 다르게 설명해 보세요.",
		plPL: "😔 Żaden film nie pasuje do Twojego opisu. Spróbuj opisać go inaczej.",
		ptBR: "😔 Nenhum filme correspondeu à sua descrição. Tente descrevê-lo de outra forma.",
		ptPT: "😔 Nenhum filme correspondeu à sua descrição. Tente descrevê-lo de outra forma.",
		ruRU: "😔 Не нашёл фильмов по вашему описанию. Попробуйте описать иначе.",
		trTR: "😔 Açıklamanızla eşleşen film bulunamadı. Farklı şekilde tarif etmeyi deneyin.",
		ukUA: "😔 Не знайшов фільмів за вашим описом. Спробуйте описати інакше.",
		uzUZ: "😔 Tavsifingizga mos film topilmadi. Boshqacha tasvirlab koʻring.",
		zhCN: "😔 没有找到符合您描述的电影。请尝试换一种描述。"},
	MovieAddedToWatchlist: {
		arEG: "🎬 تمت الإضافة إلى قائمة المشاهدة: %s",
		deDE: "🎬 Zur Watchlist hinzugefügt: %s",
		enUK: "🎬 Added to your watchlist: %s",
		esES: "🎬 Añadido a tu lista para ver: %s",
		faIR: "🎬 به فهرست تماشای شما اضافه شد: %s",
		frFR: "🎬 Ajouté à votre liste à regarder : %s",
		idID: "🎬 Ditambahkan ke daftar tontonan Anda: %s",
		itIT: "🎬 Aggiunto alla tua lista da guardare: %s",
		jaJP: "🎬 視聴リストに追加しました: %s",
		koKR: "🎬 볼 목록에 추가되었습니다: %s",
		plPL: "🎬 Dodano do listy do obejrzenia: %s",
		ptBR: "🎬 Adicionado à sua lista para assistir: %s",
		ptPT: "🎬 Adicionado à sua lista para ver: %s",
		ruRU: "🎬 Добавлено в список «Посмотреть»: %s",
		trTR: "🎬 İzleme listenize eklendi: %s",
		ukUA: "🎬 Додано до списку «Подивитися»: %s",
		uzUZ: "🎬 Tomosha roʻyxatingizga qoʻshildi: %s",
		zhCN: "🎬 已添加到您的观看列表：%s"},
	...
	SettingsCommandDescription: {
		arEG: "التحقق من إعدادات الروبوت وتعديلها",
		deDE: "Bot-Einstellungen prüfen und anpassen",
		enUK: "Check & adjust bot settings",
		esES: "Comprobar y ajustar la configuración del bot",
		faIR: "تنظیمات ربات را بررسی و تنظیم کنید",
		frFR: "Vérifier et ajuster les paramètres du robot",
		idID: "Periksa &amp; sesuaikan pengaturan bot",
		itIT: "Controlla e regola le impostazioni del bot",
		jaJP: "ボットの設定を確認して調整する",
		koKR: "봇 설정 확인 및 조정",
		plPL: "Sprawdź i dostosuj ustawienia bota",
		ptBR: "Verifique e ajuste as configurações do bot",
		ptPT: "Verifique e ajuste as definições do bot",
		ruRU: "Проверить и поменять настройки бота",
		trTR: "Bot ayarlarını kontrol edin ve ayarlayın",
		ukUA: "Перевірте та налаштуйте налаштування бота",
		uzUZ: "Bot sozlamalarini tekshiring va sozlang",
		zhCN: "检查并调整机器人设置"},
	TermsCommandDescription: {
		arEG: "الشروط والأحكام",
		deDE: "Allgemeine Geschäftsbedingungen",
		enUK: "Terms & Conditions",
		esES: "Términos y condiciones",
		faIR: "شرایط و ضوابط",
		frFR: "Conditions générales",
		idID: "Syarat &amp; Ketentuan",
		itIT: "Termini e condizioni",
		jaJP: "利用規約",
		koKR: "이용 약관",
		plPL: "Warunki korzystania",
		ptBR: "Termos e Condições",
		ptPT: "Termos e Condições",
		ruRU: "Пользовательское соглашение",
		trTR: "Şartlar ve Koşullar",
		ukUA: "Умови та положення",
		uzUZ: "Foydalanish shartlari",
		zhCN: "条款和条件"},
	TogetheredBotCommandSpots: {
		arEG: "أماكن فعالياتي",
		deDE: "Meine Veranstaltungsorte",
		enUK: "My places of events",
		esES: "Mis lugares de eventos",
		faIR: "مکان‌های برگزاری رویدادهای من",
		frFR: "Mes lieux d&#39;événements",
		idID: "Tempat acara saya",
		itIT: "I miei luoghi degli eventi",
		jaJP: "私のイベントの場所",
		koKR: "내 이벤트 장소",
		plPL: "Moje miejsca wydarzeń",
		ptBR: "Meus locais de eventos",
		ptPT: "Os meus locais de eventos",
		ruRU: "Мои места проведения событий",
		trTR: "Etkinlik yerlerim",
		ukUA: "Мої місця подій",
		uzUZ: "Mening tadbirlarim joylari",
		zhCN: "我的活动地点"},
	...
	AdvertisingCommandDescription: {
		arEG: "أعلن على @{BOT_CODE}",
		deDE: "Werben Sie bei @{BOT_CODE}",
		enUK: "Advertise at @{BOT_CODE}",
		esES: "Anúnciese en @{BOT_CODE}",
		faIR: "تبلیغات در @{BOT_CODE}",
		frFR: "Faites de la publicité sur @{BOT_CODE}",
		idID: "Beriklan di @{BOT_CODE}",
		itIT: "Pubblicizza su @{BOT_CODE}",
		jaJP: "@{BOT_CODE} に広告を掲載",
		koKR: "@{BOT_CODE}에 광고하세요",
		plPL: "Reklamuj się na @{BOT_CODE}",
		ptBR: "Anuncie em @{BOT_CODE}",
		ptPT: "Anuncie em @{BOT_CODE}",
		ruRU: "Реклама в @{BOT_CODE}",
		trTR: "@{BOT_CODE} adresinde reklam verin",
		ukUA: "Реклама на @{BOT_CODE}",
		uzUZ: "@{BOT_CODE} da reklama bering",
		zhCN: "在@{BOT_CODE}投放广告"},
	SupportUsCommandDescription: {
		arEG: "دعم تطوير @{BOT_CODE}",
		deDE: "Unterstützen Sie die Entwicklung von @{BOT_CODE}",
		enUK: "Support development of @{BOT_CODE}",
		esES: "Apoyar el desarrollo de @{BOT_CODE}",
		faIR: "پشتیبانی از توسعه @{BOT_CODE}",
		frFR: "Soutenir le développement de @{BOT_CODE}",
		idID: "Mendukung pengembangan @{BOT_CODE}",
		itIT: "Supportare lo sviluppo di @{BOT_CODE}",
		jaJP: "@{BOT_CODE}の開発をサポートする",
		koKR: "@{BOT_CODE} 개발 지원",
		plPL: "Wsparcie rozwoju @{BOT_CODE}",
		ptBR: "Apoiar o desenvolvimento de @{BOT_CODE}",
		ptPT: "Apoiar o desenvolvimento do @{BOT_CODE}",
		ruRU: "Поддержать разработку @{BOT_CODE}",
		trTR: "@{BOT_CODE} gelişimini destekleyin",
		ukUA: "Підтримка розвитку @{BOT_CODE}",
		uzUZ: "@{BOT_CODE} rivojlanishini qo&#39;llab-quvvatlash",
		zhCN: "支持@{BOT_CODE} 的开发"},
	MembershipCommandDescription: {
		arEG: "استخدم مجانًا أو احصل على حساب PRO",
		deDE: "Kostenlos nutzen oder PRO-Konto erhalten",
		enUK: "Use for free or get PRO account",
		esES: "Úselo gratis o obtenga una cuenta PRO",
		faIR: "رایگان استفاده کنید یا حساب کاربری حرفه‌ای (PRO) بگیرید",
		frFR: "Utilisez gratuitement ou obtenez un compte PRO",
		idID: "Gunakan secara gratis atau dapatkan akun PRO",
		itIT: "Utilizzalo gratuitamente o ottieni un account PRO",
		jaJP: "無料でご利用いただくか、PROアカウントを取得してください",
		koKR: "무료로 사용하거나 PRO 계정을 받으세요",
		plPL: "Użyj za darmo lub załóż konto PRO",
		ptBR: "Use gratuitamente ou obtenha uma conta PRO",
		ptPT: "Utilize gratuitamente ou obtenha uma conta PRO",
		ruRU: "Используй бесплатно или выбери ПРО акккаунт",
		trTR: "Ücretsiz kullanın veya PRO hesap edinin",
		ukUA: "Користуйтеся безкоштовно або отримайте PRO-акаунт",
		uzUZ: "Bepul foydalaning yoki PRO hisobini oling",
		zhCN: "免费使用或获取 PRO 帐户"},
	WalletCommandDescription: {
		arEG: "رصيدك ومعاملاتك الأخيرة",
		deDE: "Ihr Guthaben und Ihre letzten Transaktionen",
		enUK: "Your balance & latest transactions",
		esES: "Su saldo y últimas transacciones",
		faIR: "موجودی و آخرین تراکنش‌های شما",
		frFR: "Votre solde et vos dernières transactions",
		idID: "Saldo &amp; transaksi terakhir Anda",
		itIT: "Il tuo saldo e le ultime transazioni",
		jaJP: "残高と最新の取引",
		koKR: "잔액 및 최근 거래 내역",
		plPL: "Twoje saldo i ostatnie transakcje",
		ptBR: "Seu saldo e últimas transações",
		ptPT: "O seu saldo e últimas transações",
		ruRU: "Ваш баланс и последние транзакции",
		trTR: "Bakiyeniz ve son işlemleriniz",
		ukUA: "Ваш баланс та останні транзакції",
		uzUZ: "Balansingiz va oxirgi tranzaksiyalaringiz",
		zhCN: "您的余额和最新交易"},
	WalletTitle: {
		arEG: "رصيدي @{BOT_CODE}",
		deDE: "Mein Guthaben @{BOT_CODE}",
		enUK: "My balance @{BOT_CODE}",
		esES: "Mi saldo @{BOT_CODE}",
		faIR: "موجودی من @{BOT_CODE}",
		frFR: "Mon solde @{BOT_CODE}",
		idID: "Saldo saya @{BOT_CODE}",
		itIT: "Il mio saldo @{BOT_CODE}",
		jaJP: "残高@{BOT_CODE}",
		koKR: "내 잔액 @{BOT_CODE}",
		plPL: "Moje saldo @{BOT_CODE}",
		ptBR: "Meu saldo @{BOT_CODE}",
		ptPT: "O meu saldo @{BOT_CODE}",
		ruRU: "Мой баланс @{BOT_CODE}",
		trTR: "Bakiyem @{BOT_CODE}",
		ukUA: "Мій баланс @{BOT_CODE}",
		uzUZ: "Mening balansim @{BOT_CODE}",
		zhCN: "我的余额@{BOT_CODE}"},
	TopUpWithStarts: {
		arEG: "➕ قم بشحن رصيدك بـ ⭐",
		deDE: "➕ Aufladen mit ⭐",
		enUK: "➕ Top up with ⭐",
		esES: "➕ Recarga con ⭐",
		faIR: "➕ شارژ با ⭐",
		frFR: "➕ Recharger avec ⭐",
		idID: "➕ Isi ulang dengan ⭐",
		itIT: "➕ Ricarica con ⭐",
		jaJP: "➕ ⭐でチャージ",
		koKR: "➕ ⭐로 충전하세요",
		plPL: "➕ Doładuj za pomocą ⭐",
		ptBR: "➕ Recarregue com ⭐",
		ptPT: "➕ Recarregue com ⭐",
		ruRU: "➕ Пополнить кошелёк ⭐",
		trTR: "➕ ⭐ ile doldurun",
		ukUA: "➕ Поповнити за допомогою ⭐",
		uzUZ: "➕ ⭐ bilan toʻldiring",
		zhCN: "➕ 充值⭐"},
	StarsBalance: {
		arEG: "⭐ %d نجوم",
		deDE: "⭐ %d Sterne",
	...
	RemoveActivityButtonText: {
		arEG: "❌ إزالة النشاط",
		deDE: "❌ Aktivität entfernen",
		enUK: "❌ Remove activity",
		esES: "❌ Eliminar actividad",
		faIR: "❌ حذف فعالیت",
		frFR: "❌ Supprimer l&#39;activité",
		idID: "❌ Hapus aktivitas",
		itIT: "❌ Rimuovi attività",
		jaJP: "❌ アクティビティを削除",
		koKR: "❌ 활동 제거",
		plPL: "❌ Usuń aktywność",
		ptBR: "❌ Remover atividade",
		ptPT: "❌ Remover atividade",
		ruRU: "❌ Убрать активность",
		trTR: "❌ Etkinliği kaldır",
		ukUA: "❌ Видалити активність",
		uzUZ: "❌ Faoliyatni olib tashlang",
		zhCN: "❌ 删除活动"},
	MyFollowersTitle: {
		arEG: "🕵️‍♂ متابعي",
		deDE: "🕵️‍♂ Mein Follower",
		enUK: "🕵️‍♂ My follower",
		esES: "🕵️‍♂ Mi seguidor",
		faIR: "🕵️‍♂ دنبال‌کننده‌ی من",
		frFR: "🕵️‍♂ Mon follower",
		idID: "🕵️‍♂ Pengikut saya",
		itIT: "🕵️‍♂ Il mio follower",
		jaJP: "🕵️‍♂ 私のフォロワー",
		koKR: "🕵️‍♂ 내 팔로워",
		plPL: "🕵️‍♂ Mój obserwator",
		ptBR: "🕵️‍♂ Meu seguidor",
		ptPT: "🕵️‍♂ O meu seguidor",
		ruRU: "🕵️‍♂ Мои подписчики",
		trTR: "🕵️‍♂ Takipçim",
		ukUA: "🕵️‍♂ Мій підписник",
		uzUZ: "🕵️‍♂ Mening obunachim",
		zhCN: "🕵️‍♂ 我的追随者"},
	FollowersOfUserTitle: {
		arEG: "🕵️‍♂ متابع %s",
		deDE: "🕵️‍♂ Follower von %s",
		enUK: "🕵️‍♂ Follower of %s",
		esES: "🕵️‍♂ Seguidor de %s",
		faIR: "🕵️‍♂ دنبال‌کننده‌ی %s",
		frFR: "🕵️‍♂ Abonné de %s",
		idID: "🕵️‍♂ Pengikut %s",
		itIT: "🕵️‍♂ Seguace di %s",
		jaJP: "🕵️‍♂ %s のフォロワー",
		koKR: "🕵️‍♂ %s의 팔로워",
		plPL: "🕵️‍♂ Obserwujący %s",
		ptBR: "🕵️‍♂ Seguidor de %s",
		ptPT: "🕵️‍♂ Seguidor de %s",
		ruRU: "🕵️‍♂ Подписчики %s",
		trTR: "🕵️‍♂ %s takipçisi",
		ukUA: "🕵️‍♂ Підписник %s",
		uzUZ: "🕵️‍♂ %s obunachisi",
	...
	MyFollowsTitle: {
		...
		enUK: "👀 My follows",
		esES: "👀 Mis seguidores",
		faIR: "👀 دنبال‌کنندگان من",
		frFR: "👀 Mes abonnés",
		idID: "👀 Pengikut saya",
		itIT: "👀 I miei follower",
		jaJP: "👀 私のフォロー",
		koKR: "👀 내 팔로워",
		plPL: "👀 Moi obserwatorzy",
		ptBR: "👀 Meus seguidores",
		ptPT: "👀 Os meus seguidores",
		ruRU: "👀 Мои подписки",
		trTR: "👀 Takipçilerim",
		ukUA: "👀 Мої підписники",
		uzUZ: "👀 Mening kuzatuvchilarim",
		zhCN: "👀 我的关注者"},
	UserFollowingTitle: {
		arEG: "👀‍ %s متابعين",
		deDE: "👀‍ %s folgt",
	...
	IntentNotificationMessageUserIsGoingTo: {
		arEG: "{USER_NAME} سوف يذهب إلى",
		deDE: "{USER_NAME} wird",
		enUK: "{USER_NAME} is going to",
		esES: "{USER_NAME} va a",
		faIR: "{USER_NAME} قصد دارد",
		frFR: "{USER_NAME} va",
		idID: "{USER_NAME} akan",
		itIT: "{USER_NAME} sta per",
		jaJP: "{USER_NAME}は",
		koKR: "{USER_NAME}이(가) 갈 것입니다",
		plPL: "{USER_NAME} zamierza",
		ptBR: "{USER_NAME} vai",
		ptPT: "{USER_NAME} vai",
		ruRU: "{USER_NAME} планирует быть",
		trTR: "{USER_NAME} gidiyor",
		ukUA: "{USER_NAME} збирається",
		uzUZ: "{USER_NAME} boradi",
		zhCN: "{USER_NAME} 将要"},
	IntentNotificationPlannedActivities: {
		arEG: "الأنشطة المخطط لها",
		deDE: "Geplante Aktivitäten",
		enUK: "Planned activities",
		esES: "Actividades planificadas",
		faIR: "فعالیت‌های برنامه‌ریزی‌شده",
		frFR: "Activités prévues",
		idID: "Kegiatan yang direncanakan",
		itIT: "Attività pianificate",
		jaJP: "計画されている活動",
		koKR: "계획된 활동",
		plPL: "Zaplanowane działania",
		ptBR: "Atividades planejadas",
		ptPT: "Atividades planeadas",
		ruRU: "Запланированные активности",
		trTR: "Planlanan faaliyetler",
		ukUA: "Заплановані заходи",
		uzUZ: "Rejalashtirilgan tadbirlar",
		zhCN: "计划活动"},
	WouldYouJoinRsvpNow: {
		arEG: "هل ترغب بالانضمام؟ سجل حضورك الآن!",
		deDE: "Möchten Sie mitmachen? Jetzt anmelden!",
		enUK: "Would you join? RSVP now!",
		esES: "¿Te apuntas? ¡Confirma tu asistencia ya!",
		faIR: "آیا شما هم عضو می‌شوید؟ همین حالا درخواست خود را ثبت کنید!",
		frFR: "Souhaitez-vous vous joindre à nous ? RSVP dès maintenant !",
		idID: "Apakah Anda berminat untuk bergabung? RSVP sekarang!",
		itIT: "Vuoi partecipare? Rispondi subito!",
		jaJP: "参加しませんか？今すぐ返信してください！",
		koKR: "참여하시겠습니까? 지금 RSVP하세요!",
		plPL: "Dołączysz? Potwierdź obecność już teraz!",
		ptBR: "Você gostaria de participar? Confirme sua presença agora mesmo!",
		ptPT: "Gostaria de participar? Confirme a sua presença agora mesmo!",
		ruRU: "Вы присоеденитесь? Дайте знать!",
		trTR: "Katılmak ister misiniz? Hemen katılın!",
		ukUA: "Ви б приєдналися? Підтвердіть свою участь зараз!",
		uzUZ: "Siz qo&#39;shilasizmi? Hozir javob bering!",
		zhCN: "你想参加吗？立即回复！"},
	RsvpWizardTitle: {
		arEG: "أخبر الآخرين أين وأين ستكون.",
		deDE: "Lassen Sie andere wissen, wo Sie sind und wo Sie sein werden.",
		enUK: "Let others know where & where you going to be.",
		esES: "Hazle saber a los demás dónde y adónde vas a estar.",
		faIR: "به دیگران اطلاع دهید که کجا و به کجا خواهید رفت.",
		frFR: "Faites savoir aux autres où et où vous allez être.",
		idID: "Beritahu orang lain di mana &amp; ke mana Anda akan berada.",
		itIT: "Fai sapere agli altri dove e dove andrai.",
		jaJP: "自分がどこにいるのか、どこに行くのかを他の人に知らせましょう。",
		koKR: "다른 사람들에게 당신이 어디에 있고 어디로 갈 것인지 알려주세요.",
		plPL: "Daj znać innym, gdzie i dokąd się wybierasz.",
		ptBR: "Deixe que os outros saibam onde você estará.",
		ptPT: "Deixe que os outros saibam onde estará.",
		ruRU: "Сообщи другим, где и когда ты собираешься быть.",
		trTR: "Başkalarına nerede ve nerelerde olacağınızı bildirin.",
		ukUA: "Повідомте інших, де і як ви збираєтеся бути.",
		uzUZ: "Qaerda va qayerda bo&#39;lishingizni boshqalarga bildiring.",
		zhCN: "让其他人知道您在哪里以及要去哪里。"},
	AboutBotTitle: {
		arEG: "حول @%s",
		deDE: "Über @%s",
		enUK: "About @%s",
		...
		zhCN: "关于@%s"},
	TechStackTitle: {
		arEG: "مجموعة التكنولوجيا لدينا",
		deDE: "Unser Tech-Stack",
		enUK: "Our tech stack",
		esES: "Nuestra pila tecnológica",
		faIR: "مجموعه فناوری ما",
		frFR: "Notre pile technologique",
		idID: "Tumpukan teknologi kami",
		itIT: "Il nostro stack tecnologico",
		jaJP: "当社の技術スタック",
		koKR: "우리의 기술 스택",
		plPL: "Nasz stos technologiczny",
		ptBR: "Nossa pilha de tecnologia",
		ptPT: "A nossa pilha de tecnologia",
		ruRU: "Наш технологический стек",
		trTR: "Teknoloji yığınımız",
		ukUA: "Наш технологічний стек",
		uzUZ: "Bizning texnologiya to&#39;plami",
		zhCN: "我们的技术栈"},
	TechStackText: {
		arEG: `
- مكتوب <a href='https://go.dev/'>بلغة Go</a> 
- مستضاف على Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- البيانات مخزنة في <a href='https://firebase.google.com/products/firestore'>قاعدة بيانات Google Firestore</a> 
- يستخدم https://github.com/bots-go-framework/
- يستخدم <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> للتواصل مع Telegram
يستخدم https://github.com/dal-go/ كطبقة تجريد لقاعدة البيانات

إذا كنت تريد معرفة المزيد حول كيفية تطويرنا، اشترك في @SneatDevDiaries!
`,
		deDE: `
– in <a href='https://go.dev/'>der Sprache Go</a> geschrieben 
– auf Google <a href='https://cloud.google.com/appengine'>App Engine</a> gehostet 
– Daten in Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a> gespeichert 
– verwendet https://github.com/bots-go-framework/
– verwendet <a href='https://github.com/bots-go-framework/bots-api-telegram'>Bots-API-Telegram</a> zur Kommunikation mit Telegram
verwendet https://github.com/dal-go/ als Datenbankabstraktionsschicht

Wenn Sie mehr über unsere Entwicklung erfahren möchten, abonnieren Sie @SneatDevDiaries!
`,
		enUK: `
...
`,
		esES: `
- escrito en <a href='https://go.dev/'>lenguaje Go</a> 
- alojado en Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- datos almacenados en Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a> 
- usa https://github.com/bots-go-framework/
- usa <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> para comunicarse con Telegram
usa https://github.com/dal-go/ como capa de abstracción de base de datos

Si quieres saber más sobre cómo desarrollamos, suscríbete a @SneatDevDiaries.
`,
		faIR: `
- نوشته شده به <a href='https://go.dev/'>زبان Go</a> 
- میزبانی شده در <a href='https://cloud.google.com/appengine'>موتور برنامه</a> گوگل 
- داده ها در <a href='https://firebase.google.com/products/firestore'>پایگاه داده گوگل فایراستور</a> ذخیره می شوند 
- از https://github.com/bots-go-framework/
- از <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> برای ارتباط با تلگرام استفاده می کند 
از https://github.com/dal-go/ به عنوان لایه انتزاع پایگاه داده استفاده می کند 

اگر می خواهید در مورد نحوه توسعه ما بیشتر بدانید، در @SneatDevDiaries مشترک شوید!
`,
		frFR: `
- écrit en <a href='https://go.dev/'>langage Go</a> 
- hébergé sur Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- données stockées dans <a href='https://firebase.google.com/products/firestore'>la base de données Google Firestore</a> 
- utilise https://github.com/bots-go-framework/
- utilise <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> pour communiquer avec Telegram
utilise https://github.com/dal-go/ comme couche d&#39;abstraction de base de données

Si vous souhaitez en savoir plus sur notre façon de développer, abonnez-vous à @SneatDevDiaries !
`,
		idID: `
- ditulis dalam <a href='https://go.dev/'>bahasa Go</a> 
- dihosting di Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- data disimpan di Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a> 
- menggunakan https://github.com/bots-go-framework/
- menggunakan <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> untuk berkomunikasi dengan Telegram
menggunakan https://github.com/dal-go/ sebagai Database Abstraction Layer

Jika Anda ingin mengetahui lebih lanjut tentang cara kami mengembangkan, berlanggananlah ke @SneatDevDiaries!
`,
		itIT: `
- scritto in <a href='https://go.dev/'>linguaggio Go</a> 
- ospitato su Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- dati archiviati nel <a href='https://firebase.google.com/products/firestore'>database Google Firestore</a> 
- utilizza https://github.com/bots-go-framework/
- utilizza <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> per comunicare con Telegram
utilizza https://github.com/dal-go/ come livello di astrazione del database

Se vuoi saperne di più su come sviluppiamo, iscriviti a @SneatDevDiaries!
`,
		jaJP: `
- <a href='https://go.dev/'>Go 言語</a>で記述されています 
- Google <a href='https://cloud.google.com/appengine'>App Engine</a>でホストされています 
- データは Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a>に保存されています 
- https://github.com/bots-go-framework/ を使用しています
- Telegram との通信には<a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a>を使用しています
データベース抽象化レイヤーとして https://github.com/dal-go/ を使用しています

私たちの開発方法について詳しく知りたい場合は、@SneatDevDiaries を購読してください!
`,
		koKR: `
- <a href='https://go.dev/'>Go 언어</a> 로 작성됨 
- Google <a href='https://cloud.google.com/appengine'>App Engine</a> 에서 호스팅됨 
- Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a> 에 데이터 저장됨 
- https://github.com/bots-go-framework/ 사용
- Telegram과 통신하기 위해 <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> 사용
https://github.com/dal-go/를 데이터베이스 추상화 계층으로 사용

저희의 개발 방식에 대해 자세히 알고 싶으시다면 @SneatDevDiaries를 구독하세요!
`,
		plPL: `
- napisany w <a href='https://go.dev/'>języku Go</a> 
- hostowany na Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- dane przechowywane w Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a> 
- używa https://github.com/bots-go-framework/
- używa <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> do komunikacji z Telegramem
używa https://github.com/dal-go/ jako warstwy abstrakcji bazy danych

Jeśli chcesz dowiedzieć się więcej o tym, jak się rozwijamy, zasubskrybuj @SneatDevDiaries!
`,
		ptBR: `
- escrito em <a href='https://go.dev/'>linguagem Go</a> 
- hospedado no Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- dados armazenados no Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a> 
- usa https://github.com/bots-go-framework/
- usa <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> para se comunicar com o Telegram
usa https://github.com/dal-go/ como camada de abstração de banco de dados

Se quiser saber mais sobre como desenvolvemos, assine @SneatDevDiaries!
`,
		ptPT: `
- escrito em <a href='https://go.dev/'>linguagem Go</a> 
- alojado no Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- dados armazenados no Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a> 
- utiliza https://github.com/bots-go-framework/
- utiliza <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> para comunicar com o Telegram
utiliza https://github.com/dal-go/ como camada de abstração de base de dados

Se quiser saber mais sobre como desenvolvemos, subscreva @SneatDevDiaries!
`,
		ruRU: `
...
`,
		trTR: `
- <a href='https://go.dev/'>Go dilinde</a> yazılmış 
- Google <a href='https://cloud.google.com/appengine'>App Engine&#39;de</a> barındırılır 
- veriler Google <a href='https://firebase.google.com/products/firestore'>Firestore DB&#39;de</a> saklanır 
- https://github.com/bots-go-framework/ adresini kullanır
- Telegram ile iletişim kurmak için <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> adresini kullanır
Veritabanı Soyutlama Katmanı olarak https://github.com/dal-go/ adresini kullanır

Nasıl geliştirdiğimiz hakkında daha fazla bilgi edinmek istiyorsanız @SneatDevDiaries&#39;e abone olun!
`,
		ukUA: `
- написано <a href='https://go.dev/'>мовою Go</a> 
- розміщено на Google <a href='https://cloud.google.com/appengine'>App Engine</a> 
- дані зберігаються в <a href='https://firebase.google.com/products/firestore'>базі даних Google Firestore</a> 
- використовує https://github.com/bots-go-framework/
- використовує <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a> для зв&#39;язку з Telegram
використовує https://github.com/dal-go/ як шар абстракції бази даних

Якщо ви хочете дізнатися більше про те, як ми розробляємо, підпишіться на @SneatDevDiaries!
`,
		uzUZ: `
- <a href='https://go.dev/'>Go tilida</a> yozilgan 
- Google <a href='https://cloud.google.com/appengine'>App Engineda</a> joylashtirilgan 
- Google <a href='https://firebase.google.com/products/firestore'>Firestore DBda</a> saqlanadigan maʼlumotlar 
- https://github.com/bots-go-framework/
- Telegram bilan bogʻlanish uchun <a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegramdan</a> foydalanadi
Ablishing haqida koʻproq bilishni hohlagan holda https://github.com/dal-goction/ dan foydalanadi{} @SneatDevDiaries-ga obuna bo&#39;lishni ishlab chiqamiz!
`,
		zhCN: `
- 使用<a href='https://go.dev/'>Go 语言</a>编写 
- 托管在 Google <a href='https://cloud.google.com/appengine'>App Engine</a>上 
- 数据存储在 Google <a href='https://firebase.google.com/products/firestore'>Firestore DB</a>中 
- 使用 https://github.com/bots-go-framework/
- 使用<a href='https://github.com/bots-go-framework/bots-api-telegram'>bots-api-telegram</a>与 Telegram 通信
使用 https://github.com/dal-go/ 作为数据库抽象层

如果您想了解更多关于我们如何开发的信息，请订阅 @SneatDevDiaries！
`},
	WouldYouSupportUsTitle: {
		arEG: "هل تدعمنا؟",
		deDE: "Würden Sie uns unterstützen?",
		enUK: "Would you support us?",
		esES: "¿Nos apoyarías?",
		faIR: "آیا از ما حمایت خواهید کرد؟",
		frFR: "Nous soutiendriez-vous ?",
		idID: "Maukah Anda mendukung kami?",
		itIT: "Vorresti sostenerci?",
		jaJP: "私たちを応援していただけますか？",
		koKR: "우리를 지원해 주시겠습니까?",
		plPL: "Czy nas wesprzesz?",
		ptBR: "Você nos apoiaria?",
		ptPT: "Você apoiar-nos-ia?",
		ruRU: "Поддержите нас?",
		trTR: "Bizi destekler misiniz?",
		ukUA: "Ви б нас підтримали?",
		uzUZ: "Bizni qo&#39;llab quvvatlaysizmi?",
		zhCN: "你会支持我们吗？"},
	WouldYouSupportUsText: {
		arEG: `
إذا وجدت أن @{BOT_CODE} مفيد لك، فيرجى التفكير في تخصيص دقيقة لـ:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>التبرع</a> ببعض نجوم Telegram لتسريع التطوير

	- إعطاء نجمة لمشاريعنا مفتوحة المصدر - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> ، <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 إن <i>دعم مستخدمينا يعزز دافعنا للقيام بمزيد من الأشياء الرائعة على الروبوت!</i> 
`,
		deDE: `
Wenn @{BOT_CODE} für Sie nützlich war, nehmen Sie sich bitte eine Minute Zeit, um:&quot;)

	- einige Telegram-Sterne <a href='https://t.me/{BOT_CODE}?start=donate'>zu spenden</a> , um die Entwicklung zu beschleunigen

	- unseren Open-Source-Projekten einen Stern zu geben - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Die Unterstützung unserer Benutzer steigert unsere Motivation, mehr coole Sachen auf dem Bot zu machen!</i> 
`,
		enUK: `
...
`,
		esES: `
Si @{BOT_CODE} te resultó útil, considera tomarte un minuto para:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>donar</a> algunas estrellas de Telegram para acelerar el desarrollo

	- dar una estrella a nuestros proyectos de código abierto: <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>¡El apoyo de nuestros usuarios aumenta nuestra motivación para hacer más cosas geniales en el bot!</i> 
`,
		faIR: `
اگر @{BOT_CODE} را برای خود مفید یافتید، لطفاً یک دقیقه وقت بگذارید تا:&quot;)

	- چند ستاره تلگرام برای سرعت بخشیدن به توسعه <a href='https://t.me/{BOT_CODE}?start=donate'>اهدا کنید</a> 

	- به پروژه‌های متن‌باز ما - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> ، <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>حمایت کاربران ما انگیزه ما را برای انجام کارهای جالب‌تر روی ربات افزایش می‌دهد!</i> 
`,
		frFR: `
Si vous avez trouvé @{BOT_CODE} utile, veuillez prendre une minute pour :&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>donner</a> quelques étoiles Telegram pour accélérer le développement

	- donner une étoile à nos projets open source - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Le soutien de nos utilisateurs renforce notre motivation à faire plus de choses intéressantes sur le bot !</i> 
`,
		idID: `
Jika Anda merasa @{BOT_CODE} bermanfaat, mohon luangkan waktu sebentar untuk:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>donasikan</a> beberapa bintang Telegram untuk mempercepat pengembangan

	- berikan bintang untuk proyek sumber terbuka kami - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Dukungan dari pengguna kami meningkatkan motivasi kami untuk melakukan lebih banyak hal keren pada bot ini!</i> 
`,
		itIT: `
Se ritieni che @{BOT_CODE} ti sia utile, prenditi un minuto per:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>donare</a> alcune stelle di Telegram per velocizzare lo sviluppo

	- dare una stella ai nostri progetti open source - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Il supporto dei nostri utenti aumenta la nostra motivazione a realizzare altre cose interessanti sul bot!</i> 
`,
		jaJP: `
@{BOT_CODE} が役に立ったと思われる場合は、次のことを行ってください。&quot;)

	- 開発をスピードアップするために Telegram スターをいくつか<a href='https://t.me/{BOT_CODE}?start=donate'>寄付して</a>ください。

	- オープン ソース プロジェクトにスターを付けてください - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> 、 <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

<i>ユーザーからのサポートは、ボットでさらにクールなものを作るモチベーションを高めます。</i> 
`,
		koKR: `
@{BOT_CODE}이(가) 유용하다고 생각되시면 잠시 시간을 내어 다음을 수행해 주세요:&quot;)

	- 개발 속도를 높이기 위해 Telegram 별점을 몇 개 <a href='https://t.me/{BOT_CODE}?start=donate'>기부해 주세요</a> 

	- 오픈 소스 프로젝트인 <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> 에 별점을 주세요 &quot;)

 <i>사용자 여러분의 지원은 봇에서 더 멋진 기능을 개발하려는 동기를 부여합니다!</i> 
`,
		plPL: `
Jeśli uważasz, że @{BOT_CODE} jest dla Ciebie przydatne, prosimy o poświęcenie chwili na:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>przekazanie</a> kilku gwiazdek Telegrama, aby przyspieszyć rozwój

	- przyznanie gwiazdki naszym projektom open source - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Wsparcie od naszych użytkowników zwiększa naszą motywację do tworzenia kolejnych fajnych rzeczy w bocie!</i> 
`,
		ptBR: `
Se você achou @{BOT_CODE} útil, considere reservar um minuto para:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>doar</a> algumas estrelas do Telegram para acelerar o desenvolvimento

	- dar uma estrela para nossos projetos de código aberto - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>O suporte de nossos usuários aumenta nossa motivação para fazer mais coisas legais no bot!</i> 
`,
		ptPT: `
Se achou @{BOT_CODE} útil, considere tirar um minuto para:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>doar</a> algumas estrelas do Telegram para acelerar o desenvolvimento

	- dar uma estrela aos nossos projetos de código aberto - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>O apoio dos nossos utilizadores aumenta a nossa motivação para fazer mais coisas giras no bot!</i> 
`,
		ruRU: `
...
`,
		trTR: `
@{BOT_CODE}&#39;ı faydalı bulduysanız lütfen bir dakikanızı ayırıp şunları yapmayı düşünün:&quot;)

	- Geliştirmeyi hızlandırmak için birkaç Telegram yıldızı <a href='https://t.me/{BOT_CODE}?start=donate'>bağışlayın</a> 

	- Açık kaynaklı projelerimize bir yıldız verin - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Kullanıcılarımızdan gelen destek, botta daha harika şeyler yapma motivasyonumuzu artırıyor!</i> 
`,
		ukUA: `
Якщо @{BOT_CODE} був корисним для вас, будь ласка, приділіть хвилинку, щоб:&quot;)

	- <a href='https://t.me/{BOT_CODE}?start=donate'>пожертвувати</a> кілька зірок Telegram для пришвидшення розробки

	- поставити зірку нашим проектам з відкритим кодом - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Підтримка наших користувачів підвищує нашу мотивацію робити більше цікавих речей на боті!</i> 
`,
		uzUZ: `
Agar @{BOT_CODE} siz uchun foydali boʻlgan boʻlsa, iltimos, bir daqiqa vaqt ajrating:&quot;)

	- rivojlanishni tezlashtirish uchun bir nechta Telegram yulduzlarini <a href='https://t.me/{BOT_CODE}?start=donate'>sovgʻa qiling</a> 

	- ochiq kodli loyihalarimizga yulduz bering - <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> , <a href='https://github.com/dal-go/'>DALgo</a> &quot;)

 <i>Foydalanuvchilarimizning qoʻllab-quvvatlashi bizni ishda motivatsiyamizni yanada oshiradi!</i> 
`,
		zhCN: `
如果您发现@{BOT_CODE}对您有用，请考虑花一点时间：&quot;)

	-<a href='https://t.me/{BOT_CODE}?start=donate'>捐赠</a>一些 Telegram 星星以加速开发

	- 为我们的开源项目（ <a href='https://github.com/bots-go-framework/'>bots-go-framework</a> 、 <a href='https://github.com/dal-go/'>DALgo</a> ）点个星&quot;)

<i>来自用户的支持增强了我们在机器人上做更多酷事的动力！</i> 
`},
	PremiumAccountTitle: {
		arEG: "حساب مميز",
		deDE: "Premium-Konto",
	...
	PremiumAccountText: {
		arEG: `
- الخطة الشهرية - 100⭐ شهريًا
- الخطة السنوية - <u>وفر 25%%</u> - 75⭐ شهريًا`,
		deDE: `
- Monatsplan - 100⭐ pro Monat
- Jahresplan - <u>25 % sparen</u> - 75⭐ pro Monat`,
		enUK: `
- Monthly plan - 100⭐ per month
- Yearly plan - <u>save 25%%</u> - 75⭐ per month`,
		esES: `
- Plan mensual - 100⭐ al mes
- Plan anual - <u>ahorra 25%</u> - 75⭐ al mes`,
		faIR: `
- طرح ماهانه - ۱۰۰⭐ در ماه
- طرح سالانه - <u>۲۵٪ - ۷۵⭐ در ماه صرفه‌جویی کنید</u>`,
		frFR: `
- Forfait mensuel - 100⭐ par mois
- Forfait annuel - <u>économisez 25%%</u> - 75⭐ par mois`,
		idID: `
- Paket bulanan - 100⭐ per bulan
- Paket tahunan - <u>hemat 25%</u> - 75⭐ per bulan`,
		itIT: `
- Piano mensile - 100⭐ al mese
- Piano annuale - <u>risparmia il 25%</u> - 75⭐ al mese`,
		jaJP: `
- 月間プラン - 月額 100⭐
- 年間プラン - <u>25% 割引</u>- 月額 75⭐`,
		koKR: `
- 월간 플랜 - 월 100⭐
- 연간 플랜 - <u>25%% 절약</u> - 월 75⭐`,
		plPL: `
- Plan miesięczny - 100⭐ miesięcznie
- Plan roczny - <u>oszczędź 25%%</u> - 75⭐ miesięcznie`,
		ptBR: `
- Plano mensal - 100⭐ por mês
- Plano anual - <u>economize 25%%</u> - 75⭐ por mês`,
		ptPT: `
- Plano mensal - 100⭐ por mês
- Plano anual - <u>poupe 25%%</u> - 75⭐ por mês`,
		ruRU: `
- Ежемесячный план - 100⭐ в месяц
- Годовой план - <u>экономия 25%%</u> - 75⭐ в месяц`,
		trTR: `
- Aylık plan - Ayda 100⭐
- Yıllık plan - Ayda <u>%25 - 75⭐ tasarruf edin</u>`,
		ukUA: `
- Щомісячний план – 100⭐ на місяць
- Річний план – <u>зекономте 25%</u> – 75⭐ на місяць`,
		uzUZ: `
- Oylik reja - oyiga 100⭐
- Yillik reja - <u>25%% tejang</u> - oyiga 75⭐`,
		zhCN: `
- 月度计划 - 每月 100⭐
- 年度计划 -<u>节省 25%%</u> - 每月 75⭐`},
	GetMonthlyPlanButtonText: {
		arEG: "اخترت الخطة الشهرية",
		deDE: "Ich wähle einen Monatsplan",
		enUK: "I choose monthly plan",
		esES: "Elijo plan mensual",
		faIR: "من طرح ماهانه را انتخاب می‌کنم",
		frFR: "Je choisis un forfait mensuel",
		idID: "Saya memilih paket bulanan",
		itIT: "Ho scelto il piano mensile",
		jaJP: "月額プランを選択",
		koKR: "나는 월간 계획을 선택합니다",
		plPL: "Wybieram plan miesięczny",
		ptBR: "Eu escolho o plano mensal",
		ptPT: "Eu escolho o plano mensal",
		ruRU: "Я выбираю ежемесячный план",
		trTR: "Aylık planı seçiyorum",
		ukUA: "Я вибираю місячний план",
		uzUZ: "Men oylik rejani tanlayman",
		zhCN: "我选择包月套餐"},
	GetYearlyPlanButtonText: {
		arEG: "أختار الخطة السنوية",
		deDE: "Ich wähle den Jahresplan",
		enUK: "I choose yearly plan",
		esES: "Elijo el plan anual",
		faIR: "من طرح سالانه را انتخاب می‌کنم",
		frFR: "Je choisis un forfait annuel",
		idID: "Saya memilih paket tahunan",
		itIT: "Ho scelto il piano annuale",
		jaJP: "年間プランを選択します",
		koKR: "나는 연간 플랜을 선택합니다",
		plPL: "Wybieram plan roczny",
		ptBR: "Eu escolho o plano anual",
		ptPT: "Eu escolho o plano anual",
		ruRU: "Я выбираю ежегодный план",
		trTR: "Yıllık planı seçiyorum",
		ukUA: "Я обираю річний план",
		uzUZ: "Men yillik rejani tanlayman",
		zhCN: "我选择包年套餐"},
	WhyGoPremiumTitle: {
		arEG: "لماذا تختار الإصدار المميز؟",
		deDE: "Warum Premium werden?",
		enUK: "Why go premium?",
		esES: "¿Por qué hacerse premium?",
		faIR: "چرا باید به سراغ پریمیوم برویم؟",
		frFR: "Pourquoi passer au premium ?",
		idID: "Mengapa harus premium?",
		itIT: "Perché scegliere l&#39;abbonamento premium?",
		jaJP: "プレミアムになる理由は何ですか?",
		koKR: "프리미엄을 선택하는 이유는?",
		plPL: "Dlaczego warto wybrać wersję premium?",
		ptBR: "Por que optar por uma versão premium?",
		ptPT: "Porquê optar por uma versão premium?",
		ruRU: "Почему стоит выбрать премиум?",
		trTR: "Neden premium&#39;a geçmelisiniz?",
		ukUA: "Чому варто переходити на преміум-клас?",
		uzUZ: "Nega premiumga o&#39;ting?",
		zhCN: "为什么要选择高级版？"},
	WhyGoPremiumText: {
		arEG: `يمكنك دائمًا استخدام بوتنا مجانًا. لكن الإعلانات في البوت لا تغطي حتى تكاليف الاستضافة. باختيارك باقة مدفوعة، يمكنك: - دعم التحسينات والميزات الجديدة - إخفاء الإعلانات ( <i>باستثناء الإعلانات المحلية المرتبطة بموقع/مكان محدد</i> )`,
		deDE: `
Sie können unseren Bot jederzeit kostenlos nutzen.

Die Anzeigen in einem Bot decken jedoch kaum die Hosting-Kosten. Mit einem kostenpflichtigen Tarif:

- Unterstützen Sie Verbesserungen und neue Funktionen.
- Blenden Sie Anzeigen aus ( <i>mit Ausnahme der lokalen Anzeigen, die mit einem bestimmten Ort/Veranstaltungsort verknüpft sind</i> ).
`,
		enUK: `
...
`,
		esES: `
Siempre puedes usar nuestro bot gratis.

Pero los anuncios en un bot apenas cubren los gastos de alojamiento. Al elegir un plan de pago:

- Accedes a mejoras y nuevas funciones.
- Ocultas los anuncios ( <i>excepto los anuncios locales vinculados a un lugar específico</i> ).
`,
		faIR: `
شما همیشه می‌توانید از ربات ما به صورت رایگان استفاده کنید.

اما تبلیغات در یک ربات به سختی حتی هزینه‌های میزبانی را پوشش می‌دهد. با انتخاب یک طرح پولی شما:

- از بهبودها و ویژگی‌های جدید پشتیبانی می‌کنید
- تبلیغات را پنهان می‌کنید ( <i>به جز تبلیغات محلی مرتبط با یک مکان/محل خاص</i> )
`,
		frFR: `
Vous pouvez toujours utiliser notre bot gratuitement.

Mais les publicités d&#39;un bot couvrent à peine les frais d&#39;hébergement. En choisissant une offre payante, vous :

- Soutenez les améliorations et les nouvelles fonctionnalités ;
- Masquez les publicités ( <i>sauf celles liées à un lieu spécifique</i> ) ;
`,
		idID: `
Anda selalu dapat menggunakan bot kami secara gratis.

Namun, iklan di bot bahkan hampir tidak dapat menutupi biaya hosting. Dengan memilih paket berbayar, Anda:

- Mendukung peningkatan &amp; fitur baru
- Menyembunyikan iklan ( <i>kecuali iklan lokal yang tertaut ke tempat/lokasi tertentu</i> )
`,
		itIT: `
Puoi sempre usare il nostro bot gratuitamente.

Ma le pubblicità in un bot coprono a malapena anche le spese di hosting. Scegliendo un piano a pagamento puoi:

- Supportare miglioramenti e nuove funzionalità
- Nascondere le pubblicità ( <i>ad eccezione delle pubblicità locali collegate a un luogo/sede specifico</i> )
`,
		jaJP: `
ボットはいつでも無料でご利用いただけます。

ただし、ボット内の広告ではホスティング費用さえ賄えません。有料プランをお選びいただくと、以下のメリットがあります。

- 改善点や新機能のサポート
- 広告を非表示（<i>特定のスポット/会場にリンクされたローカル広告を除く</i>）
`,
		koKR: `
저희 봇은 언제든지 무료로 사용하실 수 있습니다.

 하지만 봇에 광고가 표시되면 호스팅 비용조차 감당하기 어렵습니다. 유료 플랜을 선택하시면 다음과 같은 혜택을 누리실 수 있습니다.

- 개선 사항 및 새로운 기능 지원
- 광고 숨기기( <i>특정 장소/장소에 연결된 지역 광고 제외</i> )
`,
		plPL: `
Zawsze możesz korzystać z naszego bota za darmo.

Ale reklamy w bocie ledwo pokrywają nawet koszty hostingu. Wybierając płatny plan:

- Wspierasz ulepszenia i nowe funkcje
- Ukrywasz reklamy ( <i>z wyjątkiem reklam lokalnych powiązanych z konkretnym miejscem/obiektem</i> )
`,
		ptBR: `
Você sempre pode usar nosso bot gratuitamente.

Mas anúncios em um bot dificilmente cobrem as despesas de hospedagem. Ao escolher um plano pago, você:

- Oferece suporte a melhorias e novos recursos;
- Oculta anúncios ( <i>exceto os anúncios locais vinculados a um local/local específico</i> );
`,
		ptPT: `
Pode sempre utilizar o nosso bot gratuitamente.

Mas os anúncios num bot dificilmente cobrem as despesas de alojamento. Ao escolher um plano pago, você:

- Suporta melhorias e novas funcionalidades;
- Oculta anúncios ( <i>exceto os anúncios locais ligados a um local/local específico</i> );
`,
		ruRU: `
...
- Скроете рекламу (<i>кроме локальной - привязанной к определённому месту проведения событий</i>)`,
		trTR: `
Botumuzu her zaman ücretsiz kullanabilirsiniz.

Ancak bir bottaki reklamlar barındırma masraflarını bile karşılamaz. Ücretli bir plan seçerek:

- İyileştirmeleri ve yeni özellikleri desteklersiniz.
- Reklamları gizlersiniz ( <i>belirli bir yere/mekana bağlı yerel reklamlar hariç</i> ).
`,
		ukUA: `
Ви завжди можете користуватися нашим ботом безкоштовно.

Але реклама в боті ледве покриває навіть витрати на хостинг. Обираючи платний план, ви:

- Підтримуєте покращення та нові функції
- Приховуєте рекламу ( <i>окрім локальної реклами, пов’язаної з певним місцем/місцем проведення</i> )
`,
		uzUZ: `
Siz har doim bizning botimizdan bepul foydalanishingiz mumkin .

Ammo botdagi reklamalar hatto hosting xarajatlarini ham qoplay olmaydi. Pulli tarif rejasini tanlash orqali siz:

- Yaxshilanishlar va yangi funksiyalarni qo‘llab-quvvatlaysiz
- Reklamalarni yashirasiz ( <i>maʼlum bir joyga/joyga bogʻlangan mahalliy reklamalardan tashqari</i> )
`,
		zhCN: `
您始终可以免费使用我们的机器人。

但是，机器人中的广告甚至难以覆盖托管费用。选择付费套餐，您可以：

- 支持改进和新功能
- 隐藏广告（<i>链接到特定地点/场地的本地广告除外</i>）
`},
	InvoiceMonthlyPlanTitle: {
		arEG: "الدفع الشهري لـ @%s",
		deDE: "Monatliche Zahlung für @%s",
		enUK: "Monthly payment for @%s",
		esES: "Pago mensual de @%s",
		faIR: "پرداخت ماهانه برای @%s",
		frFR: "Paiement mensuel pour @%s",
		idID: "Pembayaran bulanan untuk @%s",
		itIT: "Pagamento mensile per @%s",
		jaJP: "@%s の月額支払い",
		koKR: "@%s에 대한 월별 지불",
		plPL: "Miesięczna płatność za @%s",
		ptBR: "Pagamento mensal para @%s",
		ptPT: "Pagamento mensal para @%s",
		ruRU: "Ежемесячный платёж для @%s",
		trTR: "@%s için aylık ödeme",
		ukUA: "Щомісячний платіж за @%s",
		uzUZ: "@%s uchun oylik toʻlov",
		zhCN: "@%s 的每月付款"},
	InvoiceYearlyPlanTitle: {
		arEG: "الدفع السنوي لـ @%s",
		deDE: "Jährliche Zahlung für @%s",
		enUK: "Yearly payment for @%s",
		esES: "Pago anual por @%s",
		faIR: "پرداخت سالانه برای @%s",
		frFR: "Paiement annuel pour @%s",
		idID: "Pembayaran tahunan untuk @%s",
		itIT: "Pagamento annuale per @%s",
		jaJP: "@%s の年間支払い",
		koKR: "@%s에 대한 연간 지불",
		plPL: "Roczna płatność za @%s",
		ptBR: "Pagamento anual para @%s",
		ptPT: "Pagamento anual para @%s",
		ruRU: "Ежегодный платёж для @%s",
		trTR: "@%s için yıllık ödeme",
		ukUA: "Річний платіж за @%s",
		uzUZ: "@%s uchun yillik toʻlov",
		zhCN: "@%s 的年度付款"},
	StarsPerMonth: {
		arEG: "%d⭐ شهريًا",
		deDE: "%d⭐ pro Monat",
	...
	DonationToBotInvoiceTitle: {
		...
		jaJP: "@%sへの寄付",
		koKR: "@%s 에게 기부",
		plPL: "Darowizna na rzecz @%s",
		ptBR: "Doação para @%s",
		ptPT: "Donativo para @%s",
	...
	DonationInvoiceLabelCoffee: {
		arEG: "☕️ اشتري لنا قهوة ~ €%d",
		deDE: "☕️ Spendiere uns einen Kaffee ~ €%d",
		enUK: "☕️ Buy us a coffee ~ €%d",
		esES: "☕️ Invítanos a un café ~ €%d",
		faIR: "☕️ برای ما یک قهوه بخرید ~ €%d",
		frFR: "☕️ Offrez-nous un café ~ €%d",
		idID: "☕️ Belikan kami kopi ~ €%d",
		itIT: "☕️ Offrici un caffè ~ €%d",
		jaJP: "☕️ コーヒーをおごってください ~ €%d",
		koKR: "☕️ 우리에게 커피 한 잔 사주세요 ~ €%d",
		plPL: "☕️ Postaw nam kawę ~ €%d",
		ptBR: "☕️ Compre um café para nós ~ €%d",
		ptPT: "☕️ Compre-nos um café ~ €%d",
		ruRU: "☕️ Купите нам кофе ~ €%d",
		trTR: "☕️ Bize bir kahve ısmarlayın ~ €%d",
		ukUA: "☕️ Купіть нам каву ~ €%d",
		uzUZ: "☕️ Bizga kofe sotib oling ~%d",
		zhCN: "☕️ 给我们买杯咖啡〜€%d"},
	DonationInvoiceLabelBeer: {
		arEG: "🍺 اشتري لنا بيرة ~ €%d",
		deDE: "🍺 Gib uns ein Bier aus ~ €%d",
		enUK: "🍺 Buy us a beer ~ €%d",
		esES: "🍺 Invítanos a una cerveza ~ €%d",
		faIR: "🍺 برای ما یک آبجو بخرید ~ €%d",
		frFR: "🍺 Offrez-nous une bière ~ €%d",
		idID: "Belikan kami bir ~ €%d",
		itIT: "🍺 Offrici una birra ~ €%d",
		jaJP: "🍺 ビールを買ってください ~ €%d",
		koKR: "🍺 맥주 한 잔 사주세요 ~ €%d",
		plPL: "🍺 Postaw nam piwo ~ €%d",
		ptBR: "🍺 Compre uma cerveja para nós ~ €%d",
		ptPT: "🍺 Compre-nos uma cerveja ~ €%d",
		ruRU: "🍺 Купите нам пива ~ €%d",
		trTR: "🍺 Bize bir bira ısmarlayın ~ €%d",
		ukUA: "🍺 Купи нам пиво ~ €%d",
		uzUZ: "🍺 Bizga pivo sotib oling ~%d",
		zhCN: "🍺 请我们喝杯啤酒 ~ €%d"},
	DonationInvoiceLabelLunch: {
		arEG: "🍽️ اشتري لنا وجبة غداء ~ €%d",
		deDE: "🍽️ Gib uns ein Mittagessen aus ~ €%d",
		enUK: "🍽️ Buy us a lunch ~ €%d",
		esES: "🍽️ Invítanos a un almuerzo ~ €%d",
		faIR: "🍽️ برای ما ناهار بخرید ~ €%d",
		frFR: "🍽️ Offrez-nous un déjeuner ~ €%d",
		idID: "🍽️ Belikan kami makan siang ~ €%d",
		itIT: "🍽️ Offrici un pranzo ~ €%d",
		jaJP: "🍽️ 私たちにランチをご馳走してください〜€%d",
		koKR: "🍽️ 점심 사주세요 ~ €%d",
		plPL: "🍽️ Postaw nam lunch ~ €%d",
		ptBR: "🍽️ Compre um almoço para nós ~ €%d",
		ptPT: "🍽️ Compre-nos um almoço ~ €%d",
		ruRU: "🍽️ Купите нам обед ~ €%d",
		trTR: "🍽️ Bize öğle yemeği ısmarlayın ~ €%d",
		ukUA: "🍽️ Купіть нам обід ~ €%d",
		uzUZ: "🍽️ Bizga tushlik sotib oling ~%d",
		zhCN: "🍽️ 请我们吃午餐〜€%d"},
	DonationInvoiceLabelAwesomeness: {
		arEG: "🙌 هل هذا البوت رائع لهذه الدرجة؟ ~ €%d",
		deDE: "🙌 Ist dieser Bot so toll? ~ €%d",
		enUK: "🙌 Is this bot that awesome? ~ €%d",
		esES: "🙌 ¿Es tan increíble este bot? ~ €%d",
		faIR: "🙌 این ربات انقدر عالیه؟ ~ €%d",
		frFR: "🙌 Ce bot est-il si génial ? ~ €%d",
		idID: "🙌 Apakah bot ini sekeren itu? ~ €%d",
		itIT: "🙌 Questo bot è così fantastico? ~ €%d",
		jaJP: "🙌 このボットってそんなにすごいの？～€%d",
		koKR: "🙌 이 봇이 그렇게 멋진가요? ~ €%d",
		plPL: "🙌 Czy ten bot jest aż tak świetny? ~ €%d",
		ptBR: "🙌 Esse bot é tão incrível assim? ~ €%d",
		ptPT: "🙌 Este bot é assim tão incrível? ~ €%d",
		ruRU: "🙌 Является ли этот бот настолько потрясаюшим? ~ €%d",
		trTR: "🙌 Bu bot o kadar harika mı? ~ €%d",
		ukUA: "🙌 Цей бот такий класний? ~ €%d",
		uzUZ: "🙌 Bu bot shunchalik ajoyibmi? ~ €%d",
		zhCN: "🙌 这个机器人真有那么棒吗？~€%d"},
	DonationButtonCoffee: {
		arEG: "☕️ اشتري لنا قهوة %d⭐ ~ €%d",
		deDE: "☕️ Gib uns einen Kaffee aus %d⭐ ~ €%d",
		enUK: "☕️ Buy us a coffee %d⭐ ~ €%d",
		esES: "☕️ Invítanos a un café %d⭐ ~ €%d",
		faIR: "☕️ برای ما یک قهوه بخرید %d⭐ ~ €%d",
		frFR: "☕️ Offrez-nous un café %d⭐ ~ €%d",
		idID: "☕️ Belikan kami kopi %d⭐ ~ €%d",
		itIT: "☕️ Offrici un caffè %d⭐ ~ €%d",
		jaJP: "☕️ コーヒーを買ってください %d⭐ ~ €%d",
		koKR: "☕️ 커피 한 잔 사주세요 %d⭐ ~ €%d",
		plPL: "☕️ Postaw nam kawę %d⭐ ~ €%d",
		ptBR: "☕️ Compre um café para nós %d⭐ ~ €%d",
		ptPT: "☕️ Compre-nos um café %d⭐ ~ €%d",
		ruRU: "☕️ Купите нам кофе %d⭐ ~ €%d",
		trTR: "☕️ Bize bir kahve ısmarla %d⭐ ~ €%d",
		ukUA: "☕️ Купіть нам каву %d⭐ ~ €%d",
		uzUZ: "☕️ Bizga %d⭐ ~ €%d kofe sotib oling",
		zhCN: "☕️ 给我们买杯咖啡 %d⭐ ~ €%d"},
	DonationButtonBeer: {
		arEG: "🍺 اشتري لنا بيرة %d⭐ ~ €%d",
		deDE: "🍺 Gib uns ein Bier aus %d⭐ ~ €%d",
		enUK: "🍺 Buy us a beer %d⭐ ~ €%d",
		esES: "🍺 Cómpranos una cerveza %d⭐ ~ €%d",
		faIR: "🍺 برای ما یک آبجو بخرید %d⭐ ~ €%d",
		frFR: "🍺 Offrez-nous une bière %d⭐ ~ €%d",
		idID: "Belikan kami bir %d⭐ ~ €%d",
		itIT: "🍺 Offrici una birra %d⭐ ~ €%d",
		jaJP: "🍺 ビールを %d⭐ ~ €%d でお買い上げください",
		koKR: "🍺 맥주 한 잔 사주세요 %d⭐ ~ €%d",
		plPL: "🍺 Postaw nam piwo %d⭐ ~ €%d",
		ptBR: "🍺 Compre uma cerveja para nós %d⭐ ~ €%d",
		ptPT: "🍺 Compre-nos uma cerveja %d⭐ ~ €%d",
		ruRU: "🍺 Купите нам пива %d⭐ ~ €%d",
		trTR: "🍺 Bize bir bira ısmarla %d⭐ ~ €%d",
		ukUA: "🍺 Купи нам пива %d⭐ ~ €%d",
		uzUZ: "🍺 Bizga %d⭐ ~ €%d pivo sotib oling",
		zhCN: "🍺 请我们喝杯啤酒 %d⭐ ~ €%d"},
	DonationButtonLunch: {
		arEG: "🍽️ اشتري لنا وجبة غداء %d⭐ ~ €%d",
		deDE: "🍽️ Gib uns ein Mittagessen aus %d⭐ ~ €%d",
		enUK: "🍽️ Buy us a lunch %d⭐ ~ €%d",
		esES: "🍽️ Invítanos a un almuerzo %d⭐ ~ €%d",
		faIR: "🍽️ برای ما ناهار بخرید %d⭐ ~ €%d",
		frFR: "🍽️ Offrez-nous un déjeuner %d⭐ ~ €%d",
		idID: "🍽️ Belikan kami makan siang %d⭐ ~ €%d",
		itIT: "🍽️ Offrici un pranzo %d⭐ ~ €%d",
		jaJP: "🍽️ 私たちにランチをおごってください %d⭐ ~ €%d",
		koKR: "🍽️ 점심 사주세요 %d⭐ ~ €%d",
		plPL: "🍽️ Postaw nam lunch %d⭐ ~ €%d",
		ptBR: "🍽️ Compre um almoço para nós %d⭐ ~ €%d",
		ptPT: "🍽️ Compre-nos um almoço %d⭐ ~ €%d",
		ruRU: "🍽️ Купите нам обед %d⭐ ~ €%d",
		trTR: "🍽️ Bize öğle yemeği ısmarla %d⭐ ~ €%d",
		ukUA: "🍽️ Купіть нам обід %d⭐ ~ €%d",
		uzUZ: "🍽️ Bizga %d⭐ ~ €%d tushlik sotib oling",
		zhCN: "🍽️ 请我们吃午餐 %d⭐ ~ €%d"},
	DonationButtonAwesomeness: {
		arEG: "🚀🔥🙌 أنت رائع! - %s🌟 ~ €%d",
		deDE: "🚀🔥🙌 Du bist großartig! - %s🌟 ~ €%d",
		enUK: "🚀🔥🙌 You are awesome! - %s🌟 ~ €%d",
		esES: "🚀🔥🙌 ¡Eres increíble! - %s🌟 ~ €%d",
		faIR: "🚀🔥🙌 تو فوق‌العاده‌ای! - %s🌟 ~ €%d",
		frFR: "🚀🔥🙌 Tu es génial ! - %s🌟 ~ €%d",
		idID: "🚀🔥🙌 Kamu luar biasa! - %s🌟 ~ €%d",
		itIT: "🚀🔥🙌 Sei fantastico! - %s🌟 ~ €%d",
		jaJP: "🚀🔥🙌 あなたは素晴らしいです！ - %s🌟 ~ €%d",
		koKR: "🚀🔥🙌 정말 멋지세요! - %s🌟 ~ €%d",
		plPL: "🚀🔥🙌 Jesteś niesamowity! - %s🌟 ~ €%d",
		ptBR: "🚀🔥🙌 Você é incrível! - %s🌟 ~ €%d",
		ptPT: "🚀🔥🙌 És incrível! - %s🌟 ~ €%d",
		ruRU: "🚀🔥🙌 Вы супер! - %s🌟 ~ €%d",
		trTR: "🚀🔥🙌 Harikasın! - %s🌟 ~ €%d",
		ukUA: "🚀🔥🙌 Ти неймовірна! - %s🌟 ~ €%d",
		uzUZ: "🚀🔥🙌 Siz ajoyibsiz! - %s🌟 ~ €%d",
		zhCN: "🚀🔥🙌 你太棒了！- %s🌟 ~ €%d"},
	SupportDevelopmentOfBotTitle: {
		arEG: "دعم تطوير @%s",
		deDE: "Unterstützen Sie die Entwicklung von @%s",
		enUK: "Support development of @%s",
		esES: "Apoyar el desarrollo de @%s",
		faIR: "از توسعه @%s حمایت کنید",
		frFR: "Soutenir le développement de @%s",
		idID: "Dukung pengembangan @%s",
		itIT: "Supporta lo sviluppo di @%s",
		jaJP: "@%s の開発をサポートする",
		koKR: "@%s 개발 지원",
		plPL: "Wsparcie rozwoju @%s",
		ptBR: "Suporte ao desenvolvimento de @%s",
		ptPT: "Apoio ao desenvolvimento de @%s",
		ruRU: "Поддержите развитие @%s",
		trTR: "@%s&#39;nin gelişimini destekleyin",
		ukUA: "Підтримка розвитку @%s",
		uzUZ: "@%s rivojlanishini qo‘llab-quvvatlash",
		zhCN: "支持@%s的开发"},
	SupportDevelopmentOfBotText: {
		arEG: `
من خلال إعطائنا تبرعًا لمرة واحدة، فإنك تساعد في تسريع تطوير الروبوت.

سيكون من الرائع أيضًا إذا تمكنت من <a href="https://t.me/{BOT_CODE}?start=get_pro_account">الاشتراك في عضوية PRO الخاصة بنا</a> .

إذا لم يكن التبرع النقدي خيارًا في الوقت الحالي ولديك حساب على GitHub، فسنكون ممتنين إذا منحت نجمة لمشاريعنا مفتوحة المصدر:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - طبقة تجريد الوصول إلى البيانات 
`,
		deDE: `
Indem Sie uns eine einmalige Spende geben, helfen Sie, die Entwicklung des Bots zu beschleunigen.

Außerdem wäre es cool, wenn Sie <a href="https://t.me/{BOT_CODE}?start=get_pro_account">unsere PRO-Mitgliedschaft abonnieren</a> könnten.

Wenn eine Geldspende derzeit keine Option ist und Sie ein Konto bei GitHub haben, würden wir uns freuen, wenn Sie unseren Open-Source-Projekten einen Stern geben:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
...
`,
		esES: `
Al darnos una donación única, estás ayudando a acelerar el desarrollo del bot.

También sería genial si puedes <a href="https://t.me/{BOT_CODE}?start=get_pro_account">suscribirte a nuestra membresía PRO</a> .

Si la donación monetaria no es una opción en este momento y tienes una cuenta en GitHub, te agradeceríamos que otorgaras una estrella a nuestros proyectos de código abierto:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Capa de abstracción de acceso a datos 
`,
		faIR: `
با یک بار کمک مالی، شما به تسریع توسعه ربات کمک می‌کنید.

همچنین اگر بتوانید <a href="https://t.me/{BOT_CODE}?start=get_pro_account">در عضویت حرفه‌ای ما مشترک شوید،</a> عالی خواهد بود.

اگر در حال حاضر امکان کمک مالی وجود ندارد و در گیت‌هاب حساب کاربری دارید، اگر به پروژه‌های متن‌باز ما ستاره بدهید، سپاسگزار خواهیم بود:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - لایه انتزاعی دسترسی به داده 
`,
		frFR: `
En nous faisant un don unique, vous contribuez à accélérer le développement du bot.

Ce serait également intéressant si vous pouviez <a href="https://t.me/{BOT_CODE}?start=get_pro_account">vous abonner à notre abonnement PRO</a> .

Si le don monétaire n&#39;est pas une option pour le moment et que vous avez un compte sur GitHub, nous apprécierions que vous accordiez une étoile à nos projets open source :
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Data-access Abstraction Layer 
`,
		idID: `
Dengan memberikan donasi satu kali, Anda membantu mempercepat pengembangan bot.

Akan sangat keren jika Anda dapat <a href="https://t.me/{BOT_CODE}?start=get_pro_account">berlangganan keanggotaan PRO kami</a> .

Jika donasi uang tidak memungkinkan saat ini dan Anda memiliki akun di GitHub, kami akan sangat berterima kasih jika Anda memberikan bintang untuk proyek sumber terbuka kami:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Data-access Abstraction Layer 
`,
		itIT: `
Con una donazione una tantum ci aiuterai ad accelerare lo sviluppo del bot.

Sarebbe anche bello se potessi <a href="https://t.me/{BOT_CODE}?start=get_pro_account">abbonarti alla nostra membership PRO</a> .

Se al momento la donazione in denaro non è un&#39;opzione e hai un account su GitHub, apprezzeremmo se dessi una stella ai nostri progetti open source:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Data-access Abstraction Layer 
`,
		jaJP: `
1 回限りの寄付をすることで、ボットの開発を加速させることができます。

また、 <a href="https://t.me/{BOT_CODE}?start=get_pro_account">PRO メンバーシップにご登録</a>いただけると幸いです。

現時点で金銭的な寄付ができず、GitHub にアカウントをお持ちの場合は、次のオープン ソース プロジェクトにスターを付けていただけると幸いです。
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - データ アクセス抽象化レイヤー 
`,
		koKR: `
한 번만 기부해 주시면 봇 개발 속도를 높이는 데 도움이 됩니다.

 또한 <a href="https://t.me/{BOT_CODE}?start=get_pro_account">PRO 멤버십에 가입해</a> 주시면 감사하겠습니다.

 현재 금전적 기부가 어려우시고 GitHub 계정이 있으시다면, 저희 오픈 소스 프로젝트에 별점을 주시면 감사하겠습니다.
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - 데이터 액세스 추상화 계층 
`,
		plPL: `
Przekazując nam jednorazową darowiznę, pomagasz przyspieszyć rozwój bota.

Byłoby również miło, gdybyś mógł <a href="https://t.me/{BOT_CODE}?start=get_pro_account">zapisać się do naszego członkostwa PRO</a> .

Jeśli darowizna pieniężna nie jest w tej chwili opcją, a masz konto w serwisie GitHub, docenilibyśmy, gdybyś przyznał gwiazdkę naszym projektom open source:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Data-access Abstraction Layer 
`,
		ptBR: `
Ao nos fazer uma doação única, você está ajudando a acelerar o desenvolvimento do bot.

Também seria legal se você pudesse <a href="https://t.me/{BOT_CODE}?start=get_pro_account">assinar nossa assinatura PRO</a> .

Se a doação monetária não for uma opção no momento e você tiver uma conta no GitHub, agradeceríamos se você concedesse uma estrela aos nossos projetos de código aberto:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Camada de abstração de acesso a dados 
`,
		ptPT: `
Ao fazer-nos um donativo único, está a ajudar a acelerar o desenvolvimento do bot.

Também seria bom se pudesse <a href="https://t.me/{BOT_CODE}?start=get_pro_account">subscrever a nossa subscrição PRO</a> .

Se o donativo monetário não for uma opção neste momento e tiver uma conta no GitHub, agradecemos que conceda uma estrela aos nossos projetos de código aberto:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Camada de abstração de acesso a dados 
`,
		ruRU: `
...
`,
		trTR: `
Bize tek seferlik bağış yaparak botun gelişimini hızlandırmaya yardımcı oluyorsunuz.

Ayrıca <a href="https://t.me/{BOT_CODE}?start=get_pro_account">PRO üyeliğimize abone</a> olursanız harika olur.

Şu anda maddi bağışta bulunmak mümkün değilse ve GitHub&#39;da bir hesabınız varsa, açık kaynaklı projelerimize bir yıldız vermenizi rica ederiz:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Veri Erişim Soyutlama Katmanı 
`,
		ukUA: `
Зробивши нам одноразову пожертву, ви допомагаєте пришвидшити розробку бота.

Також було б чудово, якби ви могли <a href="https://t.me/{BOT_CODE}?start=get_pro_account">підписатися на нашу PRO-членство</a> .

Якщо грошова пожертва наразі не є опцією, і у вас є обліковий запис на GitHub, ми будемо вдячні, якщо ви поставите зірочку нашим проектам з відкритим кодом:
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - Шар абстракції доступу до даних 
`,
		uzUZ: `
Bizga 1 martalik xayriya berish orqali siz botning rivojlanishini tezlashtirishga yordam berasiz.

Shuningdek, <a href="https://t.me/{BOT_CODE}?start=get_pro_account">PRO aʼzoligimizga obuna</a> boʻlsangiz ham yaxshi boʻlardi.

Agar hozirda pul xayriya qilish imkoni boʻlmasa va GitHub’da akkauntingiz boʻlsa, loyihalarimizga yulduzcha taqdim etsangiz, biz juda xursand boʻlamiz:{- <a href="https://github.com/bots-go-framework/bots-fw">frame-</a> <a href="https://github.com/dal-go/dalgo">DALgo</a> - ma&#39;lumotlarga kirish abstraktsiya qatlami 
`,
		zhCN: `
通过一次性捐赠，您将帮助加速机器人的开发。

此外，如果您可以<a href="https://t.me/{BOT_CODE}?start=get_pro_account">订阅我们的 PRO 会员资格，</a>那就太好了。

如果目前无法进行金钱捐赠，并且您在 GitHub 上拥有帐户，我们将非常感激您为我们的开源项目授予一颗星：
- <a href="https://github.com/bots-go-framework/bots-fw">bots-go-framework</a> 
- <a href="https://github.com/dal-go/dalgo">DALgo</a> - 数据访问抽象层 
`},
	AdvertiseAtBotTitle: {
		arEG: "الإعلان في @%s",
		deDE: "Werbung bei @%s",
		enUK: "Advertising at @%s",
		esES: "Publicidad en @%s",
		faIR: "تبلیغات در @%s",
		frFR: "Publicité sur @%s",
		idID: "Beriklan di @%s",
		itIT: "Pubblicità su @%s",
		jaJP: "@%s での広告",
		koKR: "@%s에 광고",
		plPL: "Reklama w @%s",
		ptBR: "Publicidade em @%s",
		ptPT: "Publicidade em @%s",
		ruRU: "Реклама на @%s",
		trTR: "@%s&#39;de reklamcılık",
		ukUA: "Реклама на @%s",
		uzUZ: "@%s da reklama",
		zhCN: "在@%s 上做广告"},
	AdvertiseAtBotText: {
		arEG: `
يمكنك وضع إعلان للنشاط من هنا ( <i>مع استهداف اختياري حسب المدينة/البلد</i> ).

إذا كنت تريد إنشاء إعلان محلي لمكان ما، فافعل ذلك من قائمة المكان.`,
		deDE: `
Von hier aus können Sie eine Anzeige für eine Aktivität platzieren ( <i>mit optionaler Ausrichtung nach Stadt/Land</i> ).

Wenn Sie eine lokale Anzeige für einen Spot erstellen möchten, tun Sie dies über das Spot-Menü.`,
		enUK: `
...
If you want to create a local add for a spot do it from the spot menu.`,
		esES: `
Puedes colocar un anuncio de actividad desde aquí ( <i>con segmentación opcional por ciudad/país</i> ).

Si deseas crear un anuncio local para un lugar, hazlo desde el menú del lugar.`,
		faIR: `
شما می‌توانید از اینجا برای فعالیت خود تبلیغ قرار دهید ( <i>با امکان هدف‌گذاری بر اساس شهر/کشور</i> ). 

اگر می‌خواهید برای یک مکان، آگهی محلی ایجاد کنید، این کار را از منوی مکان انجام دهید.`,
		frFR: `
Vous pouvez placer une annonce pour une activité à partir d&#39;ici ( <i>avec un ciblage facultatif par ville/pays</i> ).

Si vous souhaitez créer une annonce locale pour un spot, faites-le à partir du menu spot.`,
		idID: `
Anda dapat memasang iklan untuk aktivitas dari sini ( <i>dengan penargetan opsional berdasarkan kota/negara</i> ).

Jika Anda ingin membuat iklan lokal untuk suatu tempat, lakukanlah dari menu tempat.`,
		itIT: `
Da qui puoi pubblicare un annuncio per un&#39;attività ( <i>con targeting facoltativo per città/paese</i> ).

Se vuoi creare un annuncio locale per un luogo, fallo dal menu del luogo.`,
		jaJP: `
ここからアクティビティの広告を掲載できます（<i>オプションで都市/国によるターゲット設定が可能</i>）。

スポットのローカル広告を作成する場合は、スポット メニューから実行してください。`,
		koKR: `
여기에서 활동에 대한 광고를 게재할 수 있습니다( <i>선택적으로 도시/국가를 타겟팅할 수 있음</i> ).

 특정 장소에 대한 지역 광고를 만들려면 장소 메뉴에서 만드세요.`,
		plPL: `
Możesz tutaj zamieścić reklamę aktywności ( <i>z opcjonalnym kierowaniem na miasto/kraj</i> ).

Jeśli chcesz utworzyć lokalną reklamę miejsca, zrób to z menu miejsca.`,
		ptBR: `
Você pode colocar um anúncio para uma atividade aqui ( <i>com segmentação opcional por cidade/país</i> ).

Se quiser criar um anúncio local para um local, faça isso no menu do local.`,
		ptPT: `
Pode colocar aqui um anúncio para uma actividade ( <i>com segmentação opcional por cidade/país</i> ).

Se pretender criar um anúncio local para um local, faça-o no menu do local.`,
		ruRU: `
...
Если вы хотите создать локальное объявление для спота, сделайте это через меню спота.`,
		trTR: `
Buradan etkinlik için bir reklam verebilirsiniz ( <i>isteğe bağlı olarak şehir/ülke hedeflemesi yapabilirsiniz</i> ).

Bir yer için yerel bir reklam oluşturmak istiyorsanız bunu yer menüsünden yapın.`,
		ukUA: `
Ви можете розмістити оголошення про активність звідси ( <i>з додатковим таргетингом за містом/країною</i> ).

Якщо ви хочете створити локальне оголошення для певного місця, зробіть це з меню місця.`,
		uzUZ: `
Siz bu yerdan faoliyat uchun reklama joylashtirishingiz mumkin ( <i>shahar/mamlakat boʻyicha ixtiyoriy maqsadli</i> ).

Agar siz joy uchun mahalliy qoʻshimcha yaratmoqchi boʻlsangiz, buni spot menyusidan qiling.`,
		zhCN: `
您可以从这里放置活动广告（<i>可选择按城市/国家/地区定位</i>）。

如果您想为某个地点创建本地添加，请从地点菜单中进行操作。`},
	MyAdsButtonText: {
		arEG: "🛍️ إعلاناتي",
		deDE: "🛍️ Meine Anzeigen",
		enUK: "🛍️ My ads",
		esES: "🛍️ Mis anuncios",
		faIR: "🛍️ تبلیغات من",
		frFR: "🛍️ Mes annonces",
		idID: "🛍️ Iklan saya",
		itIT: "🛍️ I miei annunci",
		jaJP: "🛍️ 私の広告",
		koKR: "🛍️ 내 광고",
		plPL: "🛍️ Moje reklamy",
		ptBR: "🛍️ Meus anúncios",
		ptPT: "🛍️ Os meus anúncios",
		ruRU: "🛍️ Мои объявления",
		trTR: "🛍️ Reklamlarım",
		ukUA: "🛍️ Мої оголошення",
		uzUZ: "🛍️ Mening e&#39;lonlarim",
		zhCN: "🛍️ 我的广告"},
	NewAdButtonText: {
		arEG: "➕ إعلان جديد",
		deDE: "➕ Neue Anzeige",
	...
	AdsBillingButtonText: {
		...
		ruRU: "🧾 Биллинг",
		trTR: "🧾 Faturalandırma",
		ukUA: "🧾 Виставлення рахунків",
		uzUZ: "🧾 Hisob-kitob",
		zhCN: "🧾 计费"},
	AdvertiseWithUsButtonText: {
		arEG: "🛍️ أعلن معنا",
		deDE: "🛍️ Werben Sie mit uns",
		enUK: "🛍️ Advertise with us",
		esES: "🛍️ Anúnciese con nosotros",
		faIR: "🛍️ با ما تبلیغ کنید",
		frFR: "🛍️ Faites de la publicité avec nous",
		idID: "🛍️ Beriklan bersama kami",
		itIT: "🛍️ Pubblicizza con noi",
		jaJP: "🛍️ 広告掲載のお願い",
		koKR: "🛍️ 저희와 함께 광고하세요",
		plPL: "🛍️ Reklamuj się u nas",
		ptBR: "🛍️ Anuncie conosco",
		ptPT: "🛍️ Anuncie connosco",
		ruRU: "🛍️ Реклама в боте",
		trTR: "🛍️ Bizimle reklam verin",
		ukUA: "🛍️ Рекламуйтеся у нас",
		uzUZ: "🛍️ Biz bilan reklama qiling",
		zhCN: "🛍️ 与我们一起做广告"},
	GetProAccountButtonText: {
		arEG: "🚀 احصل على حساب مميز",
		deDE: "🚀 Holen Sie sich ein PREMIUM-Konto",
		enUK: "🚀 Get PREMIUM account",
		esES: "🚀 Obtén una cuenta PREMIUM",
		faIR: "🚀 حساب کاربری پریمیوم دریافت کنید",
		frFR: "🚀 Obtenez un compte PREMIUM",
		idID: "🚀 Dapatkan akun PREMIUM",
		itIT: "🚀 Ottieni un account PREMIUM",
		jaJP: "🚀 プレミアムアカウントを取得する",
		koKR: "🚀 프리미엄 계정 받기",
		plPL: "🚀 Zdobądź konto PREMIUM",
		ptBR: "🚀 Obtenha uma conta PREMIUM",
		ptPT: "🚀 Obtenha uma conta PREMIUM",
		ruRU: "🚀 Премиальный аккаунт",
		trTR: "🚀 PREMIUM hesap edinin",
		ukUA: "🚀 Отримайте ПРЕМІУМ-акаунт",
		uzUZ: "🚀 PREMIUM hisobini oling",
		zhCN: "🚀 获取高级帐户"},
	SupportUsButtonText: {
		arEG: "❤️ ادعمنا",
		deDE: "❤️ Unterstütze uns",
		enUK: "❤️ Support us",
		esES: "❤️ Apóyanos",
		faIR: "❤️ از ما حمایت کنید",
		frFR: "❤️ Soutenez-nous",
		idID: "❤️ Dukung kami",
		itIT: "❤️ Sostienici",
		jaJP: "❤️ 応援してください",
		koKR: "❤️ 저희를 응원해주세요",
		...
		ptPT: "❤️ Apoie-nos",
		ruRU: "❤️ Поддержать",
		trTR: "❤️ Bizi destekleyin",
		ukUA: "❤️ Підтримайте нас",
		uzUZ: "❤️ Bizni qo&#39;llab quvvatlang",
		zhCN: "❤️支持我们"},
	VewEventDetailsButtonText: {
		arEG: "انتقل إلى الحدث",
		deDE: "Zur Veranstaltung",
		enUK: "Go to event",
		esES: "Ir al evento",
		faIR: "رفتن به رویداد",
		frFR: "Aller à l&#39;événement",
		idID: "Pergi ke acara",
		itIT: "Vai all&#39;evento",
		jaJP: "イベントへ移動",
		koKR: "이벤트로 이동",
		plPL: "Przejdź do wydarzenia",
		ptBR: "Ir para o evento",
		ptPT: "Ir para o evento",
		ruRU: "Перейти на мероприятие",
		trTR: "Etkinliğe git",
		ukUA: "Перейти до події",
		uzUZ: "Tadbirga boring",
		zhCN: "前往活动"},
	RsvpTpEventButtonText: {
		...
		faIR: "پاسخ دهید",
		frFR: "RSVP",
		idID: "Konfirmasikan kehadiran Anda",
		itIT: "Rispondi",
		jaJP: "お返事お願いします",
		koKR: "RSVP",
		plPL: "RSVP",
		...
		trTR: "Lütfen cevap veriniz",
		ukUA: "Підтвердити участь",
		uzUZ: "taklifga javob bering",
		zhCN: "敬请回复"},
	PromoTitle: {
		arEG: "الترويج",
		deDE: "Angebot",
		...
		zhCN: "促销"},
	TogdAd1: {
		arEG: `يمكنك <a href="https://t.me/{BOT_CODE}?start=advertising">وضع إعلان</a> أو <a href="https://t.me/{BOT_CODE}?start=get_pro_account">الحصول على <b>حساب PRO</b></a> <a href="https://t.me/{BOT_CODE}?start=support_us">لدعمنا</a> .`,
		deDE: `Sie können <a href="https://t.me/{BOT_CODE}?start=advertising">eine Anzeige schalten</a> oder <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>ein PRO-Konto</b> erhalten,</a> um <a href="https://t.me/{BOT_CODE}?start=support_us">uns zu unterstützen</a> .`,
		...
		esES: `Puedes <a href="https://t.me/{BOT_CODE}?start=advertising">colocar un anuncio</a> u <a href="https://t.me/{BOT_CODE}?start=get_pro_account">obtener <b>una cuenta PRO</b></a> para <a href="https://t.me/{BOT_CODE}?start=support_us">apoyarnos</a> .`,
		faIR: `شما می‌توانید برای <a href="https://t.me/{BOT_CODE}?start=support_us">حمایت از ما</a> <a href="https://t.me/{BOT_CODE}?start=advertising">، تبلیغ ثبت کنید</a> یا <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>حساب کاربری حرفه‌ای (PRO)</b> دریافت کنید</a> .`,
		frFR: `Vous pouvez <a href="https://t.me/{BOT_CODE}?start=advertising">placer une annonce</a> ou <a href="https://t.me/{BOT_CODE}?start=get_pro_account">obtenir <b>un compte PRO</b></a> pour <a href="https://t.me/{BOT_CODE}?start=support_us">nous soutenir</a> .`,
		idID: `Anda dapat <a href="https://t.me/{BOT_CODE}?start=advertising">memasang iklan</a> atau <a href="https://t.me/{BOT_CODE}?start=get_pro_account">mendapatkan <b>akun PRO</b></a> untuk <a href="https://t.me/{BOT_CODE}?start=support_us">mendukung kami</a> .`,
		itIT: `Puoi <a href="https://t.me/{BOT_CODE}?start=advertising">pubblicare un annuncio</a> o <a href="https://t.me/{BOT_CODE}?start=get_pro_account">ottenere <b>un account PRO</b></a> per <a href="https://t.me/{BOT_CODE}?start=support_us">supportarci</a> .`,
		jaJP: `<a href="https://t.me/{BOT_CODE}?start=support_us">私たちをサポートする</a>ために、<a href="https://t.me/{BOT_CODE}?start=advertising">広告を掲載したり</a>、 <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO アカウント</b>を取得したりする</a>ことができます。`,
		koKR: `<a href="https://t.me/{BOT_CODE}?start=advertising">광고를 게재</a> 하거나 <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO 계정을</b> 만들어서</a> <a href="https://t.me/{BOT_CODE}?start=support_us">저희를 지원해주세요</a> .`,
		plPL: `Możesz <a href="https://t.me/{BOT_CODE}?start=advertising">zamieścić reklamę</a> lub <a href="https://t.me/{BOT_CODE}?start=get_pro_account">założyć <b>konto PRO</b></a> , aby <a href="https://t.me/{BOT_CODE}?start=support_us">nas wesprzeć</a> .`,
		ptBR: `Você pode <a href="https://t.me/{BOT_CODE}?start=advertising">colocar um anúncio</a> ou <a href="https://t.me/{BOT_CODE}?start=get_pro_account">obter <b>uma conta PRO</b></a> para <a href="https://t.me/{BOT_CODE}?start=support_us">nos apoiar</a> .`,
		ptPT: `Pode <a href="https://t.me/{BOT_CODE}?start=advertising">colocar um anúncio</a> ou <a href="https://t.me/{BOT_CODE}?start=get_pro_account">obter <b>uma conta PRO</b></a> para <a href="https://t.me/{BOT_CODE}?start=support_us">nos apoiar</a> .`,
		...
		trTR: `<a href="https://t.me/{BOT_CODE}?start=support_us">Bizi desteklemek</a> için <a href="https://t.me/{BOT_CODE}?start=advertising">reklam verebilir</a> veya <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO hesap</b> alabilirsiniz</a> .`,
		ukUA: `Ви можете <a href="https://t.me/{BOT_CODE}?start=advertising">розмістити рекламу</a> або <a href="https://t.me/{BOT_CODE}?start=get_pro_account">отримати <b>PRO-акаунт</b></a> , щоб <a href="https://t.me/{BOT_CODE}?start=support_us">підтримати нас</a> .`,
		uzUZ: `<a href="https://t.me/{BOT_CODE}?start=support_us">Bizni qo&#39;llab-quvvatlash</a> uchun siz <a href="https://t.me/{BOT_CODE}?start=advertising">reklama joylashtirishingiz</a> yoki <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO hisob qaydnomasini</b> olishingiz</a> mumkin.`,
		zhCN: `您可以<a href="https://t.me/{BOT_CODE}?start=advertising">投放广告</a>或<a href="https://t.me/{BOT_CODE}?start=get_pro_account">获取<b>PRO 帐户</b></a>来<a href="https://t.me/{BOT_CODE}?start=support_us">支持我们</a>。`},
	TogdAd2: {
		arEG: `احصل على <a href="https://t.me/{BOT_CODE}?start=get_pro_account">حساب <b>PRO</b></a> للتخلص من <a href="https://t.me/{BOT_CODE}?start=advertising">الإعلانات</a> <a href="https://t.me/{BOT_CODE}?start=support_us">ودعمنا</a> .`,
		deDE: `Holen Sie sich <a href="https://t.me/{BOT_CODE}?start=get_pro_account">ein <b>PRO</b> -Konto,</a> um <a href="https://t.me/{BOT_CODE}?start=advertising">Werbung</a> loszuwerden und <a href="https://t.me/{BOT_CODE}?start=support_us">uns zu unterstützen</a> .`,
		...
		esES: `Obtenga <a href="https://t.me/{BOT_CODE}?start=get_pro_account">una cuenta <b>PRO</b></a> para deshacerse de <a href="https://t.me/{BOT_CODE}?start=advertising">la publicidad</a> y <a href="https://t.me/{BOT_CODE}?start=support_us">apoyarnos</a> .`,
		faIR: `برای خلاص شدن از شر <a href="https://t.me/{BOT_CODE}?start=advertising">تبلیغات</a> و <a href="https://t.me/{BOT_CODE}?start=support_us">حمایت از ما</a> <a href="https://t.me/{BOT_CODE}?start=get_pro_account">، حساب کاربری <b>حرفه‌ای (PRO)</b></a> تهیه کنید.`,
		frFR: `Obtenez <a href="https://t.me/{BOT_CODE}?start=get_pro_account">un compte <b>PRO</b></a> pour vous débarrasser de <a href="https://t.me/{BOT_CODE}?start=advertising">la publicité</a> et <a href="https://t.me/{BOT_CODE}?start=support_us">nous soutenir</a> .`,
		idID: `Dapatkan <a href="https://t.me/{BOT_CODE}?start=get_pro_account">akun <b>PRO</b></a> untuk menyingkirkan <a href="https://t.me/{BOT_CODE}?start=advertising">iklan</a> &amp; untuk <a href="https://t.me/{BOT_CODE}?start=support_us">mendukung kami</a> .`,
		itIT: `Ottieni <a href="https://t.me/{BOT_CODE}?start=get_pro_account">un account <b>PRO</b></a> per liberarti dalla <a href="https://t.me/{BOT_CODE}?start=advertising">pubblicità</a> e per <a href="https://t.me/{BOT_CODE}?start=support_us">supportarci</a> .`,
		jaJP: `<a href="https://t.me/{BOT_CODE}?start=advertising">広告</a>を削除して<a href="https://t.me/{BOT_CODE}?start=support_us">私たちをサポート</a>するには、 <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO</b>アカウント</a>を取得してください。`,
		koKR: `<a href="https://t.me/{BOT_CODE}?start=advertising">광고를</a> 없애고 <a href="https://t.me/{BOT_CODE}?start=support_us">저희를 지원</a> 하려면 <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO</b> 계정을</a> 만드세요.`,
		plPL: `Załóż <a href="https://t.me/{BOT_CODE}?start=get_pro_account">konto <b>PRO</b></a> , aby pozbyć się <a href="https://t.me/{BOT_CODE}?start=advertising">reklam</a> i <a href="https://t.me/{BOT_CODE}?start=support_us">nas wesprzeć</a> .`,
		ptBR: `Crie <a href="https://t.me/{BOT_CODE}?start=get_pro_account">uma conta <b>PRO</b></a> para se livrar da <a href="https://t.me/{BOT_CODE}?start=advertising">publicidade</a> e <a href="https://t.me/{BOT_CODE}?start=support_us">nos apoiar</a> .`,
		ptPT: `Crie <a href="https://t.me/{BOT_CODE}?start=get_pro_account">uma conta <b>PRO</b></a> para se livrar da <a href="https://t.me/{BOT_CODE}?start=advertising">publicidade</a> e <a href="https://t.me/{BOT_CODE}?start=support_us">apoiar-nos</a> .`,
		...
		trTR: `<a href="https://t.me/{BOT_CODE}?start=advertising">Reklamlardan</a> kurtulmak ve <a href="https://t.me/{BOT_CODE}?start=support_us">bizi desteklemek</a> için <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO</b> hesap</a> edinin.`,
		ukUA: `Отримайте <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO</b> -акаунт</a> , щоб позбутися <a href="https://t.me/{BOT_CODE}?start=advertising">реклами</a> та <a href="https://t.me/{BOT_CODE}?start=support_us">підтримати нас</a> .`,
		uzUZ: `<a href="https://t.me/{BOT_CODE}?start=advertising">Reklamadan</a> xalos bo&#39;lish va <a href="https://t.me/{BOT_CODE}?start=support_us">bizni qo&#39;llab-quvvatlash</a> uchun <a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO</b> hisobini</a> oling.`,
		zhCN: `获取<a href="https://t.me/{BOT_CODE}?start=get_pro_account"><b>PRO</b>帐户</a>以摆脱<a href="https://t.me/{BOT_CODE}?start=advertising">广告</a>并<a href="https://t.me/{BOT_CODE}?start=support_us">支持我们</a>。`},
	BeforeStartChooseTimezoneTitle: {
		arEG: "قبل أن نبدأ، يرجى اختيار منطقتك الزمنية",
		deDE: "Bevor wir beginnen, wählen Sie bitte Ihre Zeitzone",
		enUK: "Before we start please choose your timezone",
		esES: "Antes de comenzar, seleccione su zona horaria.",
		faIR: "قبل از شروع، لطفا منطقه زمانی خود را انتخاب کنید",
		frFR: "Avant de commencer, veuillez choisir votre fuseau horaire",
		idID: "Sebelum kita mulai, silakan pilih zona waktu Anda",
		itIT: "Prima di iniziare, seleziona il tuo fuso orario",
		jaJP: "始める前にタイムゾーンを選択してください",
		koKR: "시작하기 전에 시간대를 선택해주세요",
		plPL: "Zanim zaczniemy, wybierz swoją strefę czasową",
		ptBR: "Antes de começar, escolha seu fuso horário",
		ptPT: "Antes de começar, escolha o seu fuso horário",
		ruRU: "Перед тем как начать выберите вашу временную зону",
		trTR: "Başlamadan önce lütfen saat diliminizi seçin",
		ukUA: "Перш ніж ми почнемо, будь ласка, виберіть свій часовий пояс",
		uzUZ: "Boshlashdan oldin vaqt mintaqangizni tanlang",
		zhCN: "在我们开始之前请选择您的时区"},
	BeforeStartChooseTimezoneText: {
		arEG: `
هذا ضروري لـ:
 - عرض الأوقات بشكل صحيح (نستخدم التوقيت العالمي المنسق داخليًا)
 - تجنب إرسال الإشعارات أثناء ساعات الليل
 - توفير وقتك عند إنشاء المواقع والأحداث ( <i>سيتم استخدام منطقتك الزمنية افتراضيًا</i> )

أسهل طريقة لتعيين منطقتك الزمنية هي إرسال موقعك.
ولكن يمكنك أيضًا الاختيار من القائمة المقدمة.`,
		deDE: `
...
Pero también puede elegir de la lista proporcionada.`,
		faIR: `
این مورد برای موارد زیر لازم است:
 - نمایش صحیح زمان (ما به صورت داخلی از UTC استفاده می‌کنیم)
 - جلوگیری از ارسال اعلان‌ها در ساعات شب
 - صرفه‌جویی در وقت هنگام ایجاد مکان‌ها و رویدادها ( <i>منطقه زمانی شما به طور پیش‌فرض استفاده خواهد شد</i> )

ساده‌ترین راه برای تنظیم منطقه زمانی، ارسال موقعیت مکانی شما است.
اما می‌توانید از لیست ارائه شده نیز انتخاب کنید.`,
		frFR: `
...
但您也可以从提供的列表中进行选择。`},
	HintToSendLocationForTimezoneDetection: {
		arEG: `أسهل وأسرع طريقة لضبط منطقتك الزمنية هي <b>إرسال موقعك</b> - يعمل فقط على الأجهزة المحمولة. ولكن يمكنك أيضًا الاختيار من القائمة المُتاحة.`,
		deDE: `Der einfachste und schnellste Weg, Ihre Zeitzone einzustellen, ist die <b>Übermittlung Ihres Standorts</b> . Dies funktioniert nur auf Mobilgeräten. Sie können aber auch aus der bereitgestellten Liste auswählen.`,
		...
		esES: `La forma más fácil y rápida de configurar tu zona horaria es <b>enviar tu ubicación</b> (solo funciona en dispositivos móviles). También puedes elegirla de la lista proporcionada.`,
		faIR: `ساده‌ترین و سریع‌ترین راه برای تنظیم منطقه زمانی، <b>ارسال موقعیت مکانی</b> است - فقط روی دستگاه‌های تلفن همراه کار می‌کند. اما می‌توانید از لیست ارائه شده نیز انتخاب کنید.`,
		frFR: `Le moyen le plus simple et le plus rapide de définir votre fuseau horaire est d&#39; <b>envoyer votre position</b> (fonctionne uniquement sur les appareils mobiles). Vous pouvez également choisir parmi la liste proposée.`,
		idID: `Cara termudah dan tercepat untuk mengatur zona waktu Anda adalah dengan <b>mengirimkan lokasi Anda</b> - hanya berfungsi di perangkat seluler. Namun, Anda juga dapat memilih dari daftar yang tersedia.`,
		itIT: `Il modo più semplice e veloce per impostare il fuso orario è <b>inviare la propria posizione</b> (funziona solo con dispositivi mobili). Ma puoi anche scegliere dall&#39;elenco fornito.`,
		jaJP: `タイムゾーンを設定する最も簡単で迅速な方法は<b>、位置情報を送信する</b>ことです。これはモバイルデバイスでのみ機能します。ただし、リストから選択することもできます。`,
		koKR: `시간대를 설정하는 가장 쉽고 빠른 방법은 <b>위치 정보를 보내는</b> 것입니다. 모바일 기기에서만 작동합니다. 제공된 목록에서 선택할 수도 있습니다.`,
		plPL: `Najprostszym i najszybszym sposobem ustawienia strefy czasowej jest <b>wysłanie swojej lokalizacji</b> – działa tylko na urządzeniach mobilnych. Możesz również wybrać strefę z listy.`,
		ptBR: `A maneira mais fácil e rápida de definir seu fuso horário é <b>enviar sua localização</b> — funciona apenas em dispositivos móveis. Mas você também pode escolher na lista fornecida.`,
		ptPT: `A forma mais fácil e rápida de definir o seu fuso horário é <b>enviar a sua localização</b> — só funciona em dispositivos móveis. Mas também pode escolher da lista fornecida.`,
		...
		trTR: `Saat diliminizi ayarlamanın en kolay ve hızlı yolu <b>konumunuzu göndermektir</b> - bu özellik yalnızca mobil cihazlarda çalışır. Ancak, sağlanan listeden de seçim yapabilirsiniz.`,
		ukUA: `Найпростіший і найшвидший спосіб встановити часовий пояс – це <b>надіслати своє місцезнаходження</b> . Це працює лише на мобільних пристроях. Але ви також можете вибрати зі списку.`,
		uzUZ: `Vaqt mintaqangizni o&#39;rnatishning eng oson va tezkor usuli bu <b>joylashuvingizni yuborish</b> - faqat mobil qurilmalarda ishlaydi. Lekin siz taqdim etilgan ro&#39;yxatdan ham tanlashingiz mumkin.`,
		zhCN: `设置时区最简单快捷的方法是<b>发送您的位置信息</b>- 仅适用于移动设备。您也可以从提供的列表中进行选择。`},
	YourTimezoneIsSetTo: {
		arEG: "تم تعيين منطقتك الزمنية إلى %s",
		deDE: "Ihre Zeitzone ist auf %s eingestellt",
		enUK: "Your time zone is set to %s",
		esES: "Su zona horaria está establecida en %s",
		faIR: "منطقه زمانی شما روی %s تنظیم شده است",
		frFR: "Votre fuseau horaire est défini sur %s",
		idID: "Zona waktu Anda diatur ke %s",
		itIT: "Il tuo fuso orario è impostato su %s",
		jaJP: "タイムゾーンは%sに設定されています",
		koKR: "귀하의 시간대는 %s로 설정되어 있습니다.",
		plPL: "Twoja strefa czasowa jest ustawiona na %s",
		ptBR: "Seu fuso horário está definido como %s",
		ptPT: "O seu fuso horário está definido como %s",
		ruRU: "Ваша временная зона установлена: %s",
		trTR: "Saat diliminiz %s olarak ayarlandı",
		ukUA: "Ваш часовий пояс встановлено на %s",
		uzUZ: "Vaqt mintaqangiz %s qilib o‘rnatildi",
		zhCN: "您的时区设置为 %s"},
	SendLocationForTimezoneDetection: {
		arEG: "اكتشاف المنطقة الزمنية حسب الموقع",
		deDE: "Zeitzone nach Standort ermitteln",
		enUK: "Detect timezone by location",
		esES: "Detectar zona horaria por ubicación",
		faIR: "تشخیص منطقه زمانی بر اساس موقعیت مکانی",
		frFR: "Détecter le fuseau horaire par emplacement",
		idID: "Deteksi zona waktu berdasarkan lokasi",
		itIT: "Rileva il fuso orario in base alla posizione",
		jaJP: "場所からタイムゾーンを検出する",
		koKR: "위치별로 시간대 감지",
		plPL: "Wykryj strefę czasową według lokalizacji",
		ptBR: "Detectar fuso horário por localização",
		ptPT: "Detetar fuso horário por localização",
		ruRU: "Определить временную зону",
		trTR: "Konuma göre zaman dilimini algıla",
		ukUA: "Визначення часового поясу за місцем розташування",
		uzUZ: "Vaqt mintaqasini joylashuvga qarab aniqlang",
		zhCN: "根据位置检测时区"},
	YouAreNotFollowingThisSpot: {
		arEG: "أنت لا تتابع هذا المكان",
		deDE: "Sie folgen diesem Spot nicht",
		enUK: "You are not following this spot",
		esES: "No estás siguiendo este spot",
		faIR: "شما این مکان را دنبال نمی‌کنید",
		frFR: "Vous ne suivez pas cet endroit",
		idID: "Anda tidak mengikuti tempat ini",
		itIT: "Non stai seguendo questo spot",
		jaJP: "このスポットをフォローしていません",
		koKR: "이 지점을 팔로우하지 않습니다",
		plPL: "Nie śledzisz tego miejsca",
		ptBR: "Você não está seguindo este local",
		ptPT: "Não está a seguir este local",
		ruRU: "Вы не подписаны на это место.",
		trTR: "Bu noktayı takip etmiyorsunuz",
		ukUA: "Ви не стежите за цим місцем",
		uzUZ: "Siz bu joyni kuzatmayapsiz",
		zhCN: "您尚未关注此地点"},
	YouAreFollowingThisSpot: {
		arEG: "أنت تتابع هذا المكان لجميع الأنشطة",
		deDE: "Sie folgen diesem Ort für alle Aktivitäten",
		enUK: "You are following this spot for all activities",
		esES: "Estás siguiendo este lugar para todas las actividades.",
		faIR: "شما این مکان را برای همه فعالیت‌ها دنبال می‌کنید",
		frFR: "Vous suivez ce spot pour toutes les activités",
		idID: "Anda mengikuti tempat ini untuk semua aktivitas",
		itIT: "Stai seguendo questo spot per tutte le attività",
		jaJP: "すべてのアクティビティについてこのスポットをフォローしています",
		koKR: "모든 활동을 위해 이 지점을 팔로우하고 있습니다.",
		plPL: "Śledzisz to miejsce, aby śledzić wszystkie aktywności",
		ptBR: "Você está seguindo este local para todas as atividades",
		ptPT: "Está a seguir este local para todas as atividades",
		ruRU: "Вы подписаны на оповещения с этого места.",
		trTR: "Tüm aktiviteler için burayı takip ediyorsunuz",
		ukUA: "Ви слідкуєте за всіма активностями цього місця",
		uzUZ: "Siz barcha harakatlar uchun shu joyni kuzatib borasiz",
		zhCN: "您正在关注此地点的所有活动"},
	YouHaveNotRecordedFuturePlansForThisSpot: {
		arEG: "ليس لديك أي خطط مستقبلية مسجلة لهذا المكان.",
		deDE: "Sie haben für diesen Ort/Veranstaltungsort keine Zukunftspläne erfasst.",
		enUK: "You have no recorded future plans for this spot/venue.",
		esES: "No tienes planes futuros registrados para este lugar/sitio.",
		faIR: "شما هیچ برنامه‌ی آتی ثبت‌شده‌ای برای این مکان/محل برگزاری ندارید.",
		frFR: "Vous n&#39;avez aucun projet futur enregistré pour ce lieu/spot.",
		idID: "Anda tidak memiliki rencana masa depan yang tercatat untuk tempat/lokasi ini.",
		itIT: "Non hai piani futuri registrati per questo posto/sede.",
		jaJP: "このスポット/会場の今後の予定は記録されていません。",
		koKR: "이 장소/장소에 대한 향후 계획이 없습니다.",
		plPL: "Nie masz żadnych zapisanych planów na przyszłość odnośnie tego miejsca/obiektu.",
		ptBR: "Você não tem planos futuros registrados para este local/local.",
		ptPT: "Não tem planos futuros registados para este local/local.",
		ruRU: "У вас пока нет записей о планах на будущее об этом месте.",
		trTR: "Bu yer/mekan için kaydedilmiş gelecek planlarınız yok.",
		ukUA: "У вас немає зафіксованих планів на майбутнє щодо цього місця/місця проведення.",
		uzUZ: "Sizda bu joy/joy uchun hech qanday kelajak rejalaringiz yoʻq.",
		zhCN: "您没有针对该地点/场地记录的未来计划。"},
	PleaseSpecifyTimezoneForThisSpot: {
		arEG: "يرجى تحديد المنطقة الزمنية للمكان.",
		deDE: "Bitte geben Sie die Zeitzone für den Spot an.",
		enUK: "Please specify time zone for the spot.",
		esES: "Por favor, especifique la zona horaria del lugar.",
		faIR: "لطفا منطقه زمانی محل مورد نظر را مشخص کنید.",
		frFR: "Veuillez préciser le fuseau horaire du spot.",
		idID: "Harap tentukan zona waktu untuk tempat tersebut.",
		itIT: "Si prega di specificare il fuso orario del posto.",
		jaJP: "スポットのタイムゾーンを指定してください。",
		koKR: "해당 장소의 시간대를 지정해 주세요.",
		plPL: "Proszę podać strefę czasową dla tego miejsca.",
		ptBR: "Por favor, especifique o fuso horário do local.",
		ptPT: "Por favor, especifique o fuso horário do local.",
		ruRU: "Пожалуйста выберите часовой пояс для этого места.",
		trTR: "Lütfen bulunduğunuz yerin saat dilimini belirtin.",
		ukUA: "Будь ласка, вкажіть часовий пояс для місця.",
		uzUZ: "Joy uchun vaqt mintaqasini belgilang.",
		zhCN: "请指定该地点的时区。"},
	YouAreSubscribedTo1Activity: {
		arEG: "لقد اشتركت في 1 نشاط.",
		deDE: "Sie haben 1 Aktivität abonniert.",
		enUK: "You are subscribed to 1 activity.",
		esES: "Estás suscrito a 1 actividad.",
		faIR: "شما در ۱ فعالیت مشترک هستید.",
		frFR: "Vous êtes abonné à 1 activité.",
		idID: "Anda berlangganan 1 aktivitas.",
		itIT: "Sei iscritto a 1 attività.",
		jaJP: "1 件のアクティビティに登録されています。",
		koKR: "1개의 활동을 구독하고 있습니다.",
		plPL: "Subskrybujesz 1 aktywność.",
		ptBR: "Você está inscrito em 1 atividade.",
		ptPT: "Está inscrito em 1 atividade.",
		ruRU: "Вы подписаны на 1 категорию событий.",
		trTR: "1 etkinliğe abone oldunuz.",
		ukUA: "Ви підписані на 1 активність.",
		uzUZ: "Siz 1 ta harakatga obuna boʻldingiz.",
		zhCN: "您已订阅 1 项活动。"},
	YouAreSubscribedToNActivity: {
		arEG: "لقد اشتركت في {N} نشاط: {ACTIVITIES}",
		deDE: "Sie haben {N} Aktivitäten abonniert: {ACTIVITIES}",
		enUK: "You are subscribed to {N} activities: {ACTIVITIES}",
		esES: "Estás suscrito a {N} actividades: {ACTIVITIES}",
		faIR: "شما در {N} فعالیت مشترک هستید: {ACTIVITIES}",
		frFR: "Vous êtes abonné à {N} activités : {ACTIVITIES}",
		idID: "Anda berlangganan {N} aktivitas: {ACTIVITIES}",
		itIT: "Sei iscritto a {N} attività: {ACTIVITIES}",
		jaJP: "{N} 件のアクティビティを購読しています: {ACTIVITIES}",
		koKR: "{N}개 활동에 가입되어 있습니다: {ACTIVITIES}",
		plPL: "Jesteś subskrybowany do {N} aktywności: {ACTIVITIES}",
		ptBR: "Você está inscrito em {N} atividades: {ACTIVITIES}",
		ptPT: "Está inscrito em {N} atividades: {ACTIVITIES}",
		ruRU: "Вы подписаны на {N} категори(и/й):  {ACTIVITIES}",
		trTR: "{N} etkinliğe abone oldunuz: {ACTIVITIES}",
		ukUA: "Ви підписані на {N} активностей: {ACTIVITIES}",
		uzUZ: "Siz {N} ta faoliyatga obuna boʻldingiz: {ACTIVITIES}",
		zhCN: "您已订阅 {N} 个活动：{ACTIVITIES}"},
	EditIntentButtonText: {
		arEG: "📝 تعديل النية",
		deDE: "📝 Bearbeitungsabsicht",
		enUK: "📝 Edit intent",
		esES: "📝 Intención de edición",
		faIR: "📝 ویرایش قصد",
		frFR: "📝 Modifier l&#39;intention",
		idID: "📝 Edit maksud",
		itIT: "📝 Modifica intento",
		jaJP: "📝 編集意図",
		koKR: "📝 의도 편집",
		plPL: "📝 Edytuj intencję",
		ptBR: "📝 Editar intenção",
		ptPT: "📝 Editar intenção",
		ruRU: "📝 Редактировать план",
		trTR: "📝 Düzenleme amacı",
		ukUA: "📝 Редагувати намір",
		uzUZ: "📝 Niyatni tahrirlash",
		zhCN: "📝 编辑意图"},
	ShareIntentButtonText: {
		arEG: "📤 شارك النية",
		deDE: "📤 Absicht teilen",
		enUK: "📤 Share intent",
		esES: "📤 Compartir intención",
		faIR: "📤 قصد و نیت را به اشتراک بگذارید",
		frFR: "📤 Partager l&#39;intention",
		idID: "📤 Bagikan maksudnya",
		itIT: "📤 Condividi l&#39;intento",
		jaJP: "📤 意図を共有する",
		koKR: "📤 의도를 공유하세요",
		plPL: "📤 Udostępnij intencję",
		ptBR: "📤 Compartilhe a intenção",
		ptPT: "📤 Partilhe a intenção",
		ruRU: "📤 Поделиться планом",
		trTR: "📤 Paylaşım amacı",
		ukUA: "📤 Поділіться наміром",
		uzUZ: "📤 Niyatni baham ko&#39;ring",
		zhCN: "📤 分享意图"},
	ShareIntentArticleTitle: {
		arEG: "النية: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		deDE: "Absicht: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		enUK: "Intent: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		esES: "Intención: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		faIR: "هدف: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		frFR: "Intention : {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		idID: "Maksud: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		itIT: "Intento: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		jaJP: "意図: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		koKR: "의도: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		plPL: "Intencja: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		ptBR: "Intenção: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		ptPT: "Intenção: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		ruRU: "План: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		trTR: "Amaç: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		ukUA: "Намір: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		uzUZ: "Maqsad: {USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}",
		zhCN: "意图：{USER_TITLE} @ {INTENT_EMOJI} {SPOT_TITLE}"},
	ShareIntentRSVPButtonText: {
		arEG: "✔️ هل ستنضم؟ يرجى تأكيد الحضور!",
		deDE: "✔️ Machst du mit? Bitte melden Sie sich an!",
		enUK: "✔️ Will you join? Please RSVP!",
		esES: "✔️ ¿Te apuntas? ¡Confirma tu asistencia!",
		faIR: "✔️ آیا به ما ملحق خواهید شد؟ لطفا پاسخ دهید!",
		frFR: "✔️ Vous vous joindrez à nous ? Merci de confirmer votre présence !",
		idID: "✔️ Mau ikut? Silakan konfirmasi kehadiran Anda!",
		itIT: "✔️ Vuoi partecipare? Conferma la tua presenza!",
		jaJP: "✔️ ご参加いただけますか？ぜひご返信ください！",
		koKR: "✔️ 함께 하시겠습니까? RSVP 부탁드립니다!",
		plPL: "✔️ Dołączysz? Potwierdź swoją obecność!",
		ptBR: "✔️ Você vai participar? Por favor, confirme sua presença!",
		ptPT: "✔️ Vai participar? Por favor, confirme a sua presença!",
		ruRU: "✔️ Присоеденитесь? Дайте знать!",
		trTR: "✔️ Katılacak mısınız? Lütfen katılımınızı bildirin!",
		ukUA: "✔️ Приєднаєтесь? Будь ласка, підтвердіть свою участь!",
		uzUZ: "✔️ Qo&#39;shilasizmi? Iltimos, javob bering!",
		zhCN: "✔️ 你会参加吗？请回复！"},
	ShareIntentWhoElseGoingButtonText: {
		arEG: "❓من سيذهب أيضًا؟",
		deDE: "❓ Wer geht noch?",
		enUK: "❓ Who else is going?",
		esES: "❓¿Quién más va?",
		faIR: "❓ چه کس دیگری می‌رود؟",
		frFR: "❓ Qui d’autre y va ?",
		idID: "❓ Siapa lagi yang akan pergi?",
		itIT: "❓ Chi altro andrà?",
		jaJP: "❓ 他に誰が行きますか?",
		koKR: "❓ 또 누가 가나요?",
		plPL: "❓ Kto jeszcze idzie?",
		ptBR: "❓ Quem mais vai?",
		ptPT: "❓ Quem mais vai?",
		ruRU: "❓ Кто ещё идёт?",
		trTR: "❓ Başka kimler gidiyor?",
		ukUA: "❓ Хто ще йде?",
		uzUZ: "❓ Yana kim ketyapti?",
		zhCN: "❓ 还有谁要去？"},
	ShareIntentDoYouNeedARideButtonText: {
		arEG: "🚗🙋هل تحتاج إلى توصيلة؟",
		deDE: "🚗🙋 Brauchst du eine Mitfahrgelegenheit?",
		enUK: "🚗🙋 Do you need a ride?",
		esES: "🚗🙋¿Necesitas que te lleve?",
		faIR: "🚗🙋 به ماشین نیاز داری؟",
		frFR: "🚗🙋 Avez-vous besoin d&#39;un transport ?",
		idID: "🚗🙋 Apakah Anda butuh tumpangan?",
		itIT: "🚗🙋 Hai bisogno di un passaggio?",
		jaJP: "🚗🙋 乗せてもらってもいいですか？",
		koKR: "🚗🙋 태워드릴까요?",
		plPL: "🚗🙋 Potrzebujesz podwózki?",
		ptBR: "🚗🙋 Você precisa de uma carona?",
		ptPT: "🚗🙋 Precisa de boleia?",
		ruRU: "🚗🙋 Вас подвезти?",
		trTR: "🚗🙋 Bir yolculuğa mı ihtiyacınız var?",
		ukUA: "🚗🙋 Вам потрібен підвіз?",
		uzUZ: "🚗🙋 Sizga sayohat kerakmi?",
		zhCN: "🚗🙋 你需要搭车吗？"},
	ShareIntentArticleUnspecifiedActivity: {
		arEG: "نشاط غير محدد",
		deDE: "nicht näher bezeichnete Tätigkeit",
		enUK: "unspecified activity",
		esES: "actividad no especificada",
		faIR: "فعالیت نامشخص",
		frFR: "activité non spécifiée",
		idID: "aktivitas yang tidak ditentukan",
		itIT: "attività non specificata",
		jaJP: "不明な活動",
		koKR: "지정되지 않은 활동",
		plPL: "nieokreślona aktywność",
		ptBR: "atividade não especificada",
		ptPT: "atividade não especificada",
		ruRU: "не указано что будет делаться",
		trTR: "belirtilmemiş etkinlik",
		ukUA: "невизначена діяльність",
		uzUZ: "aniqlanmagan faoliyat",
		zhCN: "未指明的活动"},
	ShareIntentArticleDescription: {
		arEG: "النقر على هذا سوف يؤدي إلى مشاركة خطتك للدردشة الحالية.",
		deDE: "Wenn Sie hierauf klicken, wird Ihr Plan für den aktuellen Chat freigegeben.",
		enUK: "Clicking to this will share your plan for the to the current chat.",
		esES: "Al hacer clic aquí, compartirá su plan para el chat actual.",
		faIR: "با کلیک روی این گزینه، برنامه شما برای چت فعلی به اشتراک گذاشته می‌شود.",
		frFR: "En cliquant dessus, vous partagerez votre plan pour le chat en cours.",
		idID: "Mengklik ini akan membagikan rencana Anda untuk obrolan saat ini.",
		itIT: "Facendo clic qui, il tuo piano verrà condiviso con la chat corrente.",
		jaJP: "これをクリックすると、現在のチャットの計画が共有されます。",
		koKR: "이것을 클릭하면 현재 채팅에 대한 귀하의 계획이 공유됩니다.",
		plPL: "Kliknięcie tego spowoduje udostępnienie planu bieżącego czatu.",
		ptBR: "Clicar aqui compartilhará seu plano para o bate-papo atual.",
		ptPT: "Clicar aqui partilhará o seu plano para o chat atual.",
		ruRU: "Кликнув вы поделитесь своим планом в текущем чате.",
		trTR: "Buraya tıkladığınızda mevcut sohbete ait planınız paylaşılacaktır.",
		ukUA: "Натиснувши на це, ви поділитеся своїм планом у поточному чаті.",
		uzUZ: "Buni bossangiz, joriy chat uchun rejangizni baham ko&#39;rasiz.",
		zhCN: "单击此处将与当前聊天分享您的计划。"},
	ShareIntentArticleTextLikelihoodYes: {
		arEG: "من <u>المؤكد</u> أن <b>{USER}</b> سيكون في <b>{SPOT}</b> في <b>{DATE}</b> من <code>{ARRIVAL}</code> حتى <code>{DEPARTURE}</code> ويقوم بما يلي:",
		deDE: "<b>{USER}</b> wird <u>mit Sicherheit</u> vom <code>{ARRIVAL}</code> bis <code>{DEPARTURE}</code> am <b>{DATE}</b> um <b>{SPOT}</b> sein und Folgendes tun:",
		enUK: "<b>{USER}</b> is <u>certainly going</u> to be at <b>{SPOT}</b> on <b>{DATE}</b> from <code>{ARRIVAL}</code> till <code>{DEPARTURE}</code> doing:",
		esES: "<b>{USER}</b> <u>seguramente</u> estará en <b>{SPOT}</b> el <b>{DATE}</b> desde <code>{ARRIVAL}</code> hasta <code>{DEPARTURE}</code> haciendo:",
		faIR: "<b>{USER}</b> <u>مطمئناً</u> از <code>{ARRIVAL}</code> تا <code>{DEPARTURE}</code> در <b>{SPOT}</b> <b>و {DATE}</b> خواهد بود و کارهای زیر را انجام می‌دهد:",
		frFR: "<b>{USER}</b> <u>sera certainement</u> à <b>{SPOT}</b> le <b>{DATE}</b> de <code>{ARRIVAL}</code> à <code>{DEPARTURE}</code> pour faire :",
		idID: "<b>{USER}</b> <u>pasti akan</u> berada di <b>{SPOT}</b> pada <b>{DATE}</b> dari <code>{ARRIVAL}</code> sampai <code>{DEPARTURE}</code> melakukan:",
		itIT: "<b>{USER}</b> <u>sarà sicuramente</u> a <b>{SPOT}</b> il <b>{DATE}</b> dal <code>{ARRIVAL}</code> al <code>{DEPARTURE}</code> facendo:",
		jaJP: "<b>{USER}</b>は、 <b>{DATE}</b>の<code>{ARRIVAL}</code>から<code>{DEPARTURE}</code>まで、 <b>{SPOT}</b>に<u>必ず出席し、次のことを行います</u>:",
		koKR: "<b>{USER}</b> 은 <u>확실히</u> <b>{DATE}의 {SPOT}</b> <b>에</b> <code>{ARRIVAL}</code> 부터 <code>{DEPARTURE}</code> 까지 있을 것입니다.",
		plPL: "<b>{USER}</b> <u>z pewnością będzie</u> o <b>{SPOT}</b> dnia <b>{DATE}</b> od <code>{ARRIVAL}</code> do <code>{DEPARTURE}</code> , wykonując:",
		ptBR: "<b>{USER}</b> <u>certamente estará</u> em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ptPT: "<b>{USER}</b> <u>estará certamente</u> em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ruRU: "<b>{USER}</b> <u>точно будет</u> на/в <b>{SPOT}</b> <b>{DATE}</b> c <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code>, будет делать:",
		trTR: "<b>{USER}</b> <u>kesinlikle</u> <b>{DATE}</b> günü <b>{SPOT}</b> konumunda olacak ve <code>{ARRIVAL}</code> ile <code>{DEPARTURE}</code> arasında şunları yapacak:",
		ukUA: "<b>{USER}</b> <u>точно буде</u> в <b>{SPOT}</b> на <b>{DATE}</b> з <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code> , роблячи:",
		uzUZ: "<b>{USER}</b> <u>, albatta,</u> <b>{DATE}</b> kuni <b>{SPOT}</b> da <code>{ARRIVAL}</code> dan <code>{DEPARTURE}</code> gacha boʻladi:",
		zhCN: "<b>{USER}</b><u>肯定会</u>在<b>{DATE} 的 {SPOT}</b><b>从</b><code>{ARRIVAL}</code>到<code>{DEPARTURE}</code>执行以下任务："},
	ShareIntentArticleTextLikelihoodNo: {
		arEG: "<u>لن</u> يكون <b>{USER}</b> في <b>{SPOT}</b> يوم <b>{DATE}</b> من <code>{ARRIVAL}</code> حتى <code>{DEPARTURE}</code>",
		deDE: "<b>{USER}</b> wird vom <code>{ARRIVAL}</code> bis <code>{DEPARTURE}</code> <u>nicht</u> am <b>{DATE}</b> um <b>{SPOT}</b> sein.",
		enUK: "<b>{USER}</b> is <u>not going</u> to be at <b>{SPOT}</b> on <b>{DATE}</b> from <code>{ARRIVAL}</code> till <code>{DEPARTURE}</code>",
		esES: "<b>{USER}</b> <u>no estará</u> en <b>{SPOT}</b> el <b>{DATE}</b> desde <code>{ARRIVAL}</code> hasta <code>{DEPARTURE}</code>",
		faIR: "<b>{USER}</b> در <b>{SPOT}</b> و <b>{DATE}</b> از <code>{ARRIVAL}</code> تا <code>{DEPARTURE}</code> <u>نخواهد</u> بود.",
		frFR: "<b>{USER}</b> <u>ne sera pas</u> présent à <b>{SPOT}</b> le <b>{DATE}</b> de <code>{ARRIVAL}</code> à <code>{DEPARTURE}</code>",
		idID: "<b>{USER}</b> <u>tidak akan</u> berada di <b>{SPOT}</b> pada <b>{DATE}</b> dari <code>{ARRIVAL}</code> sampai <code>{DEPARTURE}</code>",
		itIT: "<b>{USER}</b> <u>non sarà</u> presente a <b>{SPOT}</b> il <b>{DATE}</b> dal <code>{ARRIVAL}</code> al <code>{DEPARTURE}</code>",
		jaJP: "<b>{USER}</b><b>は</b>{DATE} の<code>{ARRIVAL}</code>から<code>{DEPARTURE}</code>まで<b>{SPOT}</b>には<u>いません</u>",
		koKR: "<b>{USER}은</b> <code>{ARRIVAL}</code> 부터 <code>{DEPARTURE}</code> 까지 <b>{DATE}</b> 의 <b>{SPOT}</b> 에 있지 <u>않습니다</u> .",
		plPL: "<b>{USER}</b> <u>nie będzie</u> w <b>{SPOT}</b> dnia <b>{DATE}</b> od <code>{ARRIVAL}</code> do <code>{DEPARTURE}</code>",
		ptBR: "<b>{USER}</b> <u>não estará</u> em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code>",
		ptPT: "<b>{USER}</b> <u>não estará</u> em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code>",
		ruRU: "<b>{USER}</b> <u>не будет</u> на/в <b>{SPOT}</b> <b>{DATE}</b> c <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code>",
		trTR: "<b>{USER}</b> <code>{ARRIVAL}</code> ile <code>{DEPARTURE}</code> arasında <b>{DATE}</b> tarihinde <b>{SPOT}</b> konumunda <u>olmayacak</u>",
		ukUA: "<b>{USER}</b> <u>не буде</u> в <b>{SPOT}</b> у <b>{DATE}</b> з <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code>",
		uzUZ: "<b>{USER}</b> <b>{DATE} kuni {SPOT}</b> <b>da</b> <code>{ARRIVAL}</code> dan <code>{DEPARTURE}</code> gacha <u>boʻlmaydi</u>",
		zhCN: "<b>{USER}</b><u>不会</u>在<b>{DATE} 的</b><b>{SPOT}</b>出现，从<code>{ARRIVAL}</code>到<code>{DEPARTURE}</code>"},
	ShareIntentArticleTextLikelihoodMaybe: {
		arEG: "<u>قد</u> يكون <b>{USER}</b> في <b>{SPOT}</b> في <b>{DATE}</b> من <code>{ARRIVAL}</code> حتى <code>{DEPARTURE}</code> يفعل:",
		deDE: "<b>{USER}</b> ist <u>möglicherweise</u> am <b>{DATE}</b> von <code>{ARRIVAL}</code> bis <code>{DEPARTURE}</code> um <b>{SPOT}</b> und macht Folgendes:",
		enUK: "<b>{USER}</b> <u>might</u> be at <b>{SPOT}</b> on <b>{DATE}</b> from <code>{ARRIVAL}</code> till <code>{DEPARTURE}</code> doing:",
		esES: "<b>{USER}</b> <u>podría</u> estar en <b>{SPOT}</b> el <b>{DATE}</b> desde <code>{ARRIVAL}</code> hasta <code>{DEPARTURE}</code> haciendo:",
		faIR: "<u>ممکن است</u> <b>{USER}</b> در <b>{SPOT}</b> و <b>{DATE}</b> از <code>{ARRIVAL}</code> تا <code>{DEPARTURE}</code> باشد و این کار را انجام دهد:",
		frFR: "<b>{USER}</b> <u>pourrait</u> être à <b>{SPOT}</b> le <b>{DATE}</b> de <code>{ARRIVAL}</code> à <code>{DEPARTURE}</code> en train de faire :",
		idID: "<b>{USER}</b> <u>mungkin</u> berada di <b>{SPOT}</b> pada <b>{DATE}</b> dari <code>{ARRIVAL}</code> sampai <code>{DEPARTURE}</code> melakukan:",
		itIT: "<b>{USER}</b> <u>potrebbe</u> essere a <b>{SPOT}</b> il <b>{DATE}</b> da <code>{ARRIVAL}</code> fino a <code>{DEPARTURE}</code> facendo:",
		jaJP: "<b>{USER} は</b><b>{DATE}</b>の<code>{ARRIVAL}</code>から<code>{DEPARTURE}</code>まで<b>{SPOT}</b>にいる<u>可能性があります</u>:",
		koKR: "<b>{USER}은</b> <code>{ARRIVAL}</code> 부터 <code>{DEPARTURE}</code> 까지 <b>{DATE}</b> 의 <b>{SPOT}</b> 에 있을 <u>수</u> 있습니다.",
		plPL: "<b>{USER}</b> <u>może</u> być o <b>{SPOT}</b> dnia <b>{DATE}</b> od <code>{ARRIVAL}</code> do <code>{DEPARTURE}</code> wykonując:",
		ptBR: "<b>{USER}</b> <u>pode</u> estar em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ptPT: "<b>{USER}</b> <u>pode</u> estar em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ruRU: "<b>{USER}</b> <u>возможно</u> будет на/в <b>{SPOT}</b> <b>{DATE}</b> c <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code>, будет делать:",
		trTR: "<b>{USER},</b> <b>{DATE}</b> günü <code>{ARRIVAL}</code> ile <code>{DEPARTURE}</code> arasında <b>{SPOT}</b> konumunda <u>olabilir</u> :",
		ukUA: "<b>{USER}</b> <u>може</u> бути в <b>{SPOT}</b> на <b>{DATE}</b> з <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code> , виконуючи:",
		uzUZ: "<b>{USER}</b> <b>{DATE} kuni {SPOT}</b> <b>da</b> <code>{ARRIVAL}</code> dan <code>{DEPARTURE}</code> gacha boʻlishi <u>mumkin</u> :",
		zhCN: "<b>{USER}</b><u>可能</u>会在<b>{DATE}</b>的<b>{SPOT}</b>从<code>{ARRIVAL}</code>到<code>{DEPARTURE}</code>执行以下任务："},
	ShareIntentArticleTextLikelihoodLikely: {
		arEG: "من <u>المرجح</u> أن يكون <b>{USER}</b> في <b>{SPOT}</b> في <b>{DATE}</b> من <code>{ARRIVAL}</code> حتى <code>{DEPARTURE}</code> ويقوم بما يلي:",
		deDE: "<b>{USER}</b> ist <u>wahrscheinlich</u> am <b>{DATE}</b> von <code>{ARRIVAL}</code> bis <code>{DEPARTURE}</code> um <b>{SPOT}</b> und macht Folgendes:",
		enUK: "<b>{USER}</b> is <u>likely</u> to be at <b>{SPOT}</b> on <b>{DATE}</b> from <code>{ARRIVAL}</code> till <code>{DEPARTURE}</code> doing:",
		esES: "Es <u>probable</u> que <b>{USER}</b> esté en <b>{SPOT}</b> el <b>{DATE}</b> desde <code>{ARRIVAL}</code> hasta <code>{DEPARTURE}</code> haciendo:",
		faIR: "<u>احتمالاً</u> <b>{USER}</b> در <b>{SPOT}</b> و <b>{DATE}</b> از <code>{ARRIVAL}</code> تا <code>{DEPARTURE}</code> در حال انجام کارهای زیر است:",
		frFR: "<b>{USER}</b> sera <u>probablement</u> à <b>{SPOT}</b> le <b>{DATE}</b> de <code>{ARRIVAL}</code> à <code>{DEPARTURE}</code> en train de faire :",
		idID: "<b>{USER}</b> <u>kemungkinan</u> berada di <b>{SPOT}</b> pada <b>{DATE}</b> dari <code>{ARRIVAL}</code> sampai <code>{DEPARTURE}</code> melakukan:",
		itIT: "È <u>probabile</u> che <b>{USER}</b> sia a <b>{SPOT}</b> il <b>{DATE}</b> da <code>{ARRIVAL}</code> fino a <code>{DEPARTURE}</code> eseguendo:",
		jaJP: "<b>{USER} は</b><b>、</b> <code>{ARRIVAL}</code>から<code>{DEPARTURE}</code>まで<b>{SPOT}</b>にいて、次のことを行う<u>可能性が</u>あります:",
		koKR: "<b>{USER}</b> <code>{ARRIVAL}</code> 부터 <code>{DEPARTURE}</code> 까지 <b>{DATE}</b> 의 <b>{SPOT}</b> 에 있을 <u>가능성이</u> 높습니다.",
		plPL: "<b>{USER}</b> <u>prawdopodobnie</u> będzie o <b>{SPOT}</b> dnia <b>{DATE}</b> od <code>{ARRIVAL}</code> do <code>{DEPARTURE}</code> , wykonując:",
		ptBR: "<b>{USER}</b> <u>provavelmente</u> estará em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ptPT: "<b>{USER}</b> estará <u>provavelmente</u> em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ruRU: "<b>{USER}</b> <u>скорее всего будет</u> на/в <b>{SPOT}</b> <b>{DATE}</b> c <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code>, будет делать:",
		trTR: "<b>{USER}</b> <b>&#39;ın {DATE}</b> günü <code>{ARRIVAL}</code> ile <code>{DEPARTURE}</code> arasında <b>{SPOT}</b> konumunda olması <u>muhtemeldir</u> :",
		ukUA: "<b>{USER}</b> , <u>ймовірно,</u> буде в <b>{SPOT}</b> у <b>{DATE}</b> з <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code> , виконуючи такі дії:",
		uzUZ: "<b>{USER}</b> <b>{DATE} kuni {SPOT}</b> <b>da</b> <code>{ARRIVAL}</code> dan <code>{DEPARTURE}</code> gacha boʻlishi <u>mumkin</u> :",
		zhCN: "<b>{USER}</b><u>可能</u>会在<b>{DATE} 的 {SPOT}</b><b>从</b><code>{ARRIVAL}</code>到<code>{DEPARTURE}</code>执行以下任务："},
	ShareIntentArticleTextLikelihoodUnlikely: {
		arEG: "من غير المرجح أن يكون <b>{USER}</b> في <b>{SPOT}</b> في <b>{DATE}</b> من <code>{ARRIVAL}</code> حتى <code>{DEPARTURE}</code> يقوم بما يلي:",
		deDE: "Es ist unwahrscheinlich, dass <b>{USER}</b> vom <code>{ARRIVAL}</code> bis <code>{DEPARTURE}</code> am <b>{DATE}</b> um <b>{SPOT}</b> ist und Folgendes tut:",
		enUK: "<b>{USER}</b> is unlikely to be at <b>{SPOT}</b> on <b>{DATE}</b> from <code>{ARRIVAL}</code> till <code>{DEPARTURE}</code> doing:",
		esES: "Es poco probable que <b>{USER}</b> esté en <b>{SPOT}</b> el <b>{DATE}</b> desde <code>{ARRIVAL}</code> hasta <code>{DEPARTURE}</code> haciendo:",
		faIR: "بعید است که <b>{USER}</b> از <code>{ARRIVAL}</code> تا <code>{DEPARTURE}</code> در <b>{SPOT}</b> و <b>{DATE}</b> باشد و کارهای زیر را انجام دهد:",
		frFR: "Il est peu probable que <b>{USER}</b> soit à <b>{SPOT}</b> le <b>{DATE}</b> de <code>{ARRIVAL}</code> à <code>{DEPARTURE}</code> en train de faire :",
		idID: "<b>{USER}</b> tidak mungkin berada di <b>{SPOT}</b> pada <b>{DATE}</b> dari <code>{ARRIVAL}</code> sampai <code>{DEPARTURE}</code> melakukan:",
		itIT: "È improbabile che <b>{USER}</b> sia a <b>{SPOT}</b> il <b>{DATE}</b> dal <code>{ARRIVAL}</code> al <code>{DEPARTURE}</code> eseguendo:",
		jaJP: "<b>{USER} は</b>、 <code>{ARRIVAL}</code>から<code>{DEPARTURE}</code>まで<b>{DATE} の {SPOT}</b><b>にいて</b>、次のことを行う可能性は低いです:",
		koKR: "<b>{USER}</b> <code>{ARRIVAL}</code> 부터 <code>{DEPARTURE}</code> 까지 <b>{DATE}</b> 의 <b>{SPOT}</b> 에 있을 가능성이 낮습니다.",
		plPL: "Jest mało prawdopodobne, że <b>{USER}</b> będzie o <b>{SPOT}</b> w dniu <b>{DATE}</b> od <code>{ARRIVAL}</code> do <code>{DEPARTURE}</code> , wykonując:",
		ptBR: "É improvável que <b>{USER}</b> esteja em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ptPT: "É improvável que <b>{USER}</b> esteja em <b>{SPOT}</b> em <b>{DATE}</b> de <code>{ARRIVAL}</code> até <code>{DEPARTURE}</code> fazendo:",
		ruRU: "<b>{USER}</b> <u>скорее всего НЕ будет</u> на/в <b>{SPOT}</b> <b>{DATE}</b> c <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code>, будет делать:",
		trTR: "<b>{USER}</b> &#39;ın <b>{DATE}</b> günü <code>{ARRIVAL}</code> ten <code>{DEPARTURE}</code> e kadar <b>{SPOT}</b> konumunda olması pek olası değil:",
		ukUA: "<b>{USER}</b> навряд чи буде в <b>{SPOT}</b> у <b>{DATE}</b> з <code>{ARRIVAL}</code> до <code>{DEPARTURE}</code> , виконуючи:",
		uzUZ: "<b>{USER}</b> <b>{DATE} kuni {SPOT}</b> <b>da</b> <code>{ARRIVAL}</code> dan <code>{DEPARTURE}</code> gacha boʻlishi mumkin emas:",
		zhCN: "<b>{USER}</b>不太可能在<b>{DATE} 的 {SPOT}</b><b>从</b><code>{ARRIVAL}</code>到<code>{DEPARTURE}</code>执行以下任务："},
	RsvpDateTime: {
		arEG: "في: <b>{DATE}</b> @ {START_TIME} حتى {END_TIME}",
		deDE: "Am: <b>{DATE}</b> @ {START_TIME} bis {END_TIME}",
		enUK: "On: <b>{DATE}</b> @ {START_TIME} till {END_TIME}",
		esES: "En: <b>{DATE}</b> a las {START_TIME} hasta las {END_TIME}",
		faIR: "روشن: <b>{DATE}</b> @ {START_TIME} تا {END_TIME}",
		frFR: "Du : <b>{DATE}</b> à {START_TIME} jusqu&#39;à {END_TIME}",
		idID: "Aktif: <b>{DATE}</b> @ {START_TIME} sampai {END_TIME}",
		itIT: "Acceso: <b>{DATE}</b> @ {START_TIME} fino a {END_TIME}",
		jaJP: "オン: <b>{DATE}</b> @ {START_TIME}、{END_TIME}まで",
		koKR: "시작일: <b>{DATE}</b> @ {START_TIME}부터 {END_TIME}까지",
		plPL: "W dniu: <b>{DATE}</b> @ {START_TIME} do {END_TIME}",
		ptBR: "Em: <b>{DATE}</b> @ {START_TIME} até {END_TIME}",
		ptPT: "Em: <b>{DATE}</b> @ {START_TIME} até {END_TIME}",
		ruRU: "Дата и время: <b>{DATE}</b> @ с {START_TIME} по {END_TIME}",
		trTR: "Açık: <b>{DATE}</b> @ {START_TIME} - {END_TIME}",
		ukUA: "З: <b>{DATE}</b> о {START_TIME} до {END_TIME}",
		uzUZ: "Yoniq: <b>{DATE}</b> @ {START_TIME} dan {END_TIME} gacha",
		zhCN: "时间： <b>{DATE}</b> @ {START_TIME} 至 {END_TIME}"},
	LocalAds: {
		arEG: "الإعلانات المحلية",
		deDE: "Lokale Anzeigen",
		enUK: "Local ads",
		esES: "Anuncios locales",
		faIR: "تبلیغات محلی",
		frFR: "Annonces locales",
		idID: "Iklan lokal",
		itIT: "Annunci locali",
		jaJP: "ローカル広告",
		koKR: "지역 광고",
		plPL: "Reklamy lokalne",
		ptBR: "Anúncios locais",
		ptPT: "Anúncios locais",
		ruRU: "Локальная реклама",
		trTR: "Yerel reklamlar",
		ukUA: "Локальні оголошення",
		uzUZ: "Mahalliy reklamalar",
		zhCN: "本地广告"},
	NoLocalAdsYet: {
		arEG: "لا توجد إعلانات محلية لـ <code>%s</code> حتى الآن.\nكن أول من <b>يضع إعلانًا مقابل <u>1 ⭐ فقط في اليوم</u></b> !",
		deDE: "Es gibt noch keine lokalen Anzeigen für <code>%s</code> .\nSeien Sie der Erste, der <b>eine Anzeige für <u>nur 1 ⭐ pro Tag</u> platziert</b> !",
		enUK: "There is no local ads for <code>%s</code> yet.\nBe first to <b>place an ad for <u>just 1 ⭐ per day</u></b>!",
		esES: "Aún no hay anuncios locales para <code>%s</code> .\n¡Sé el primero en <b>colocar un anuncio por <u>solo 1 ⭐ por día</u></b> !",
		faIR: "هنوز هیچ تبلیغ محلی برای <code>%s</code> وجود ندارد.\nاولین نفری باشید <b>که <u>فقط با ۱⭐ دلار در روز</u> تبلیغ ثبت می‌کند</b> !",
		frFR: "Il n&#39;y a pas encore d&#39;annonces locales pour <code>%s</code> .\nSoyez le premier à <b>placer une annonce pour <u>seulement 1 ⭐ par jour</u></b> !",
		idID: "Belum ada iklan lokal untuk <code>%s</code> .\nJadilah yang pertama <b>memasang iklan <u>hanya dengan 1 ⭐ per hari</u></b> !",
		itIT: "Non ci sono ancora annunci locali per <code>%s</code> .\nSii il primo a <b>pubblicare un annuncio per <u>solo 1 ⭐ al giorno</u></b> !",
		jaJP: "<code>%s</code>のローカル広告はまだありません。\n1<b><u>日 1 ⭐</u>で最初に広告を掲載しましょう</b>。",
		koKR: "<code>%s</code> 에 대한 지역 광고가 아직 없습니다.\n <b><u>하루에 1⭐</u> 로 가장 먼저 광고를 게재하세요</b> !",
		plPL: "Nie ma jeszcze żadnych lokalnych ogłoszeń dla <code>%s</code> .\nBądź pierwszy i <b>dodaj ogłoszenie za <u>jedyne 1 ⭐ dziennie</u></b> !",
		ptBR: "Ainda não há anúncios locais para <code>%s</code> . Seja o primeiro a <b>anunciar por <u>apenas 1 ⭐ por dia</u></b> !",
		ptPT: "Ainda não existem anúncios locais para <code>%s</code> . Seja o primeiro a <b>anunciar por <u>apenas 1 ⭐ por dia</u></b> !",
		ruRU: "Пока ещё нет локальной рекламы для <code>%s</code>. Разместите рекаламу первыми <u>всего за 1 ⭐ в день.</u>!",
		trTR: "<code>%s</code> için henüz yerel reklam yok.\n <b><u>nGünde sadece 1 ⭐</u> karşılığında ilk reklamı veren</b> siz olun!",
		ukUA: "Поки що немає місцевих оголошень для <code>%s</code> .\nБудьте першим, хто <b>розмістить оголошення <u>лише за 1 ⭐ на день</u></b> !",
		uzUZ: "Hozircha <code>%s</code> uchun mahalliy reklamalar yo‘q.\nBirinchi bo‘lib <b><u>kuniga atigi 1 ⭐</u> evaziga reklama joylashtiring</b> !",
		zhCN: "<code>%s</code>尚无本地广告。\n抢先<b>发布广告，<u>每天仅需 1⭐</u></b> ！"},
	TimezoneIsNotSet: {
		arEG: "لم يتم تحديد المنطقة الزمنية.",
		deDE: "Die Zeitzone ist nicht festgelegt.",
		enUK: "Time zone is not set.",
		esES: "La zona horaria no está establecida.",
		faIR: "منطقه زمانی تنظیم نشده است.",
		frFR: "Le fuseau horaire n&#39;est pas défini.",
		idID: "Zona waktu belum ditetapkan.",
		itIT: "Il fuso orario non è impostato.",
		jaJP: "タイムゾーンが設定されていません。",
		koKR: "시간대가 설정되지 않았습니다.",
		plPL: "Strefa czasowa nie jest ustawiona.",
		ptBR: "O fuso horário não está definido.",
		ptPT: "O fuso horário não está definido.",
		ruRU: "Часовой пояс не задан.",
		trTR: "Saat dilimi ayarlanmamış.",
		ukUA: "Часовий пояс не встановлено.",
		uzUZ: "Vaqt mintaqasi belgilanmagan.",
		zhCN: "未设置时区。"},
	TimezoneLabel: {
		arEG: "المنطقة الزمنية: %s",
		deDE: "Zeitzone: %s",
		enUK: "Timezone: %s",
		esES: "Zona horaria: %s",
		faIR: "منطقه زمانی: %s",
		frFR: "Fuseau horaire : %s",
		idID: "Zona waktu: %s",
		itIT: "Fuso orario: %s",
		jaJP: "タイムゾーン: %s",
		koKR: "시간대: %s",
		plPL: "Strefa czasowa: %s",
		ptBR: "Fuso horário: %s",
		ptPT: "Fuso horário: %s",
		ruRU: "Часовой пояс: %s",
		trTR: "Saat dilimi: %s",
		ukUA: "Часовий пояс: %s",
		uzUZ: "Vaqt mintaqasi: %s",
		zhCN: "时区：%s"},
	...
	Subscribed1Activity: {
		arEG: "👀 مشترك: 1 نشاط",
		deDE: "👀 Abonniert: 1 Aktivität",
		enUK: "👀 Subscribed: 1 activity",
		esES: "👀 Suscritos: 1 actividad",
		faIR: "👀 مشترک شده: ۱ فعالیت",
		frFR: "👀 Abonné : 1 activité",
		idID: "👀 Berlangganan: 1 aktivitas",
		itIT: "👀 Iscritto: 1 attività",
		jaJP: "👀 登録済み: 1 件のアクティビティ",
		koKR: "👀 구독: 1개 활동",
		plPL: "👀 Subskrybowano: 1 aktywność",
		ptBR: "👀 Inscrito: 1 atividade",
		ptPT: "👀 Inscrito: 1 atividade",
		ruRU: "👀 Подписки: 1 категория",
		trTR: "👀 Abone olundu: 1 etkinlik",
		ukUA: "👀 Підписано: 1 активність",
		uzUZ: "👀 Obuna boʻlgan: 1 ta harakat",
		zhCN: "👀 订阅：1 个活动"},
	SubscribedNActivities: {
		arEG: "👀 مشترك: %s أنشطة",
		deDE: "👀 Abonniert: %s Aktivitäten",
		enUK: "👀 Subscribed: %s activities",
		esES: "👀 Suscritos: %s actividades",
		faIR: "👀 مشترک شده: %s فعالیت",
		frFR: "👀 Abonné : %s activités",
		idID: "👀 Berlangganan: %s aktivitas",
		itIT: "👀 Iscritto: %s attività",
		jaJP: "👀 登録済み: %s 件のアクティビティ",
		koKR: "👀 구독: %s 활동",
		plPL: "👀 Zasubskrybowano: %s aktywności",
		ptBR: "👀 Inscrito: %s atividades",
		ptPT: "👀 Inscrito: %s atividades",
		ruRU: "👀 Подписки: %s категори(и/й)",
		trTR: "👀 Abone olundu: %s etkinlik",
		ukUA: "👀 Підписано: %s активностей",
		uzUZ: "👀 Obuna boʻlgan: %s faoliyat",
		zhCN: "👀 已订阅：%s 个活动"},
	AdStarGiveawaysSneatDevDiaries: {
		arEG: "🎁 <b>نقوم كل شهر بإهداء Telegram ⭐⭐</b> إلى مشترك عشوائي في @SneatDevDiaries.",
		deDE: "🎁 Jeden Monat <b>verlosen wir Telegram ⭐⭐</b> an einen zufällig ausgewählten Abonnenten von @SneatDevDiaries.",
		enUK: "🎁 Every month <b>we are giving away Telegram ⭐⭐</b> to a random subscriber of @SneatDevDiaries.",
		esES: "🎁 Todos los meses <b>regalamos un Telegram ⭐⭐</b> a un suscriptor aleatorio de @SneatDevDiaries.",
		faIR: "🎁 هر ماه به یک مشترک تصادفی @SneatDevDiaries، <b>تلگرام ⭐⭐ هدیه می‌دهیم</b> .",
		frFR: "🎁 Chaque mois, <b>nous offrons un Telegram ⭐⭐</b> à un abonné aléatoire de @SneatDevDiaries.",
		idID: "🎁 Setiap bulan <b>kami memberikan Telegram ⭐⭐</b> kepada pelanggan acak @SneatDevDiaries.",
		itIT: "🎁 Ogni mese <b>regaliamo un Telegram ⭐⭐</b> a un abbonato casuale di @SneatDevDiaries.",
		jaJP: "🎁 毎月、@SneatDevDiaries のランダムな登録者に<b>Telegram ⭐⭐ をプレゼントしています</b>。",
		koKR: "🎁 매달 @SneatDevDiaries 구독자 중 무작위로 한 명에게 <b>Telegram ⭐⭐을 드립니다</b> .",
		plPL: "🎁 Co miesiąc <b>rozdajemy Telegram ⭐⭐</b> losowemu subskrybentowi @SneatDevDiaries.",
		ptBR: "🎁 Todo mês <b>estamos dando um Telegram ⭐⭐</b> para um assinante aleatório do @SneatDevDiaries.",
		ptPT: "🎁 Todos os meses <b>estamos a oferecer um Telegram ⭐⭐</b> a um subscritor aleatório do @SneatDevDiaries.",
		ruRU: "🎁 Каждый месяц <b>мы дарим Телеграм ⭐⭐</b> случайному подписчику @SneatDevDiaries.",
		trTR: "🎁 Her ay @SneatDevDiaries&#39;ın rastgele bir abonesine <b>Telegram ⭐⭐ hediye ediyoruz</b> .",
		ukUA: "🎁 Щомісяця <b>ми розігруємо Telegram ⭐⭐</b> випадковому підписнику @SneatDevDiaries.",
		uzUZ: "🎁 <b>Biz har oyda @SneatDevDiaries-ning tasodifiy obunachisiga Telegram ⭐⭐ sovg&#39;a qilamiz</b> .",
		zhCN: "🎁<b>我们每个月都会向@SneatDevDiaries 的随机订阅者赠送 Telegram ⭐⭐</b> 。"},
	AdLearAboutOurDevelopment: {
		arEG: "🧐 يمكنك <a href='https://t.me/SneatDevDiaries/4'>التعرف على كيفية تطوير</a> هذا الروبوت والتطبيقات الأخرى على قناتنا @SneatDevDiaries.",
		deDE: "🧐 Auf unserem Kanal @SneatDevDiaries <a href='https://t.me/SneatDevDiaries/4'>erfahren Sie, wie wir diesen Bot und andere Apps entwickeln</a> .",
		enUK: "🧐 You can <a href='https://t.me/SneatDevDiaries/4'>learn how we develop</a> this bot and other apps at our channel @SneatDevDiaries.",
		esES: "🧐 Puedes <a href='https://t.me/SneatDevDiaries/4'>aprender cómo desarrollamos</a> este bot y otras aplicaciones en nuestro canal @SneatDevDiaries.",
		faIR: "🧐 می‌توانید <a href='https://t.me/SneatDevDiaries/4'>نحوه‌ی توسعه‌ی این ربات و سایر برنامه‌ها را در کانال ما @SneatDevDiaries بیاموزید</a> .",
		frFR: "🧐 Vous pouvez <a href='https://t.me/SneatDevDiaries/4'>découvrir comment nous développons</a> ce bot et d&#39;autres applications sur notre chaîne @SneatDevDiaries.",
		idID: "🧐 Anda dapat <a href='https://t.me/SneatDevDiaries/4'>mempelajari cara kami mengembangkan</a> bot ini dan aplikasi lainnya di saluran kami @SneatDevDiaries.",
		itIT: "🧐 Puoi <a href='https://t.me/SneatDevDiaries/4'>scoprire come sviluppiamo</a> questo bot e altre app sul nostro canale @SneatDevDiaries.",
		jaJP: "🧐 このボットや他のアプリ<a href='https://t.me/SneatDevDiaries/4'>の開発方法については、当社のチャンネル @SneatDevDiaries で学べ</a>ます。",
		koKR: "🧐 @SneatDevDiaries 채널에서 이 봇과 다른 앱을 <a href='https://t.me/SneatDevDiaries/4'>개발하는 방법을 알아볼</a> 수 있습니다.",
		plPL: "🧐 Możesz <a href='https://t.me/SneatDevDiaries/4'>dowiedzieć się, jak rozwijamy</a> tego bota i inne aplikacje, na naszym kanale @SneatDevDiaries.",
		ptBR: "🧐 Você pode <a href='https://t.me/SneatDevDiaries/4'>aprender como desenvolvemos</a> este bot e outros aplicativos em nosso canal @SneatDevDiaries.",
		ptPT: "🧐 Pode <a href='https://t.me/SneatDevDiaries/4'>aprender como desenvolvemos</a> este bot e outras aplicações no nosso canal @SneatDevDiaries.",
		ruRU: "🧐 Вы можете <a href='https://t.me/SneatDevDiaries/4'>узнать о том как мы ведём разработку</a> этого бота и других приложенй на нашем канале @SneatDevDiaries.",
		trTR: "🧐 Bu botu ve diğer uygulamaları <a href='https://t.me/SneatDevDiaries/4'>nasıl geliştirdiğimizi @SneatDevDiaries kanalımızdan öğrenebilirsiniz</a> .",
		ukUA: "🧐 Ви можете <a href='https://t.me/SneatDevDiaries/4'>дізнатися, як ми розробляємо</a> цього бота та інші додатки, на нашому каналі @SneatDevDiaries.",
		uzUZ: "🧐 Ushbu bot va boshqa ilovalarni <a href='https://t.me/SneatDevDiaries/4'>qanday ishlab chiqishimizni @SneatDevDiaries kanalimizda bilib</a> olishingiz mumkin.",
		zhCN: "🧐 您可以在我们的频道@SneatDevDiaries 上<a href='https://t.me/SneatDevDiaries/4'>了解我们如何开发</a>这个机器人和其他应用程序。"},
	AdCuriosAboutHowItIsDeveloped: {
		arEG: "🧐 هل ترغب بمعرفة كيفية تطوير هذا البوت؟ تابعونا على قناة المطورين @SneatDevDiaries.",
		deDE: "🧐 Bist du neugierig, wie dieser Bot entwickelt wird? Erhalte Einblicke auf unserem Entwicklerkanal @SneatDevDiaries.",
		enUK: "🧐 Are you curious how this bot is developed? Get insights at our devs channel @SneatDevDiaries.",
		esES: "¿Te interesa saber cómo se desarrolla este bot? Descubre más en nuestro canal de desarrolladores @SneatDevDiaries.",
		faIR: "🧐 آیا کنجکاو هستید که این ربات چگونه توسعه داده شده است؟ برای کسب اطلاعات بیشتر به کانال توسعه‌دهندگان ما @SneatDevDiaries مراجعه کنید.",
		frFR: "🧐 Vous êtes curieux de savoir comment ce bot est développé ? Découvrez-en plus sur notre chaîne dédiée aux développeurs @SneatDevDiaries.",
		idID: "Penasaran bagaimana bot ini dikembangkan? Dapatkan wawasannya di kanal pengembang kami @SneatDevDiaries.",
		itIT: "🧐 Sei curioso di sapere come viene sviluppato questo bot? Scopri di più sul nostro canale dedicato agli sviluppatori @SneatDevDiaries.",
		jaJP: "🧐 このボットがどのように開発されているか興味がありますか？開発者チャンネル @SneatDevDiaries で詳細をご覧ください。",
		koKR: "🧐 이 봇이 어떻게 개발되었는지 궁금하세요? 저희 개발자 채널 @SneatDevDiaries에서 더 많은 정보를 얻어보세요.",
		plPL: "🧐 Jesteście ciekawi, jak powstaje ten bot? Dowiedzcie się więcej na naszym kanale dla deweloperów @SneatDevDiaries.",
		ptBR: "🧐 Curioso para saber como este bot foi desenvolvido? Descubra mais detalhes no nosso canal de desenvolvedores @SneatDevDiaries.",
		ptPT: "🧐 Curioso para saber como foi desenvolvido este bot? Descubra mais detalhes no nosso canal de programadores @SneatDevDiaries.",
		ruRU: "🧐 Вам любопытно как этот бот разрабатывается? Инсайты на канале наших разработчиков @SneatDevDiaries.",
		trTR: "🧐 Bu botun nasıl geliştirildiğini merak ediyor musunuz? Geliştirici kanalımız @SneatDevDiaries&#39;den fikir edinin.",
		ukUA: "🧐 Вам цікаво, як розробляється цей бот? Отримайте аналітику на нашому каналі розробників @SneatDevDiaries.",
		uzUZ: "🧐 Bu bot qanday ishlab chiqilganiga qiziqasizmi? Bizning ishlab chiquvchilar kanalimiz @SneatDevDiaries haqida ma&#39;lumot oling.",
		zhCN: "🧐 好奇这款机器人是如何开发的吗？欢迎访问我们的开发者频道 @SneatDevDiaries 获取更多见解。"},
	AdSupportUs: {
		arEG: "🙏 يمكنك /support_us للمساعدة في تطوير هذا الروبوت.",
		deDE: "🙏 Sie können uns per /support_us bei der Entwicklung dieses Bots helfen.",
		enUK: "🙏 You can /support_us to help with development of this bot.",
		esES: "🙏 Puedes usar /support_us para ayudar con el desarrollo de este bot.",
		faIR: "🙏 شما می‌توانید با استفاده از /support_us به توسعه‌ی این ربات کمک کنید.",
		frFR: "🙏 Vous pouvez /support_us pour aider au développement de ce bot.",
		idID: "🙏 Anda dapat /support_us untuk membantu pengembangan bot ini.",
		itIT: "🙏 Puoi /support_us per aiutare nello sviluppo di questo bot.",
		jaJP: "🙏 このボットの開発に協力したい場合は /support_us してください。",
		koKR: "🙏 이 봇의 개발에 도움을 주시려면 /support_us를 눌러주세요.",
		plPL: "🙏 Możesz wesprzeć nas w rozwoju tego bota.",
		ptBR: "🙏 Você pode /support_us para ajudar no desenvolvimento deste bot.",
		ptPT: "🙏 Pode /support_us para ajudar no desenvolvimento deste bot.",
		ruRU: "🙏 Вы можете поддержать разработку бота небольшим донатом - /support_us",
		trTR: "🙏 Bu botun geliştirilmesine yardımcı olmak için /support_us adresini kullanabilirsiniz.",
		ukUA: "🙏 Ви можете /підтримати_нас, щоб допомогти з розробкою цього бота.",
		uzUZ: "🙏 Ushbu botni ishlab chiqishda yordam berish uchun /support_us ga murojaat qilishingiz mumkin.",
		zhCN: "🙏 您可以通过 /support_us 来帮助开发这个机器人。"},
	AdStarGiveaways: {
		arEG: "🌟 اشترك في @StarGiveaways_EN للمشاركة في المسابقات المنتظمة لنجوم Telegram ⭐⭐⭐",
		deDE: "🌟 Abonnieren Sie @StarGiveaways_EN, um an regelmäßigen Giveaways von Telegram-Stars teilzunehmen ⭐⭐⭐",
		enUK: "🌟 Subscribe to @StarGiveaways_EN to participate in regular giveaways of Telegram stars ⭐⭐⭐",
		esES: "🌟 Suscríbete a @StarGiveaways_ES para participar en sorteos regulares de estrellas de Telegram ⭐⭐⭐",
		faIR: "🌟 برای شرکت در قرعه‌کشی‌های منظم ستاره‌های تلگرام، در @StarGiveaways_EN مشترک شوید ⭐⭐⭐",
		frFR: "🌟 Abonnez-vous à @StarGiveaways_EN pour participer régulièrement aux concours des stars de Telegram ⭐⭐⭐",
		idID: "🌟 Berlangganan @StarGiveaways_EN untuk berpartisipasi dalam hadiah rutin bintang Telegram ⭐⭐⭐",
		itIT: "🌟 Iscriviti a @StarGiveaways_EN per partecipare ai giveaway regolari delle stelle di Telegram ⭐⭐⭐",
		jaJP: "🌟 @StarGiveaways_EN に登録して、Telegram スターの定期的なプレゼントにご参加ください ⭐⭐⭐",
		koKR: "🌟 @StarGiveaways_EN을 구독하고 Telegram 스타 정기 경품 이벤트에 참여하세요 ⭐⭐⭐",
		plPL: "🌟 Subskrybuj @StarGiveaways_EN, aby brać udział w regularnych rozdaniach gwiazd Telegramu ⭐⭐⭐",
		ptBR: "🌟 Inscreva-se em @StarGiveaways_EN para participar de sorteios regulares de estrelas do Telegram ⭐⭐⭐",
		ptPT: "🌟 Inscreva-se em @StarGiveaways_EN para participar em sorteios regulares de estrelas do Telegram ⭐⭐⭐",
		ruRU: "🌟 Подпишитесь на @StarGiveaways_RU чтобы учавствовать в регулярных раздачах Телеграм звёзд ⭐⭐⭐",
		trTR: "🌟 Telegram yıldızlarının düzenli çekilişlerine katılmak için @StarGiveaways_EN&#39;e abone olun ⭐⭐⭐",
		ukUA: "🌟 Підпишіться на @StarGiveaways_EN, щоб брати участь у регулярних розіграшах зірок Telegram ⭐⭐⭐",
		uzUZ: "🌟 Telegram yulduzlarining muntazam sovg&#39;alarida qatnashish uchun @StarGiveaways_EN ga obuna bo&#39;ling ⭐⭐⭐",
		zhCN: "🌟 订阅@StarGiveaways_EN 即可参与 Telegram 星星的定期赠送活动⭐⭐⭐"},
	InlineUserProfileMessageTextTitle: {
		arEG: "<b>{USER_TITLE}</b> | الملف الشخصي @{BOT_CODE}",
		deDE: "<b>{USER_TITLE}</b> | Profil @{BOT_CODE}",
		enUK: "<b>{USER_TITLE}</b> | Profile @{BOT_CODE}",
		esES: "<b>{USER_TITLE}</b> | Perfil @{BOT_CODE}",
		faIR: "<b>{USER_TITLE}</b> | پروفایل @{BOT_CODE}",
		frFR: "<b>{USER_TITLE}</b> | Profil @{BOT_CODE}",
		idID: "<b>{USER_TITLE}</b> | Profil @{BOT_CODE}",
		itIT: "<b>{USER_TITLE}</b> | Profilo @{BOT_CODE}",
		jaJP: "<b>{USER_TITLE}</b> | プロフィール @{BOT_CODE}",
		koKR: "<b>{USER_TITLE}</b> | 프로필 @{BOT_CODE}",
		plPL: "<b>{USER_TITLE}</b> | Profil @{BOT_CODE}",
		ptBR: "<b>{USER_TITLE}</b> | Perfil @{BOT_CODE}",
		ptPT: "<b>{USER_TITLE}</b> | Perfil @{BOT_CODE}",
		ruRU: "<b>{USER_TITLE}</b> | Профиль @{BOT_CODE}",
		trTR: "<b>{USER_TITLE}</b> | Profil @{BOT_CODE}",
		ukUA: "<b>{USER_TITLE}</b> | Профіль @{BOT_CODE}",
		uzUZ: "<b>{USER_TITLE}</b> | Profil @{BOT_CODE}",
		zhCN: "<b>{USER_TITLE}</b> | 个人资料 @{BOT_CODE}"},
	InlineUserProfileMessageSpotsLabel: {
		arEG: "<b>الأماكن المفضلة</b> : %s",
		deDE: "<b>Lieblingsorte</b> : %s",
		enUK: "<b>Favorite spots</b>: %s",
		esES: "<b>Lugares favoritos</b> : %s",
		faIR: "<b>جاهای مورد علاقه</b> : %s",
		frFR: "<b>Endroits préférés</b> : %s",
		idID: "<b>Tempat favorit</b> : %s",
		itIT: "<b>Posti preferiti</b> : %s",
		jaJP: "<b>お気に入りのスポット</b>: %s",
		koKR: "<b>좋아하는 장소</b> : %s",
		plPL: "<b>Ulubione miejsca</b> : %s",
		ptBR: "<b>Lugares favoritos</b> : %s",
		ptPT: "<b>Locais favoritos</b> : %s",
		ruRU: "<b>Избранные места</b>: %s",
		trTR: "<b>Favori mekanlar</b> : %s",
		ukUA: "<b>Улюблені місця</b> : %s",
		uzUZ: "<b>Sevimli joylar</b> : %s",
		zhCN: "<b>最喜欢的地方</b>：%s"},
	InlineUserProfileMessageFollowersLabel: {
		arEG: "<b>المتابعون</b> : %s",
		deDE: "<b>Follower</b> : %s",
		enUK: "<b>Followers</b>: %s",
		esES: "<b>Seguidores</b> : %s",
		faIR: "<b>دنبال‌کنندگان</b> : %s",
		frFR: "<b>Abonnés</b> : %s",
		idID: "<b>Pengikut</b> : %s",
		itIT: "<b>Seguaci</b> : %s",
		jaJP: "<b>フォロワー</b>: %s",
		koKR: "<b>팔로워</b> : %s",
		plPL: "<b>Obserwujący</b> : %s",
		ptBR: "<b>Seguidores</b> : %s",
		ptPT: "<b>Seguidores</b> : %s",
		ruRU: "<b>Подписчиков</b>: %s",
		trTR: "<b>Takipçiler</b> : %s",
		ukUA: "<b>Підписники</b> : %s",
		uzUZ: "<b>Obunachilar</b> : %s",
		zhCN: "<b>关注者</b>：%s"},
	SpotUserPlanRow: {
		arEG: "<code>#%d</code> %s، <b>%s</b> من <i>%s</i> حتى <i>%s</i> - <u>%s</u>",
		deDE: "<code>#%d</code> %s, <b>%s</b> von <i>%s</i> bis <i>%s</i> - <u>%s</u>",
		enUK: "<code>#%d</code> %s, <b>%s</b> from <i>%s</i> till <i>%s</i> - <u>%s</u>",
		esES: "<code>#%d</code> %s, <b>%s</b> desde <i>%s</i> hasta <i>%s</i> - <u>%s</u>",
		faIR: "<code>#%d</code> %s، <b>%s</b> از <i>%s</i> تا <i>%s</i> - <u>%s</u>",
		frFR: "<code>#%d</code> %s, <b>%s</b> de <i>%s</i> à <i>%s</i> - <u>%s</u>",
		idID: "<code>#%d</code> %s, <b>%s</b> dari <i>%s</i> sampai <i>%s</i> - <u>%s</u>",
		itIT: "<code>#%d</code> %s, <b>%s</b> <i>da</i> %s <i>a %s</i> - <u>%s</u>",
		jaJP: "<code>#%d</code> %s, <b>%s</b> <i>%s</i>から<i>%s</i>まで - <u>%s</u>",
		koKR: "<code>#%d</code> %s, <b>%s</b> ( <i>%s</i> 부터 <i>%s</i> - <u>%s</u> 까지)",
		plPL: "<code>#%d</code> %s, <b>%s</b> od <i>%s</i> do <i>%s</i> - <u>%s</u>",
		ptBR: "<code>#%d</code> %s, <b>%s</b> de <i>%s</i> até <i>%s</i> - <u>%s</u>",
		ptPT: "<code>#%d</code> %s, <b>%s</b> de <i>%s</i> até <i>%s</i> - <u>%s</u>",
		ruRU: "<code>#%d</code> %s, <b>%s</b> с <i>%s</i> по <i>%s</i> - <u>%s</u>",
...
к примеру <code>%s</code> для позавчера.`,
	...
	EventiusHostRsvpReceivedSubject: {
		arEG: "رد جديد على فعاليتك",
		deDE: "Neue Antwort auf deine Veranstaltung",
		enUK: "New RSVP for your event",
		esES: "Nueva respuesta para tu evento",
		faIR: "پاسخ جدید به رویداد شما",
		frFR: "Nouvelle réponse à votre événement",
		idID: "Tanggapan baru untuk acara Anda",
		itIT: "Nuova risposta per il tuo evento",
		jaJP: "イベントへの新しい出欠回答",
		koKR: "이벤트에 대한 새로운 참석 응답",
		plPL: "Nowa odpowiedź na Twoje wydarzenie",
		ptBR: "Nova resposta para o seu evento",
		ptPT: "Nova resposta para o seu evento",
		ruRU: "Новый ответ на ваше событие",
		trTR: "Etkinliğiniz için yeni yanıt",
		ukUA: "Нова відповідь на ваш захід",
		uzUZ: "Tadbiringiz uchun yangi javob",
		zhCN: "您活动的新回复"},
	EventiusHostRsvpReceived: {
		arEG: "%s ردّ على فعاليتك: %s",
		deDE: "%s hat auf deine Veranstaltung geantwortet: %s",
		enUK: "%s responded to your event: %s",
		esES: "%s respondió a tu evento: %s",
		faIR: "%s به رویداد شما پاسخ داد: %s",
		frFR: "%s a répondu à votre événement : %s",
		idID: "%s menanggapi acara Anda: %s",
		itIT: "%s ha risposto al tuo evento: %s",
		jaJP: "%s さんがあなたのイベントに回答しました：%s",
		koKR: "%s님이 회원님의 이벤트에 응답했습니다: %s",
		plPL: "%s odpowiedział(a) na Twoje wydarzenie: %s",
		ptBR: "%s respondeu ao seu evento: %s",
		ptPT: "%s respondeu ao seu evento: %s",
		ruRU: "%s ответил(а) на ваше событие: %s",
		trTR: "%s etkinliğinize yanıt verdi: %s",
		ukUA: "%s відповів(ла) на ваш захід: %s",
		uzUZ: "%s tadbiringizga javob berdi: %s",
		zhCN: "%s 回复了您的活动：%s"},
	EventiusHostRsvpReceivedNoName: {
		arEG: "رد جديد على فعاليتك: %s",
		deDE: "Neue Antwort auf deine Veranstaltung: %s",
		enUK: "New RSVP for your event: %s",
		esES: "Nueva respuesta para tu evento: %s",
		faIR: "پاسخ جدید به رویداد شما: %s",
		frFR: "Nouvelle réponse à votre événement : %s",
		idID: "Tanggapan baru untuk acara Anda: %s",
		itIT: "Nuova risposta per il tuo evento: %s",
		jaJP: "あなたのイベントへの新しい回答：%s",
		koKR: "회원님의 이벤트에 대한 새로운 응답: %s",
		plPL: "Nowa odpowiedź na Twoje wydarzenie: %s",
		ptBR: "Nova resposta para o seu evento: %s",
		ptPT: "Nova resposta para o seu evento: %s",
		ruRU: "Новый ответ на ваше событие: %s",
		trTR: "Etkinliğiniz için yeni yanıt: %s",
		ukUA: "Нова відповідь на ваш захід: %s",
		uzUZ: "Tadbiringiz uchun yangi javob: %s",
		zhCN: "您活动的新回复：%s"},
	EventiusRsvpAnswerGoing: {
		arEG: "سيحضر",
		deDE: "nimmt teil",
		enUK: "going",
		esES: "asistirá",
	...
	EventiusRsvpAnswerDeclined: {
		...
		koKR: "불참",
		plPL: "nie będzie",
		ptBR: "não vai comparecer",
		ptPT: "não vai comparecer",
		ruRU: "не придёт",
		trTR: "katılmıyor",
		ukUA: "не прийде",
		uzUZ: "kelmaydi",
